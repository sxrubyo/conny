...
const bubleeHome = process.env.BUBLEE_HOME || path.join(os.homedir(), ".bublee");
const repoDir = path.join(bubleeHome, "repo");
const runtimeDir = path.join(bubleeHome, "runtime");
const workspaceConfigPath = path.join(bubleeHome, "config.json");
const sharedTelegramRoutesPath = path.join(bubleeHome, "shared_telegram_routes.json");
const entrypoint = path.join(repoDir, "bublee_app.py");
const legacyEntrypoint = path.join(repoDir, "bublee_cli.py");
...
function printBanner() {
  console.log();
  console.log(chalk.hex('#EC4899').bold('  bublee-agent') + 
...
function installRuntimeDependencies(runtime, spinner, allowFullFailure = true) {
  let result;
  if (spinner) {
    spinner.text = chalk.hex('#8B5CF6')("Installing Bublee CLI runtime dependencies...");
  }
  result = spawnSync(
    runtime,
  ...
  if (result.status !== 0) {
    if (spinner) spinner.fail(chalk.red("✕ Could not install Bublee's required Python packages."));
...
function ensureRuntime(spinner) {
  ...
  if (!localSpinner) {
    localSpinner = ora({
      text: chalk.hex('#8B5CF6')("Iniciando Bublee..."),
      color: "magenta"
    }).start();
  }
  ...
  if (!python) {
    const errMsg = process.platform === "win32"
      ? "✕ No encontré Python 3.9+ en este host. Instálalo y vuelve a ejecutar `bublee`."
      : "✕ No encontré python3/python 3.9+ en este host. Instálalo y vuelve a ejecutar `bublee`.";
...
function bootstrapFromPackage() {
  ...
  try {
    ...
    ensureDir(path.join(bubleeHome, "instances"));
...
function needsBootstrap() {
  ...
  return process.env.BUBLEE_FORCE_SYNC === "1";
}

function execBublee(argv) {
  const runtime = resolveRuntime();
  ...
  const result = runAndReturn(
    ...
    [entrypoint, ...argv],
    {
      BUBLEE_HOME: bubleeHome,
      BUBLEE_DIR: repoDir,
      INSTANCES_DIR: process.env.INSTANCES_DIR || path.join(bubleeHome, "instances"),
      BUBLEE_BACKUPS: process.env.BUBLEE_BACKUPS || path.join(bubleeHome, "backups"),
      BUBLEE_SHARED_TELEGRAM_ROUTES:
        process.env.BUBLEE_SHARED_TELEGRAM_ROUTES || sharedTelegramRoutesPath,
      BUBLEE_WORKSPACE_CONFIG: process.env.BUBLEE_WORKSPACE_CONFIG || workspaceConfigPath,
    }
  );
...
if (isVersion) {
  if (isJson) {
    ...
    console.log(`bublee ${packageVersion}`);
  }
  process.exit(0);
}
...
if (isBootstrapCheck) {
  const runtime = resolveRuntime();
  if (!runtime || !runtimeLooksHealthy(runtime)) {
    fail(`Bublee runtime is not ready in ${bubleeHome}`);
  }
  console.log(`bublee runtime ok ${packageVersion}`);
  process.exit(0);
}
...
if (!execBublee(launchArgs)) {
  fail(`No pude iniciar Bublee desde ${bubleeHome}`);
}
