#!/usr/bin/env python3
"""bublee_demo_v2.py — Ultra-realistic demo with Colombian personas and memory."""
...
MASTER_KEY = os.getenv("MASTER_API_KEY", "bublee_master_2026_santiago")
...
async def send_to_bublee(message: str, chat_id: str) -> Dict:
    """Send message to Bublee API."""
...
async def run_demo(persona_key: str = "dona_carmen", instance_id: str = "default"):
    ...
    print()
    print(color("╔══════════════════════════════════════════════╗", "1;36"))
    print(color("║  BUBLEE DEMO v2.0 — Simulación Realista     ║", "1;36"))
    print(color("╚══════════════════════════════════════════════╝", "1;36"))
    ...
    for i, msg in enumerate(persona["messages"]):
        ...
        # Get Bublee's response
        try:
            result = await send_to_bublee(display_msg, chat_id)
            bubbles = result.get("bubbles", [result.get("response", "error")])
            # Simulate Bublee "thinking"
            await asyncio.sleep(random.uniform(1.0, 2.5))
            for bubble in bubbles:
                print(f"  {color('[BUBLEE]', '1;35')} {bubble}")
                if len(bubbles) > 1:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Bublee Demo v2 — Realistic Colombian personas")
