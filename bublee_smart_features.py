"""bublee_smart_features.py — 10 power features for human-like intelligence."""
...
log = logging.getLogger("bublee.smart")
...
def is_conversation_ending(text: str) -> bool:
    ...
    t = text.lower().strip()
    if "?" in t or any(w in t for w in ["precio", "cuánto", "cuanto", "cuesta", "cita", "agendar", "valoración", "tratamiento", "servicio", "información", "informacion"]):
        return False
    if len(t) > 35:
        return False
...
class WeeklyStats:
    ...
    def calculate(self, db_path: str = "bublee.db") -> Dict[str, int]:
