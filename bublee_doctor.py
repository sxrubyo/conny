#!/usr/bin/env python3
"""bublee_doctor.py — Health check + auto-heal for Bublee instances."""
...
class BubleeDoctor:
    ...
    async def _check_llm_latency(self):
        port = self._get_port()
        key = os.getenv("MASTER_API_KEY", "bublee_master_2026_santiago")
        try:
            start = time.time()
    ...
    async def _check_persona(self):
        persona_paths = [
            Path(f"/home/ubuntu/bublee/instances/{self.instance_id}/personas/persona.yaml"),
            Path(f"/home/ubuntu/bublee/personas/{self.instance_id}/persona.yaml"),
            Path(f"/home/ubuntu/bublee/personas/{self.instance_id}/runtime_override.json"),
        ]
    ...
    def _get_port(self) -> int:
        ...
        env_path = Path(f"/home/ubuntu/bublee/instances/{self.instance_id}/.env")
        if env_path.exists():
            for line in env_path.read_text().splitlines():
                if line.startswith("PORT="):
                    return int(line.split("=")[1].strip())
        # Default: main instance on 8001
        if self.instance_id in ("default", "bublee"):
            return 8001
        return 8003
    ...
    def print_report(self):
        print()
        print(bold(f"🔍 Bublee Doctor — {self.instance_id}"))
        print("─" * 45)
    ...
    async def auto_heal(self):
        ...
        for check in self.checks:
            ...
            if "PM2" in check.name and "not found" not in check.message:
                try:
                    subprocess.run(["pm2", "restart", f"bublee-{self.instance_id}"], capture_output=True, timeout=10)
                    healed.append(f"Restarted PM2: bublee-{self.instance_id}")
                except:
                    pass
        if healed:
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(prog="bublee doctor", description="Health check for Bublee instances")
    parser.add_argument("instance", nargs="?", default="bublee", help="Instance ID to check")
    parser.add_argument("--fix", action="store_true", help="Attempt auto-heal for errors")
    parser.add_argument("--json", action="store_true", help="Output as JSON")
    args = parser.parse_args()

    doctor = BubleeDoctor(args.instance)
