#!/usr/bin/env python3
"""
Chat interactivo continuo con Bublee via HTTP.
...
BUBLEE_URL = "http://localhost:8001"
MASTER_KEY = "bublee_master_2026_santiago"

def chat_loop(user_id: str):
    """Loop de conversación interactiva con Bublee."""
    
    print("""
╔══════════════════════════════════════════════════════════╗
║         💬 CHAT CONTINUO CON BUBLEE v8.0               ║
...
def send_message(session, text: str, user_id: str) -> str:
    """Envía mensaje a Bublee y retorna respuesta."""
    try:
        resp = session.post(
            f"{BUBLEE_URL}/test",
...
if __name__ == "__main__":
    ...
    print(f"🔗 Conectando a Bublee en {BUBLEE_URL}...")
    chat_loop(user_id)