#!/usr/bin/env python3
"""bublee_persona_cli.py — Agency persona control CLI."""
import argparse
import json
import os
import sys
...
PERSONAS_DIR = Path("/home/ubuntu/bublee/personas")
...
def _get_key():
    return os.getenv("ADMIN_API_KEY", os.getenv("MASTER_API_KEY", "bublee_master_2026_santiago"))
...
def cmd_test(args):
    ...
    for msg in test_messages:
        try:
            ...
            data = r.json()
            print(f"  [USER] {msg}")
            print(f"  [BUBLEE] {data.get('response', 'error')[:120]}")
            print()
        except Exception as e:
            print(f"  [ERROR] {e}")
...
def main():
    parser = argparse.ArgumentParser(prog="bublee persona", description="Agency persona control")
