"""
nova_bridge.py — Puente entre Nova y Bublee

Nova es el sistema nervioso de Bublee:
  - Antes de enviar cualquier mensaje, Bublee pregunta a Nova: "¿puedo?"
...
Cómo funciona:
  1. BubleeGuard intercepta cada mensaje ANTES de enviarlo
  2. Llama a Nova /validate con la acción
  3. APPROVED → mensaje se envía normalmente
  4. BLOCKED → Bublee responde algo alternativo, no el mensaje bloqueado
  5. ESCALATED → Bublee le pregunta al admin antes de enviar

Requisitos:
  - Nova server corriendo (puerto 9002 por defecto)
  - NOVA_TOKEN configurado en .env (token del agente "Bublee")
  - NOVA_URL=http://localhost:9002

Sin Nova activo → Bublee funciona normal (modo degradado seguro)
"""
...
NOVA_TOKEN    = os.getenv("NOVA_TOKEN",    "")   # Token del agente Bublee en Nova
...
class NovaClient:
    ...
    def _headers(self) -> Dict:
        h = {"Content-Type": "application/json", "User-Agent": "bublee/5.0"}
        if self.api_key:
    ...
    async def create_agent_rule(
        ...
        """
        Crea o actualiza el agente Bublee en Nova con nuevas reglas.
        Se llama cuando el admin agrega una nueva regla en lenguaje natural.
        """
        payload = {
            "agent_name":    agent_name,
            "description":   "Bublee — recepcionista virtual de clínica estética",
            "can_do":        can_do,
            "cannot_do":     cannot_do,
...
class BubleeGuard:
    """
    Interceptor principal entre Bublee y sus mensajes.
    ...
    async def should_send(
        ...
        """
        Decide si Bublee puede enviar este mensaje.
...
async def nl_to_nova_rules(
    instruction: str,
    llm_complete_fn,  # función llm_engine.complete de bublee
    existing_rules: Dict = None
) -> Dict:
    """
    Traduce una instrucción en lenguaje natural a reglas de Nova.
    
    Ej: "No permitas que Bublee le envíe precios a clientes con menos de 3 visitas"
    ...
    prompt = f"""Eres un motor de políticas para Bublee, una recepcionista virtual de clínica estética.
...
async def setup_bublee_agent(
    client: NovaClient,
    clinic_name: str,
    agent_name: str = "Bublee"
) -> Optional[str]:
    """
    Crea el agente Bublee en Nova con reglas base para clínicas estéticas.
    ...
    result = await client.create_agent_rule(
        agent_name=f"Bublee - {clinic_name}",
        can_do=can_do,
        cannot_do=cannot_do,
        authorized_by="admin"
    )
    ...
    if "error" not in result:
        ...
        log.info(f"[nova] agente Bublee creado: {token_id[:20]}...")
        return token_id
    return None
...
_guard: Optional[BubleeGuard] = None
_client: Optional[NovaClient] = None


def init_nova() -> BubleeGuard:
...
def get_guard() -> Optional[BubleeGuard]:
    return _guard
