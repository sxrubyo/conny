...
log = logging.getLogger("bublee.handoff")
...
def _format_admin_notification(ticket: HandoffTicket) -> str:
    ...
    if ticket.context:
        ...
        for turn in ticket.context[-4:]:
            role_label = "Cliente" if turn.get("role") == "user" else "Bublee"
...
def _looks_like_handoff_system_echo(text: str) -> bool:
    ...
    return normalized.startswith("⚡ *bublee necesita tu ayuda*".lower()) or normalized.startswith(
...
async def _polish_admin_reply(
    ...
    system = f"""Eres Bublee, la agente de inteligencia artificial de {clinic_name}.
...
Responde SOLO con el mensaje final para el cliente."""
    ...
    user_prompt = (
        ...
        "Transforma en respuesta natural de Bublee:"
    )
...
class SmartAdminHandoff:
    ...
    async def try_intercept_admin_reply(
        ...
        *,
        ...
        # Bublee aprende de la respuesta del admin (V9)
        try:
        ...
        self._mark_replied(ticket_id, admin_raw=text, bublee_reply=polished)
        ...
        try:
            ...
            return [
                HandoffLearning(
                    ...
                    bublee_polish=row["bublee_polish"],
    ...
    def _clone_ticket(self, ticket: HandoffTicket, *, admin_chat_id: Optional[str] = None) -> HandoffTicket:
        return HandoffTicket(
            ...
            bublee_reply=ticket.bublee_reply,
    ...
    def _row_to_ticket(self, row: sqlite3.Row) -> HandoffTicket:
        ...
        return HandoffTicket(
            ...
            bublee_reply=row["bublee_reply"] or "",
    ...
    def _mark_replied(self, ticket_id: str, admin_raw: str, bublee_reply: str) -> None:
        try:
            conn = _get_db()
            conn.execute(
                """
                ...
                """,
                (admin_raw, bublee_reply, time.time(), ticket_id),
            )
            conn.commit()
            conn.close()
        except Exception as exc:
        ...
        try:
            ...
            if existing:
                conn.execute(
                    ...
                    SET admin_raw = ?, bublee_polish = ?, learned_at = ?
                    WHERE id = ?
                    """,
                    (admin_raw, bublee_polish, time.time(), existing["id"]),
                )
            else:
                conn.execute(
                    """
                    INSERT INTO handoff_learnings
                    (id, clinic_key, question_norm, admin_raw, bublee_polish, learned_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (str(uuid.uuid4()), clinic_key, q_norm, admin_raw, bublee_polish, time.time()),
                )
...
async def handle_handoff_admin_command(
    ...
    if cmd in ("/handoff-aprendizajes", "/handoff-learnings", "/lo-que-aprendi"):
        ...
        if not learnings:
            return ["Bublee aún no ha aprendido nada vía handoff para esta clínica."]
        ...
        for learning in learnings[:10]:
            lines.append(
                ...
                f"  Respuesta: _{learning.bublee_polish[:80]}_"
            )
        return ["\n".join(lines)]
