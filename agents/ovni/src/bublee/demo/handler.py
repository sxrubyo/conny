...
"""
    ...
    if _waiting_url_confirm and not detected_cmd:
        ...
        if _is_url_yes:
            # Confirmado — cargar info y arrancar demo
            ...
            if _found_ctx and len(_found_ctx.strip()) > 60:
                # Generar confirmación con datos reales ahora que el dueño confirmó
...
SIN mayúscula inicial (salvo nombres propios). Sin punto final. Sin emojis. Sin ¿ ni ¡. Máximo 1 oración por burbuja."""
    ...
    if _waiting_demo and not detected_cmd:
        _text_clean = text.lower().strip().rstrip(".")
        ...
        if not is_confirm:
            try:
                ...
                is_confirm = confirm_check and "SI" in confirm_check.upper() and "NO" not in confirm_check.upper()[:10]
            except Exception:
                is_confirm = False
