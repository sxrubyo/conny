"""FastAPI routes extracted from bublee.py monolith."""
from __future__ import annotations
from src.domain.prompts.human_writer import STYLE_GUIDELINES
import logging
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from contextlib import asynccontextmanager

log = logging.getLogger("bublee.api.routes")

@asynccontextmanager
async def lifespan(app: FastAPI):
    yield

# TODO: These routes reference globals: bublee, db, llm_engine, Config, app
...
async def test_message(request: Request):
    ...
    try:
        ...
        responses: list = await bublee.process_message(user_id, message)
        # Unir los fragmentos si hay varios (Bublee puede devolver lista)
...
class ConversationSimulator:
    ...
    async def _get_bublee_response(self, user_msg: str, history: List[Dict],
        ...
        if scope_mode == "meta":
            ...
            return f"Hola, soy Bublee, la asesora virtual de {clinic_name} ||| dime qué te gustaría revisar"
        ...
        if is_first_turn:
            first_turn_block = """
PRIMER TURNO:
- preséntate corto como Bublee, la asesora virtual del negocio
- no abras con "hola"
...
"""
        system = f"""Eres Bublee, la asesora virtual de {clinic.get('name','una clínica')}.
...
Bublee: Hola, soy Bublee, la asesora virtual de la clínica ||| qué es lo que más te preocupa, que se note demasiado o quedar tiesa
{addon}"""
...
class AdminIntelligentBriefing:
    ...
    async def _generate_recommendation(self, new_convs: int, new_apts: int,
        ...
        prompt = f"""{STYLE_GUIDELINES}\n\nEres el coach de ventas de una clínica. Basado en estos datos:
...
class AIDiagnostician:
    ...
    async def diagnose(self, limit=15):
        ...
        prompt = f"""{STYLE_GUIDELINES}\n\nEres analista de agentes IA para negocios en Colombia.
