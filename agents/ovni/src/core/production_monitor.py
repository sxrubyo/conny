...
class BubleeProduction:
    ...
    async def _generate_admin_alert_via_llm(
        ...
        try:
            ...
            if reason == "first_contact":
                return f"Hola! El paciente ...{patient_display} me escribió por primera vez, pero aún no tengo suficiente información configurada para atenderle. ¡Por favor enséñame!"
            else:
                return f"Hola! El paciente ...{patient_display} preguntó: \"{patient_text}\". No tengo esta información confirmada, ¿qué le respondo?"
    ...
    async def handle(self, chat_id: str, text: str, clinic: Dict,
        ...
        try:
            ...
            if should_esc or wants_human:
                ...
                if admin_ids:
                    ...
                    alert = f"oye, un paciente (...{patient_display}) {reason_text}:\n\"{text[:150]}\""
                    _esc_notified = 0
        ...
        support_for_answer = "\n".join(
            part for part in (soul_context, kb_context, teachings_context, admin_rules_context)
            if part
        )
