...
CÓMO USAR — al inicio de bublee.py (después de los imports opcionales):
  try:
      from bublee_nuke_robot_phrases import apply_patch
      apply_patch()
...
log = logging.getLogger("bublee.nuke_robot")
...
# ════════════════════════════════════════════════════════════════════════════════
# PATCH DE ARCHIVO — modifica bublee.py directamente (uso offline)
# ════════════════════════════════════════════════════════════════════════════════
...
def patch_file(filepath: str) -> bool:
    """
    Aplica el patch directamente al archivo bublee.py.
    Reemplaza el bloque _robot_phrases completo por la versión mínima.

    Usar offline antes de deployar:
        python3 -c "from bublee_nuke_robot_phrases import patch_file; patch_file('bublee.py')"
    """
    try:
...
def apply_patch() -> bool:
    ...
    Llama esto al inicio de bublee.py:
        from bublee_nuke_robot_phrases import apply_patch
        apply_patch()
    """
    patched = 0
...
# ════════════════════════════════════════════════════════════════════════════════
# USO DIRECTO: aplicar sobre bublee.py localmente
# ════════════════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    target = Path(_sys.argv[1]) if len(_sys.argv) > 1 else Path("bublee.py")
    ...
    if success:
        ...
        print("   Reinicia Bublee para que tome efecto.")
    else:
