#!/usr/bin/env python3
"""
bublee_i18n.py — Multilingual support for Bublee
...
def _workspace_config_path() -> Path:
    explicit = os.environ.get("BUBLEE_WORKSPACE_CONFIG")
    if explicit:
        return Path(explicit)
    bublee_home = os.environ.get("BUBLEE_HOME")
    if bublee_home:
        return Path(bublee_home) / "config.json"
    return Path.home() / ".bublee" / "config.json"
...
def _load_persisted_language() -> str:
    for candidate in (
        os.environ.get("BUBLEE_LANG"),
        os.environ.get("BUBLEE_UI_LANG"),
        os.environ.get("BUBLEE_INIT_LANG"),
    ):
...
def _persist_language(lang: str) -> None:
    if lang not in SUPPORTED_LANGUAGES:
        return
    os.environ["BUBLEE_LANG"] = lang
    os.environ["BUBLEE_UI_LANG"] = lang
    try:
        path = _workspace_config_path()
...
def _es() -> TranslationSet:
    return TranslationSet(
        ui={
            ...
            "test": "Probar",
            "secure": "Seguridad",
            "welcome": "Bienvenido a Bublee",
            "enter_choice": "Ingresa tu opción",
        },
        bot={
            "welcome": "¡Hola! 👋 Soy Bublee, tu asistente virtual. ¿En qué puedo ayudarte hoy?",
...
def _en() -> TranslationSet:
    return TranslationSet(
        ui={
            ...
            "test": "Test",
            "secure": "Security",
            "welcome": "Welcome to Bublee",
            "enter_choice": "Enter your option",
        },
        bot={
            "welcome": "Hello! 👋 I'm Bublee, your virtual assistant. How can I help you today?",
...
def _pt() -> TranslationSet:
    return TranslationSet(
        ui={
            ...
            "test": "Testar",
            "secure": "Segurança",
            "welcome": "Bem-vindo ao Bublee",
            "enter_choice": "Digite sua opção",
        },
        bot={
            "welcome": "Olá! 👋 Sou a Bublee, sua assistente virtual. Como posso ajudar hoje?",
...
def _fr() -> TranslationSet:
    return TranslationSet(
        ui={
            ...
            "test": "Tester",
            "secure": "Sécurité",
            "welcome": "Bienvenue sur Bublee",
            "enter_choice": "Entrez votre option",
        },
        bot={
            "welcome": "Bonjour! 👋 Je suis Bublee, votre assistante virtuelle. Comment puis-je vous aider aujourd'hui?",
...
def _de() -> TranslationSet:
    return TranslationSet(
        ui={
            ...
            "test": "Testen",
            "secure": "Sicherheit",
            "welcome": "Willkommen bei Bublee",
            "enter_choice": "Option eingeben",
        },
        bot={
            "welcome": "Hallo! 👋 Ich bin Bublee, Ihre virtuelle Assistentin. Wie kann ich Ihnen heute helfen?",
