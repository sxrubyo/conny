#!/usr/bin/env python3
"""
Bridge de terminal para Bublee con memoria local en SQLite.

Funciones principales:
- Conversacion interactiva y no interactiva contra /test
- Persistencia de turnos en la tabla bridge_sessions de bublee.db
...
DEFAULT_DB_PATH = ROOT_DIR / "bublee.db"
DEFAULT_ENV_PATH = ROOT_DIR / ".env"
DEFAULT_LOGS_DIR = ROOT_DIR / "logs"
DEFAULT_URL = os.getenv("BUBLEE_BRIDGE_URL", "http://localhost:8001")
DEFAULT_TIMEOUT = float(os.getenv("BUBLEE_BRIDGE_TIMEOUT", "45"))
...
class LocalBubleeTransport:
    def __init__(self) -> None:
        import bublee as bublee_module

        self._module = bublee_module
        if getattr(self._module, "bublee", None) is None:
            asyncio.run(self._module.init_bublee())

    def send(self, message: str, user_id: str) -> TransportReply:
        responses = asyncio.run(self._module.bublee.process_message(user_id, message))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="bublee_bridge")
    parser.add_argument("--url", default=DEFAULT_URL, help="Base URL de Bublee")
    parser.add_argument("--transport", choices=("auto", "http", "local"), default="auto",
                        help="Modo de conexión con Bublee")
    parser.add_argument("--db-path", default=str(DEFAULT_DB_PATH), help="Ruta a bublee.db")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.transport == "local":
        transport = LocalBubleeTransport()
    elif args.transport == "http":
        transport = BubleeTransport(args.url, master_key=master_key, timeout=args.timeout)
    else:
        try:
            transport = LocalBubleeTransport()
        except Exception:
