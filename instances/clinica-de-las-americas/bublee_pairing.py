...
BUBLEE_PAIRING_BASE_DIR = os.path.join(os.path.expanduser("~"), ".bublee", "pairing")

# Ensure the base directory exists
os.makedirs(BUBLEE_PAIRING_BASE_DIR, exist_ok=True)
...
def _get_channel_paths(channel: str) -> Tuple[str, str]:
    ...
    pairing_path = os.path.join(BUBLEE_PAIRING_BASE_DIR, f"{channel_safe}-pairing.json")
    allow_path = os.path.join(BUBLEE_PAIRING_BASE_DIR, f"{channel_safe}-allowFrom.json")
...
if __name__ == "__main__":
    ...
    try:
        os.remove(os.path.join(BUBLEE_PAIRING_BASE_DIR, f"{TEST_CHANNEL}-pairing.json"))
        os.remove(os.path.join(BUBLEE_PAIRING_BASE_DIR, f"{TEST_CHANNEL}-allowFrom.json"))
        ...
        try:
            if not os.listdir(BUBLEE_PAIRING_BASE_DIR):
                os.rmdir(BUBLEE_PAIRING_BASE_DIR)
        except OSError:
            pass
