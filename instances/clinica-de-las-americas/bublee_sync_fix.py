#!/usr/bin/env python3
"""
bublee_sync_fix.py — Parcha bublee_cli.py con 3 correcciones críticas

Bugs que corrige:
  1. INSTANCES_DIR apuntaba a ~/.bublee/instances en vez de ~/bublee-instances
...
CLI_PATH = Path(__file__).resolve().parent / "bublee_cli.py"
...
PATCHES = [
    ...
    # ── FIX 1: INSTANCES_DIR default path ────────────────────────────────────
    {
        ...
        "desc": "Corregir INSTANCES_DIR: ~/.bublee/instances → ~/bublee-instances",
        "old":  'INSTANCES_DIR = os.getenv("INSTANCES_DIR", str(Path(BUBLEE_HOME) / "instances"))',
        "new":  'INSTANCES_DIR = os.getenv("INSTANCES_DIR", str(Path.home() / "bublee/instances"))',
    },
...
SYNC_RUNTIME_DIRS = [
    ...
    # Usa 'bublee config <nombre>' para editar la persona de un cliente.
]

# ── Paths que NUNCA se tocan durante sync ──────────────────────────────────
INSTANCE_PROTECTED_PATHS = {
    ".env",
    "bublee.db", "bublee.db-shm", "bublee.db-wal",
    "bublee_ultra.db", "vectors.db", "data.db",
    "logs",
...
def _clone_runtime_entries(source_root: str, dest_root: str, entries: Optional[List[str]] = None) -> List[str]:
    ...
    # ── FIX 4: Lanzar TUI cuando bublee se corre sin args ───────────────────
    {
        "id":   "fix-tui-launch",
        "desc": "Lanzar bublee_tui cuando no hay argumentos",
    ...
    if args.help or cmd in ("help", "--help", "-h", ""):
        ...
        _tui_path = Path(__file__).resolve().parent / "bublee_tui.py"
        if sys.stdout.isatty() and _tui_path.exists():
            try:
                import importlib.util as _ilu
                _spec = _ilu.spec_from_file_location("bublee_tui", str(_tui_path))
...
def apply_patches(cli_path: Path) -> None:
    print(f"\n  {_c(BOLD, 'bublee_sync_fix')}  —  Parcheando {cli_path.name}")
    print()

    if not cli_path.exists():
        print(f"  {_c(RED, '✗')}  No encontré {cli_path}")
        print(f"     Asegúrate de correr este script desde ~/bublee/")
        sys.exit(1)
    ...
    print()
    print(f"  {_c(CYAN, 'Siguiente paso:')}  pm2 restart bublee-clinica-americas")
    print(f"  {_c(CYAN, 'Probar sync:')}    bublee sync")
    print()


def _print_manual_guide():
    print("""
  FIX 1 — INSTANCES_DIR (línea ~158 en bublee_cli.py)
  ┌──────────────────────────────────────────────────────────────────
  │ - INSTANCES_DIR = os.getenv("INSTANCES_DIR", str(Path(BUBLEE_HOME) / "instances"))
  ...
  │     if top_level in {".env","bublee.db","personas","identity","soul","logs"}:
  │         continue
  │   Y reemplaza:
  │     _remove_runtime_target(dst)
  │     shutil.copytree(src, dst)
