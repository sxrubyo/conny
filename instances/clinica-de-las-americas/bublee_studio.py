#!/usr/bin/env python3
"""bublee_studio.py — Interactive CLI session with live monitoring."""
import asyncio
import json
import os
import sys
...
from bublee_voice import BubleeVoice

STUDIO_DIR = Path.home() / ".bublee" / "studio" / "memory"
...
class BubleeStudio:
    def __init__(self, instance_id="default", master_key=None):
        ...
        self.voice = BubleeVoice()
        self.master_key = master_key or os.getenv("MASTER_API_KEY", "bublee_master_2026_santiago")
        self.history = []
    ...
    def print_header(self):
        ...
        print("\033[1;36m║  BUBLEE STUDIO v1.0                         ║\033[0m")
    ...
    async def run(self):
        ...
        while True:
            ...
            for bubble in bubbles:
                print(f"\033[1;35m[BUBLEE]\033[0m {bubble}")
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Bublee Studio — Interactive chat with monitoring")
    ...
    studio = BubleeStudio(instance_id=args.instance, master_key=args.key)
    await studio.run()
