...
bublee_brain_v10.py
════════════════════════════════════════════════════════════════════════════════
CEREBRO v10.2 — LLM PRIMERO, PLANTILLAS COMO ÚLTIMO RECURSO
════════════════════════════════════════════════════════════════════════════════

CAMBIOS v10.2 (este archivo):
  - FRUSTRATION_SIGNALS: se quitaron "y ya" y "otra vez" sueltos — eran
    demasiado genéricos y disparaban falsos positivos con clientes tranquilos
    (ej. "confirmo el jueves y ya quedamos", "vengo otra vez la otra semana").
    Se conservan las variantes específicas ("otra vez lo mismo", "dime ya").
  - patch_llm_first ahora se invoca de verdad desde bublee.py (init_bublee),
    vía auto_patch() con fallback — antes el parche estaba escrito pero
    ningún archivo subido lo llamaba.
...
CÓMO USAR (al final de bublee.py, en init o en el bloque de startup):

    try:
        from bublee_brain_v10 import patch_llm_first, init_brain
        init_brain()
...
log = logging.getLogger("bublee.brain_v10")
...
# ── Señales de frustración por respuestas repetitivas ───────────────────────
# v10.2: se quitaron "y ya" y "otra vez" sueltos (demasiado genéricos, alto
# riesgo de falso positivo con clientes tranquilos). Se conservan solo las
# variantes que son inequívocamente de frustración.
_FRUSTRATION_SIGNALS = [
    "que fastidio", "qué fastidio", "ya le dije", "ya te dije",
    "ya dije", "ya lo dije", "te lo dije",
    "no me entiendes", "no entiendes", "me repites",
    "otra vez lo mismo", "siempre lo mismo",
    "dime ya", "dime el precio",
    "que pesado", "qué pesado", "eso ya lo dije",
    "pregunta lo mismo", "mismo cuento", "das vueltas",
]
...
def extract_short_memory(history: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Lee el historial reciente y extrae:
      ...
      - repeated_question_detected: NUEVO — si Bublee hizo la misma pregunta 2+ veces
    ...
    # Para detectar preguntas repetidas de Bublee
    ...
    for msg in history:
        ...
        if role == "assistant":
            ...
            # NUEVO: detectar si Bublee está repitiendo la misma pregunta
...
class LLMResponseValidator:
    ...
    def is_repeating_previous(self, response: str, history: List[Dict[str, Any]]) -> bool:
        ...
        una respuesta anterior de Bublee.
...
def _make_llm_first_generate(original_generate):
    ...
    v10.2: usa inspect.signature(...).bind() en vez de parámetros nombrados
    fijos. La firma real en globals.py es distinta a la que esta función
    asumía (el primer parámetro se llama "message", no "user_msg", y
    search_context/kb_context/chat_id tienen default "" que esta función
    no replicaba) — si el llamador real usaba kwargs o se apoyaba en esos
    defaults, esto tiraba TypeError. bind() se adapta a la firma real que
    tenga original_generate, sea cual sea, en vez de asumirla.
    """
    import inspect
    sig = inspect.signature(original_generate)

    async def llm_first_generate(self, *args, **kwargs):
        ...
        kb_context = a.get("kb_context") or ""
        ...
        if history:
            ...
            if memory_block:
                kb_context = f"{kb_context}\n\n{memory_block}" if kb_context else memory_block
                a["kb_context"] = kb_context
                log.debug(f"[brain_v10] memoria inyectada: {memory_block[:80]}...")

        call_args = dict(a)
        call_args.pop("self", None)
        return await original_generate(self, **call_args)
...
def patch_llm_first(generator) -> bool:
    ...
    Idempotente a propósito: ahora hay más de un punto que puede llamar a
    este patch (app.py lo llama en el lifespan de FastAPI, y bublee.py
    también lo intenta como red de seguridad). Sin este guard, un segundo
    llamado envolvería _normalize_first_patient_turn/generate por partida
    doble, ejecutando toda la lógica de frustración/anti-loop dos veces
    por turno — no rompe, pero hace doble trabajo y puede duplicar logs.
    """
    if generator is None:
        log.warning("[brain_v10] generator es None — patch no aplicado")
        return False

    if getattr(generator, "_brain_v10_patched", False):
        log.info("[brain_v10] generator ya estaba parchado — se omite (evita doble-wrap)")
        return True
    ...
    try:
        ...
        generator._brain_v10_patched = True
        return True
...
def auto_patch() -> bool:
    ...
    bublee_module = sys.modules.get("__main__") or sys.modules.get("bublee")
    if bublee_module is None:
        for name, mod in sys.modules.items():
            if name == "bublee" or (name == "__main__" and hasattr(mod, "generator")):
                bublee_module = mod
                break

    if bublee_module is None:
        log.warning("[brain_v10] no se encontró módulo bublee — auto_patch fallido")
        return False

    gen = getattr(bublee_module, "generator", None)
    if gen is None:
...
def build_estetica_sector_layer(is_poblado: bool = False) -> str:
    ...
    Usar en bublee.py reemplazando el _sector_layer de estetica no-Poblado:
        from bublee_brain_v10 import build_estetica_sector_layer
