...
log = logging.getLogger("bublee.demo_admin")
...
def is_admin_chat(chat_id: str, clinic: Dict, db) -> bool:
    from src.bublee.utils.helpers import _parse_admin_ids
...
async def handle_admin_contact_command(
    ...
    if not phone:
        if demo_llm:
            try:
                r = await demo_llm(
                    "Eres Bublee. Te pidieron contactar a alguien pero no diste el número.",
    ...
    if not intro:
        try:
            r, _ = await llm_engine.complete(
                [{"role": "system", "content": "Eres Bublee. Preséntate breve y natural, como en WhatsApp. Máximo 2 líneas."},
    ...
    if demo_llm:
        try:
            r = await demo_llm(
                "Eres Bublee. Acabas de hacer lo que el admin te pidió. Respondé en 2 burbujas separadas por |||. Primera: confirmá. Segunda: preguntá si necesita algo más.",
    ...
    try:
        ...
        if gen_eng:
            r, _ = await gen_eng.complete(
                [{"role": "system", "content": "Eres Bublee. Respondé en 2 mensajes separados por |||. Primero confirmá, segundo preguntá si necesita algo más."},
