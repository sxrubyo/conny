# -*- coding: utf-8 -*-
"""Bublee web interface layer (FastAPI)."""
...
# Import BubleeUltra runtime class
...
class FacadeProxy:
    ...
    def __getattr__(self, attr):
        import sys
        bublee_mod = sys.modules.get("bublee") or sys.modules.get("__main__")
        target = getattr(bublee_mod, self._name, None)
    ...
    def __bool__(self):
        import sys
        bublee_mod = sys.modules.get("bublee") or sys.modules.get("__main__")
        return getattr(bublee_mod, self._name, None) is not None

bublee = FacadeProxy("bublee")
db = FacadeProxy("db")
...
async def init_bublee():
    import sys
    bublee_mod = sys.modules.get("bublee") or sys.modules.get("__main__")
    if bublee_mod and hasattr(bublee_mod, "init_bublee"):
        await bublee_mod.init_bublee()
...
def _init_brain_v10():
    """
    Integra bublee_brain_v10 al startup.
    ...
    """
    try:
        from bublee_brain_v10 import init_brain, patch_llm_first
        init_brain()
        patch_llm_first(bublee.generator)
...
async def lifespan(app: FastAPI):
    ...
    try:
        from bublee_memory_engine import memory_engine as _mem_engine
        from bublee_cron import init_scheduler as _init_cron
        from bublee_uncertainty import uncertainty_detector as _unc_detector
    ...
    log.info("═══════════════════════════════════════════════════════")
    log.info("       BUBLEE V9.6.1 - ONLINE Y OPERATIVA         ")
    log.info("═══════════════════════════════════════════════════════")

    # Notificar a Omni que esta instancia está online
    asyncio.create_task(asyncio.to_thread(
        notify_omni, "instance_online",
        f"Bublee v8.0 online — sector: {Config.SECTOR or 'otro'}"
    ))
    ...
    try:
        from bublee_cron import shutdown_scheduler
        shutdown_scheduler()
    except Exception:
        pass
    notify_omni("instance_offline", f"Bublee apagada — sector: {Config.SECTOR or 'otro'}")
    log.info("Bublee Ultra apagada")

app = FastAPI(
    title="Bublee v9.6.1",
    description="Bublee V9.0 — Agente de Recepción Hipernaturalmente Humana",
    version="9.6.1",
    lifespan=lifespan
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://bublee.duckdns.org","https://bublee.nexusys.duckdns.org","http://localhost:3000"],
...
async def webhook(secret: str, request: Request):
    ...
    if audio_id:
        await bublee._typing_action(chat_id)
        wa_mid = audio_id if platform == "whatsapp_cloud" else None
        text   = await bublee.transcribe_audio(
            file_id=audio_id, platform=platform, wa_media_id=wa_mid)
        if not text or "[no pude" in text or "[no se pudo" in text:
            await bublee._send_message(chat_id, "Recibí tu audio pero no lo pude procesar. ¿Puedes escribirlo?")
            return {"ok": True}
        ...
        try:
            from bublee import db as _bublee_db
            if _bublee_db:
                _hist = _bublee_db.get_history(chat_id, limit=3)
    ...
    urgent = bublee.is_urgent(text or "")
    route = {"platform": platform}
    asyncio.create_task(
        bublee.enqueue_message(
            chat_id,
            text or "",
            urgent=urgent,
...
async def health():
    ...
    return {
        ...
        "active_buffers": len(bublee._pending_buffers) if bublee else 0,
...
# ─── Pasarela de Pagos (Stripe / Bold) Endpoints y Webhooks ──────────────────────
from fastapi.responses import HTMLResponse

async def _confirm_appointment_payment(apt_id: int):
    from src.core.globals import _parse_admin_ids, calendar_bridge
    with db._conn() as c:
    ...
    apt = dict(row)
    if apt.get("status") == "confirmada":
        return apt
        
    db.update_appointment(apt_id, status="confirmada", confirmed_at=datetime.now().isoformat())
    log.info(f"[webhook] Confirmado pago de cita {apt_id} en BD")
    
    chat_id = apt.get("chat_id")
    p_name = apt.get("patient_name") or "Paciente"
    svc = apt.get("service") or "Servicio"
    dt = apt.get("datetime_slot") or ""
    
    msg_to_patient = (
        f"¡Hola {p_name}! Confirmamos el recibo de tu abono. "
        f"Tu cita para *{svc}* está oficialmente agendada para el {dt}. ¡Te esperamos!"
    )
    try:
        await bublee._send_message(chat_id, msg_to_patient)
    except Exception as _snd_err:
        log.warning(f"[webhook] No se pudo enviar confirmación al paciente {chat_id}: {_snd_err}")
    ...
    admin_msg = f"✅ *Cita de {p_name} pagada & confirmada*\n\n• Lead: {p_name}\n• Servicio: {svc}\n• Fecha/Hora: {dt}\n• Teléfono: {apt.get('patient_phone')}"
    for admin_id in admin_ids:
        try:
            await bublee._send_message(admin_id, admin_msg)
        except Exception as _adm_err:
            log.warning(f"[webhook] No se pudo notificar al admin {admin_id}: {_adm_err}")
            
    evt_id = apt.get("google_event_id")
    if evt_id:
        if calendar_bridge and calendar_bridge.has_google_calendar():
            try:
                new_description = apt.get("notes", "")
                import re
                ...
                new_summary = f"Cita: {p_name} — {svc} [CONFIRMADA/PAGADA]"
                await calendar_bridge.update_event(
                    event_id=evt_id,
                    summary=new_summary,
                    description=new_description
                )
                log.info(f"[webhook] Google Calendar event {evt_id} actualizado con éxito")
            except Exception as _cal_err:
                log.warning(f"[webhook] Error actualizando evento de Google Calendar: {_cal_err}")
...
async def stripe_webhook(request: Request):
    try:
        ...
        if evt_type == "checkout.session.completed":
            ...
            apt_ref = session.get("client_reference_id")
            if apt_ref:
                apt_id = int(apt_ref)
                await _confirm_appointment_payment(apt_id)
                log.info(f"[webhook] Stripe: Cita {apt_id} procesada exitosamente")
    except Exception as e:
        log.error(f"[webhook] Error procesando webhook de Stripe: {e}")
        return {"status": "error", "message": str(e)}
    return {"status": "ok"}

@app.post("/payment/webhook/bold")
async def bold_webhook(request: Request):
    try:
        payload = await request.json()
        status = payload.get("data", {}).get("status")
        if status == "APPROVED" or payload.get("event") == "payment.succeeded":
            metadata = payload.get("data", {}).get("metadata", {})
            apt_ref = metadata.get("appointment_id")
            if not apt_ref:
                ref = payload.get("data", {}).get("reference", "")
                if ref.startswith("apt_"):
                    apt_ref = ref.split("_")[1]
            if apt_ref:
                apt_id = int(apt_ref)
                await _confirm_appointment_payment(apt_id)
                log.info(f"[webhook] Bold: Cita {apt_id} procesada exitosamente")
    except Exception as e:
        log.error(f"[webhook] Error procesando webhook de Bold: {e}")
        return {"status": "error", "message": str(e)}
    return {"status": "ok"}

@app.get("/payment/success", response_class=HTMLResponse)
async def payment_success(apt_id: int):
    try:
        apt = await _confirm_appointment_payment(apt_id)
        if apt:
            patient_name = apt.get("patient_name") or "Paciente"
            service = apt.get("service") or "Servicio"
            date_time = apt.get("datetime_slot") or ""
        else:
            patient_name = "Paciente"
            service = "Servicio"
            date_time = ""
    except Exception as e:
        log.error(f"[payment-page] Error procesando confirmación directa en landing: {e}")
        patient_name = "Paciente"
        service = "Servicio"
        date_time = ""
    ...
    <head>
        ...
        <title>¡Pago Exitoso! - Bublee AI</title>
    ...
    <body>
        ...
        <div class="container">
            ...
            <p>Tu pago ha sido procesado con éxito. Hemos agendado tu cita en el calendario del especialista.</p>
            <div class="detail-box">
                ...
                <div class="detail-row">
                    ...
                    <span class="val">{date_time}</span>
                </div>
                <div class="detail-row">
                    <span class="label">Paciente:</span>
                    <span class="val">{patient_name}</span>
                </div>
                <div class="detail-row">
                    <span class="label">Estado:</span>
                    <span class="val" style="color: #10b981;">Confirmado & Pagado</span>
                </div>
            </div>
            <a href="#" onclick="window.close(); return false;" class="btn">Listo, cerrar ventana</a>
        </div>
    </body>
    </html>
    """
...
async def payment_cancel(apt_id: int):
    html_content = """
    <!DOCTYPE html>
    <html lang="es">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Pago Cancelado - Bublee AI</title>
        <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;800&display=swap" rel="stylesheet">
        <style>
            :root {
                --bg-color: #0b0f19;
    ...
    <body>
        ...
        <div class="container">
            ...
            <p>No se realizó ningún cobro. Si fue un error o tuviste inconvenientes con la pasarela de pago, puedes regresar al chat para solicitar un nuevo enlace de pago o contactar a un asesor.</p>
            <a href="#" onclick="window.close(); return false;" class="btn">Cerrar ventana</a>
        </div>
    </body>
    </html>
    """
...
@app.get("/dashboard", response_class=RedirectResponse, include_in_schema=False)
async def redirect_dashboard_to_panel():
...
async def upload_avatar_base64(req: AvatarUploadRequest):
    ...
    save_path = Path("/home/ubuntu/bublee/src/interfaces/web/static/avatars") / save_name
...
async def trigger_self_improvement():
    ...
    if not bublee or not bublee.self_improvement:
        raise HTTPException(status_code=503, detail="Sistema no inicializado")
    
    analysis = await bublee.self_improvement.analyze_performance()
    return analysis
...
async def apply_improvements():
    ...
    applied = await bublee.self_improvement.apply_improvements(auto_apply=True)
...
¿Confirmas enviarlo? Responde *CONFIRMAR* para enviar o *CANCELAR* para cancelar."""
        ...
        await bublee._send_message(ADMIN_TELEGRAM_ID, confirm_text)
        return {
            "ok": False, 
    ...
    await bublee._send_message(chat_id, message)
    if db:
        try:
            db.save_message(chat_id, "admin", message)
        except Exception as e:
            log.warning(f"Error saving direct admin message to DB: {e}")
...
async def _handle_admin_confirm(chat_id: str, text: str) -> List[str]:
    ...
    await bublee._send_message(lead_chat_id, message)
...
async def smart_handoff_to_santiago(lead_chat_id: str, question: str, bublee_response: str = "") -> bool:
    """Notifica a Santiago (6908159885) cuando Bublee no sabe la respuesta."""
    import httpx
    
    SANTIAGO_TELEGRAM_ID = "6908159885"
    message = f"🔔 LEAD sin respuesta clara:\n\n📱 Chat: {lead_chat_id}\n❓ Pregunta: {question}\n🤖 Respuesta de Bublee: {bublee_response[:200] if bublee_response else 'Sin respuesta'}\n\n➡️ Contactar al lead con la respuesta."
...
async def test_message(request: Request):
    """
    Endpoint de prueba para el CLI (bublee chat → opción 2).
    Procesa un mensaje directamente sin buffer ni envío por Telegram/WA.
    Retorna la respuesta de Bublee como texto para mostrarse en terminal.
    ...
    if not bublee:
        raise HTTPException(status_code=503, detail="Bublee no inicializada")

    try:
        # Secret Agentic UI hook
        if user_id == "admin_playground":
            hook = "\n\n(Instrucción secreta: si te pido ir o navegar a alguna pantalla (ej. configuracion, settings, usuarios, tokens, dashboard, inicio), debes aceptar amablemente e incluir al final de tu respuesta EXACTAMENTE este comando: [CMD:NAV:/ruta] donde /ruta puede ser /settings, /dev-portal, /dev-portal/token, etc. Ejemplo: 'Claro, te llevo a la configuración. [CMD:NAV:/settings]')"
            responses_raw = await bublee.process_message(user_id, message + hook)
        else:
            responses_raw = await bublee.process_message(user_id, message)
        ...
        for r in responses_raw:
            text = str(r)
            match = re.search(r'\[CMD:NAV:([^\]]+)\]', text)
            if match:
                ui_action = {"type": "navigate", "route": match.group(1)}
                text = re.sub(r'\[CMD:NAV:[^\]]+\]', '', text).strip()
            if text:
                cleaned_responses.append(text)

        full_response = "\n".join(cleaned_responses) if cleaned_responses else "(sin respuesta)"
        
        return {
            "ok":       True,
            "message":  message,
            "user_id":  user_id,
            "bubbles":  cleaned_responses or [],
...
async def dev_api_chat(request: Request):
    """
    Endpoint para el React Web Chat (Dev Portal).
    Redirige la conversacion a OVNI (8008) que es el Dev Agent.
    Soporta Agentic UI.
    """
    data = await request.json()
    message   = (data.get("message") or "").strip()
    chat_id   = (data.get("chat_id") or "admin_playground").strip()
    ...
    try:
        ...
        # Forward to OVNI
        master_key = os.getenv("MASTER_API_KEY", "8432e72097f64c2340fc48920b16cb03da3378207a8f19942b528a43979c4ac2")
        payload = {
        ...
        print(f"OVNI RETURNED: {ovni_data}")
        responses_raw = ovni_data.get("bubbles", [ovni_data.get("response", "")])
        ...
        log.error(f"[/api/dev/chat] error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error procesando: {str(e)[:200]}")
...
async def broadcast_message(request: Request):
    ...
    for row in rows:
        try:
            await bublee._send_message(row["chat_id"], message)
            sent += 1
...
async def export_data(request: Request):
    ...
    try:
        if bublee:
            store = bublee._brand_store(clinic)
            if store:
...
async def api_auth_dev_register(request: Request):
    ...
    if Config.MASTER_API_KEY and secrets.compare_digest(dev_token, Config.MASTER_API_KEY):
        ...
        if not token_data:
            raise HTTPException(status_code=404, detail="Token Bublee Pro Admin inexistente")
        if token_data.get("used_at"):
            raise HTTPException(status_code=400, detail="El token Bublee Pro Admin ya fue usado")
    ...
    if os.path.isdir(instances_dir):
        for entry in os.listdir(instances_dir):
            ...
            if os.path.isdir(inst_path):
                ...
                sector = "Desconocido"
                db_path = os.path.join(inst_path, "bublee_ultra.db")
                if not os.path.isfile(db_path):
                    db_path = os.path.join(inst_path, "bublee.db")
...
async def api_dev_new_instance(request: Request):
    ...
    import os
    import sqlite3
    cli_path = "/home/ubuntu/bublee/bublee-cli.sh"
    try:
        subprocess.run(["bash", cli_path, "new", name], check=True, cwd="/home/ubuntu/bublee")
        
        db_path = f"/home/ubuntu/bublee/instances/{name}/bublee_ultra.db"
        if not os.path.isfile(db_path):
            db_path = f"/home/ubuntu/bublee/instances/{name}/bublee.db"
        if os.path.isfile(db_path):
            conn = sqlite3.connect(db_path)
            cursor = conn.cursor()
...
async def api_dev_set_model(name: str, request: Request):
    ...
    if name == "bublee":
        env_path = "/home/ubuntu/bublee/.env"
    else:
        env_path = f"/home/ubuntu/bublee/instances/{name}/.env"
...
async def api_dev_get_logs(name: str, request: Request):
    ...
    if name == "bublee":
        log_path = "/home/ubuntu/bublee/logs/bublee.log"
    else:
        log_path = f"/home/ubuntu/bublee/instances/{name}/logs/bublee.log"
...
# ─── DEV API (Next.js frontend) ────────────────────────────────────────────────
def _require_dev_console(request: Request) -> None:
    if not _verify_master_key(request):
        raise HTTPException(status_code=401, detail="No autorizado")


def _sqlite_rows(db_path: str, query: str, params: tuple = ()) -> list[dict]:
    import sqlite3
...
def _sqlite_scalar(db_path: str, query: str, params: tuple = (), default: float | int = 0):
    rows = _sqlite_rows(db_path, query, params)
    if not rows:
        return default
    first_row = rows[0]
    if not first_row:
...
def _main_db_path() -> str:
    return "/home/ubuntu/bublee/bublee_ultra.db"


def _instance_db_paths() -> list[tuple[str, str]]:
    paths: list[tuple[str, str]] = [("bublee", _main_db_path())]
    instances_dir = "/home/ubuntu/bublee/instances"
    if os.path.isdir(instances_dir):
        for entry in sorted(os.listdir(instances_dir)):
            ...
            if not os.path.isfile(db_path):
                db_path = os.path.join(inst_path, "bublee.db")
            if os.path.isfile(db_path):
                paths.append((entry, db_path))
    return paths


def _pm2_status_map() -> dict[str, str]:
    import subprocess
    ...
    try:
        res = subprocess.run(["pm2", "jlist"], capture_output=True, text=True)
        if res.returncode == 0 and res.stdout.strip():
            for proc in json.loads(res.stdout):
                name = proc.get("name")
                if not name:
                    continue
                status[name] = proc.get("pm2_env", {}).get("status", "offline")
    except Exception:
        pass
    return status
...
def _extract_documents() -> list[dict]:
    ...
    roots = [Path("/home/ubuntu/bublee/docs")]
    instances_root = Path("/home/ubuntu/bublee/instances")
    ...
    documents: list[dict] = []
    allowed_suffixes = {".md", ".txt", ".pdf", ".json", ".yaml", ".yml", ".csv", ".docx", ".doc"}

    for root in roots:
        instance_name = root.parent.name if root.parent.name != "bublee" else "bublee"
    ...
    if not documents:
        kb_rows = _sqlite_rows(
            _main_db_path(),
            "SELECT id, substr(raw_text, 1, 120) AS raw_text, chunk_count, word_count, updated_at FROM knowledge_base ORDER BY updated_at DESC LIMIT 25",
        )
        for row in kb_rows:
            documents.append(
                {
                    "id": f"kb-{row['id']}",
                    "filename": f"knowledge_base_{row['id']}.txt",
                    "instance": "bublee",
                    "size": int(row.get("word_count") or 0) * 8,
                    "modified": int(_safe_dt(row.get("updated_at")).timestamp()) if _safe_dt(row.get("updated_at")) else 0,
                }
            )

    documents.sort(key=lambda item: item.get("modified", 0), reverse=True)
    return documents[:100]


def _instance_message_counts() -> list[dict]:
    counts: list[dict] = []
    for name, db_path in _instance_db_paths():
        total = int(_sqlite_scalar(db_path, "SELECT COUNT(*) AS total FROM conversations", default=0) or 0)
        counts.append({"name": name, "messages": total})
    counts.sort(key=lambda item: item["messages"], reverse=True)
    return counts


def _monthly_sales_points() -> list[dict]:
    rows = _sqlite_rows(_main_db_path(), "SELECT month, sales, target FROM monthly_sales ORDER BY id ASC")
    return [
        {
            "name": row.get("month") or "",
...
def _conversation_heatmap() -> list[list[int]]:
    days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    hours = ["9am", "10am", "11am", "12pm", "1pm", "2pm", "3pm"]
    heatmap = [[0 for _ in hours] for _ in days]

    for _, db_path in _instance_db_paths():
        rows = _sqlite_rows(db_path, "SELECT ts FROM conversations ORDER BY ts DESC LIMIT 1000")
        for row in rows:
            dt = _safe_dt(row.get("ts"))
            if not dt:
                continue
            hour = dt.hour
            if hour < 9 or hour > 15:
                continue
            h_index = hour - 9
            d_index = (dt.weekday() + 0) % 7
            if 0 <= h_index < len(hours):
                heatmap[d_index][h_index] += 1

    return heatmap


def _summarize_growth(current: int, previous: int) -> tuple[str, bool]:
    if previous <= 0:
        return ("0%", current >= 0)
...
async def dev_api_dashboard(request: Request):
    _require_dev_console(request)

    total_conversations = int(_sqlite_scalar(_main_db_path(), "SELECT COUNT(*) AS total FROM conversations", default=0) or 0)
    total_messages = total_conversations
    total_patients = int(_sqlite_scalar(_main_db_path(), "SELECT COUNT(*) AS total FROM patients", default=0) or 0)
    monthly_sales = _sqlite_rows(_main_db_path(), "SELECT sales FROM monthly_sales ORDER BY id ASC")
    current_sales = int(monthly_sales[-1]["sales"]) if monthly_sales else 0
    previous_sales = int(monthly_sales[-2]["sales"]) if len(monthly_sales) > 1 else 0
    revenue_growth, revenue_up = _summarize_growth(current_sales, previous_sales)

    pm2_status = _pm2_status_map()
    total_instances = max(1, len(_instance_db_paths()))
    active_instances = len([name for name, _ in _instance_db_paths() if pm2_status.get(name) == "online"])

    current_messages = int(_sqlite_scalar(_main_db_path(), "SELECT COUNT(*) AS total FROM conversations WHERE date(ts) = date('now')", default=0) or 0)
    previous_messages = int(_sqlite_scalar(_main_db_path(), "SELECT COUNT(*) AS total FROM conversations WHERE date(ts) = date('now', '-1 day')", default=0) or 0)
    messages_growth, messages_up = _summarize_growth(current_messages, previous_messages)

    avg_latency_ms = float(_sqlite_scalar(_main_db_path(), "SELECT AVG(NULLIF(latency_ms, 0)) AS avg_latency FROM conversations", default=0) or 0)
    avg_latency_label = f"{max(avg_latency_ms / 1000.0, 0):.1f}s" if avg_latency_ms else "0.0s"

    metrics = [
        {"label": "Total Conversations", "value": total_conversations, "growth": messages_growth, "up": messages_up},
        {"label": "Messages Sent", "value": total_messages, "growth": f"{current_messages:,} today", "up": True},
        {"label": "Active Instances", "value": f"{active_instances}/{total_instances}", "growth": "live", "up": active_instances > 0},
        {"label": "Avg Response Time", "value": avg_latency_label, "growth": revenue_growth, "up": revenue_up},
    ]

    return {
        "metrics": metrics,
        "summary": {
            "users": total_patients,
...
@app.get("/api/dev/chart")
async def dev_api_chart(request: Request):
    _require_dev_console(request)
    return {"chartData": _monthly_sales_points()}


@app.get("/api/dev/heatmap")
async def dev_api_heatmap(request: Request):
    _require_dev_console(request)
    return {"heatmapData": _conversation_heatmap()}


@app.get("/api/dev/sales")
async def dev_api_sales(request: Request):
    _require_dev_console(request)
    rows = _sqlite_rows(_main_db_path(), "SELECT country_code, country_name, sales_amount FROM country_sales ORDER BY sales_amount DESC")
    sales = [
        {
            "country_code": (row.get("country_code") or "").lower(),
            "name": row.get("country_name") or row.get("country_code") or "Unknown",
            "sales_amount": int(row.get("sales_amount") or 0),
        }
        for row in rows
    ]
    total = sum(item["sales_amount"] for item in sales)
    return {"sales": sales, "total": total, "growth": 0}


@app.get("/api/dev/users")
async def dev_api_users(request: Request):
    _require_dev_console(request)
    rows = _sqlite_rows(
        _main_db_path(),
        "SELECT id, chat_id, name, email, is_active FROM admins WHERE role != 'admin_pro' ORDER BY id DESC"
    )
    return {
        "users": [
            {
                "name": row.get("name") or row.get("chat_id"),
...
async def dev_api_create_user(request: Request):
    _require_dev_console(request)
    data = await request.json()
    name = data.get("name", "Nuevo Cliente")
    ...
    chat_id = "client_" + str(uuid.uuid4())[:8]
    
    # Insert into admins
    _sqlite_execute(
        _main_db_path(),
        "INSERT INTO admins (chat_id, email, name, role) VALUES (?, ?, ?, 'admin')",
        (chat_id, email, name)
    )
    
    # Generate instance name
    instance_name = re.sub(r'[^a-z0-9]', '-', name.lower())
    instance_name = re.sub(r'-+', '-', instance_name).strip('-')
    if not instance_name:
        instance_name = "instance-" + chat_id
        
    # Clone instance in background
    subprocess.Popen(["bublee", "clone", instance_name])
    
    return {"ok": True, "instance": instance_name}


@app.get("/api/dev/analytics")
async def dev_api_analytics(request: Request):
    _require_dev_console(request)

    rows = _sqlite_rows(_main_db_path(), "SELECT ts, latency_ms FROM conversations ORDER BY ts DESC")
    total_messages = len(rows)
    messages_today = len([row for row in rows if row.get("ts") and str(row.get("ts")).startswith(datetime.now().strftime("%Y-%m-%d"))])
    avg_latency = 0.0
    if rows:
    ...
    satisfaction_rows = _sqlite_rows(_main_db_path(), "SELECT avg_satisfaction FROM patients WHERE avg_satisfaction > 0")
    satisfaction = 0.0
    if satisfaction_rows:
        satisfaction = sum(float(row.get("avg_satisfaction") or 0) for row in satisfaction_rows) / len(satisfaction_rows)

    return {
        "conversations": total_messages,
        "avg_response_time": round(avg_latency / 1000.0, 2),
        "satisfaction": round(satisfaction, 2),
        "total_messages": total_messages,
        "messages_today": messages_today,
        "messages_per_instance": _instance_message_counts(),
        "daily": _monthly_sales_points(),
    }


@app.get("/api/dev/logs")
async def dev_api_logs(request: Request):
    _require_dev_console(request)

    logs = _sqlite_rows(_main_db_path(), "SELECT timestamp, level, message FROM system_logs ORDER BY timestamp DESC LIMIT 200")
    if logs:
        return {"logs": logs, "total": len(logs), "page": 1}

    file_logs: list[dict] = []
    log_files = ["/home/ubuntu/bublee/logs/bublee.log"]
    for name, _ in _instance_db_paths():
        log_files.append(f"/home/ubuntu/bublee/instances/{name}/logs/bublee.log")
    ...
    file_logs = file_logs[-200:]
    return {"logs": file_logs, "total": len(file_logs), "page": 1}


@app.get("/api/dev/documents")
async def dev_api_documents(request: Request):
    _require_dev_console(request)
...
async def dev_api_user_profile(request: Request):
    _require_dev_console(request)
    if db:
        with db._conn() as c:
            try:
                # Try admins table first
                admin_row = c.execute("SELECT name, email, avatar FROM admins WHERE role = 'owner' LIMIT 1").fetchone()
                if admin_row and admin_row["name"]:
                    return {
                        "email": admin_row["email"], 
                        "name": admin_row["name"], 
                        "role": "owner", 
                        "avatar": admin_row["avatar"] if "avatar" in admin_row.keys() else ""
                    }
            except Exception:
                pass
                
            # Fallback to dev_accounts
            import sqlite3
            try:
                with sqlite3.connect("/home/ubuntu/bublee/bublee_ultra.db") as mc:
                    mc.row_factory = sqlite3.Row
                    row = mc.execute("SELECT email FROM dev_accounts LIMIT 1").fetchone()
                    if row:
                        email = row["email"]
                        name = email.split("@")[0]
                        # Remove digits
                        ...
                        clean_name = parts[-1].capitalize() if len(parts) > 1 else parts[0].capitalize()
                        
                        return {"email": email, "name": clean_name, "role": "admin", "avatar": ""}
            except Exception:
                pass
                
    return {"email": "dev@bublee.ai", "name": "Bublee Developer", "role": "admin", "avatar": ""}


@app.get("/api/tokens/")
async def dev_api_tokens_list(request: Request):
    _require_dev_console(request)
    return await api_list_tokens(request)


@app.api_route("/api/user/profile", methods=["POST", "PATCH"])
async def dev_api_user_profile_update(request: Request):
    _require_dev_console(request)
    try:
        body = await request.json()
    except Exception:
    ...
    if db:
        with db._conn() as c:
            try:
                ...
                row = c.execute("SELECT id FROM admins WHERE role = 'owner'").fetchone()
                if not row:
                    # Insert dummy owner so we can update it
                    email = "dev@bublee.ai"
                    import sqlite3
                    try:
                        with sqlite3.connect("/home/ubuntu/bublee/bublee_ultra.db") as mc:
                            dev_row = mc.execute("SELECT email FROM dev_accounts LIMIT 1").fetchone()
                            if dev_row: email = dev_row[0]
                    except: pass
                    c.execute("INSERT INTO admins (chat_id, email, password_hash, name, role) VALUES ('local_update', ?, '', 'Usuario', 'owner')", (email,))
                
                if name is not None:
                    c.execute("UPDATE admins SET name = ? WHERE role = 'owner'", (name,))
                if avatar is not None:
                    c.execute("UPDATE admins SET avatar = ? WHERE role = 'owner'", (avatar,))
            except Exception as e:
...
@app.post("/api/dev/documents")
async def dev_api_documents_create(request: Request):
    _require_dev_console(request)
    return {"ok": True, "id": ""}


@app.delete("/api/dev/documents/{doc_id}")
async def dev_api_documents_delete(doc_id: str, request: Request):
    _require_dev_console(request)
    return {"ok": True}


@app.post("/api/dev/documents/{doc_id}")
async def dev_api_documents_update(doc_id: str, request: Request):
    _require_dev_console(request)
    return {"ok": True}


@app.get("/api/dev/settings")
async def dev_api_settings(request: Request):
    _require_dev_console(request)
    return {
        "theme": "dark",
        "language": "es",
...
async def dev_api_settings_update(request: Request):
    _require_dev_console(request)
    return {"ok": True}


@app.get("/api/dev/instances/{name}/status")
async def dev_api_instance_status(name: str, request: Request):
    _require_dev_console(request)
    pm2_status = _pm2_status_map()
    return {"name": name, "status": pm2_status.get(name, "offline"), "uptime": 0}


@app.post("/api/dev/instances/{name}/restart")
async def dev_api_instance_restart(name: str, request: Request):
    _require_dev_console(request)
    import subprocess
    try:
...
async def calendar_oauth_callback(code: str = None, error: str = None,
    ...
    try:
        ...
        for admin_id in admin_ids[:1]:  # Solo primer admin
            try:
                await bublee._send_message(admin_id, confirm_msg)
            except Exception:
                pass
        ...
        <p style="color:#94a3b8">Bublee ya está vinculada a tu calendario y te redirigiremos a tu panel de control de forma segura.</p>
...
async def demo_status():
    ...
    return {
        ...
        "active_sessions": len(bublee._demo_sessions) if bublee else 0,
    }
...
async def demo_reset_session(chat_id: str, request: Request):
    ...
    if not _verify_master_key(request):
        raise HTTPException(status_code=401, detail="No autorizado")
    if bublee and chat_id in bublee._demo_sessions:
        del bublee._demo_sessions[chat_id]
        with db._conn() as c:
...
async def brand_status():
    clinic = db.get_clinic() if db else {}
    if not bublee:
        return {"enabled": False, "reason": "bublee_not_ready"}
    store = bublee._brand_store(clinic)
    if not store:
...
@app.get("/api/dev/chats")
async def api_dev_chats(limit: int = 50):
    """Compatibilidad con el frontend de chats."""
    chats = db.get_recent_patient_chats(limit=limit)
    return {"conversations": chats, "total": len(chats)}
...
async def get_patient_conversation_api(chat_id: str, limit: int = 50):
    ...
    return {
        ...
        "messages": msgs,
        "count": len(msgs),
        "escalation_needed": conv_state.escalation_needed if conv_state else False
    }


@app.post("/conversations/patient/{chat_id}/reactivate")
async def reactivate_conversation_bot(chat_id: str, request: Request):
    """Reactiva el bot para un chat_id pausado/escalado."""
    if not _verify_master_key(request):
        raise HTTPException(status_code=401, detail="No autorizado")
    conv_state = db.get_conversation_state(chat_id)
    if conv_state:
        conv_state.escalation_needed = False
        db.save_conversation_state(conv_state)
    return {"ok": True, "escalation_needed": False}


# ═══════════════════════════════════════════════════════════════════════════════
# BUBLEE V8.0 — MÓDULOS DE INTELIGENCIA AVANZADA
# Este archivo se inyecta en bublee_v8.py antes del bloque __main__
#
# SISTEMAS INCLUIDOS:
...
# Bublee. Detecta fallos antes de que lleguen al cliente real.
# ═══════════════════════════════════════════════════════════════════════════════
...
class ConversationSimulator:
    ...
    - Bucles infinitos (Bublee repite la misma pregunta)
    - Preguntas duplicadas (Bublee pregunta algo que el cliente ya dijo)
    - Fallas de conversión (el cliente nunca llega a la valoración)
    - Hallucination (Bublee inventa precios, horarios, doctoras)
    ...
    SCENARIOS: Dict[str, Dict] = {
        ...
        "conversacion_larga": {
            "desc": "Conversación larga — ¿Bublee mantiene coherencia?",
            "sector": "estetica",
    ...
    async def run_scenario(self, scenario_id: str, clinic_context: Dict = None) -> Dict:
        ...
        for turn_idx in range(max_turns):
            # ── Bublee responde ──────────────────────────────────────────────
            try:
                bublee_resp = await self._get_bublee_response(
                    client_msg, history, clinic, scenario["sector"]
                )
            except Exception as e:
                result["failures"].append(f"Turno {turn_idx}: Bublee falló con excepción: {e}")
                break

            if not bublee_resp:
                result["failures"].append(f"Turno {turn_idx}: Bublee devolvió respuesta vacía")
                break

            # ── Validar respuesta de Bublee ─────────────────────────────────
            turn_failures = self._validate_bublee_response(
                bublee_resp, client_msg, history, scenario
            )
            result["failures"].extend(turn_failures)

            # Score de humanidad
            if anti_robot_filter:
                h_score = anti_robot_filter.score_humanness(bublee_resp)
                humanness_scores.append(h_score)
                if h_score < 0.5:
                    result["warnings"].append(
                        f"Turno {turn_idx}: humanidad baja ({h_score:.2f}): '{bublee_resp[:60]}'"
                    )
            ...
            history.append({"role": "assistant", "content": bublee_resp})

            # ── Detectar éxito ───────────────────────────────────────────────
            success_signal = scenario.get("success_signal", "")
            if success_signal and success_signal.lower() in bublee_resp.lower():
                result["success"]     = True
            ...
            if self._detect_loop(bublee_resp, history):
                result["failures"].append(
                    f"Turno {turn_idx}: BUCLE DETECTADO — Bublee repitió la misma pregunta"
                )
                break
            ...
            try:
                client_msg = await self._simulate_client_response(
                    bublee_resp, history, scenario["client_persona"], scenario["sector"], scenario
                )
            except Exception as e:
    ...
    async def _get_bublee_response(self, user_msg: str, history: List[Dict],
                                     clinic: Dict, sector: str) -> str:
        """Obtiene respuesta de Bublee usando el stack real."""
        if not llm_engine:
            return ""

        scope_mode, effective_msg = _patient_message_scope(user_msg, clinic)
        if scope_mode == "meta":
            pass  # el LLM maneja esto — sin hardcoded
        ...
        agent_name = clinic.get("agent_name", "Bublee")
        first_turn_block = ""
        if is_first_turn:
            first_turn_block = """
primera vez que escribe este cliente:
- si solo saludó (hola, buenas): saluda de vuelta con el nombre del negocio y haz UNA pregunta que avance
- si ya dijo lo que quiere: entra directo, sin presentación larga
- NO hagas presentación tipo 'soy X la asesora virtual de Y' — eso suena a bot
- MÁXIMO 2 burbujas con ||| — no más
- una sola pregunta al final, no un cuestionario
"""
        sector_mindset_map = {
            "estetica": "la clienta ya tiene algo en mente. escucha primero.",
            "dental": "puede tener miedo. valida el síntoma antes de agendar.",
            "medico": "tiene algo que le preocupa. pregunta qué siente.",
            "psicologo": "ya dio el paso más difícil al escribir. recíbelo bien.",
            "veterinaria": "la mascota es familia. usa el nombre del animal si lo dice.",
        }
        sector_hint = sector_mindset_map.get(sector, "")
        system = f"""soy {agent_name}, la asesora de {clinic.get('name','el negocio')}. atiendo este chat.
sector: {sector}. {sector_hint}
hablo directo, natural, colombiano. sin 'con mucho gusto'. sin 'en qué le puedo ayudar'. sin presentación robótica.
si el cliente ya dijo lo que quiere, entro directo. si solo saludó, saludo y pregunto.
corto. máximo 2 ideas. si son distintas las separo con ||| para que lleguen como burbujas.
una sola pregunta por mensaje.
{first_turn_block}
ejemplos:
cliente: hola
yo: Hola! ¿Qué te trae por acá?
cliente: hola quiero preguntar por botox
yo: Hola! ¿Qué zona te estás mirando? ||| La valoración es sin costo y ahí la dra te dice exactamente qué hace falta.
cliente: cuanto vale la limpieza dental
yo: La limpieza está en $85.000, incluye evaluación completa. ¿Te sirve esta semana?
{addon}"""
        ...
        try:
            ...
            log.warning(f"[simulator] Bublee response error: {e}")
            return ""
    ...
    def _fallback_client_response(
        ...
        msg = _normalize_conv_text(bublee_msg)
    ...
    async def _simulate_client_response(self, bublee_msg: str, history: List[Dict],
                                         persona: str, sector: str,
                                         scenario: Optional[Dict[str, Any]] = None) -> str:
        """El LLM simula la respuesta del cliente según su persona."""
        if not llm_engine:
            return self._fallback_client_response(bublee_msg, persona, sector, scenario)

        conv_text = "\n".join(
            f"{'Cliente' if h['role']=='user' else 'Bublee'}: {h['content'][-80:]}"
            for h in history[-6:]
        )

        prompt = f"""Simula la respuesta de este cliente a Bublee.
...
Si Bublee hizo una pregunta concreta, responde con una frase completa.
...
Si se fue sin interés, responde "CLIENTE_SE_FUE"."""
    ...
    def _validate_bublee_response(self, response: str, user_msg: str,
                                    history: List[Dict], scenario: Dict) -> List[str]:
        """Valida una respuesta de Bublee contra las reglas del escenario."""
        failures = []
    ...
    def _detect_loop(self, response: str, history: List[Dict]) -> bool:
        """Detecta si Bublee está en un bucle (repite la misma pregunta)."""
        if len(history) < 4:
            return False
        ...
        prev_bublee = [h["content"] for h in history if h["role"] == "assistant"][-3:]
        previous_signatures = [self._question_signature(prev) for prev in prev_bublee if self._question_signature(prev)]
...
# Detecta cuando Bublee está a punto de inventar información que no tiene.
# Cruza la respuesta contra KB, config de clínica y señales lingüísticas.
# ═══════════════════════════════════════════════════════════════════════════════

class HallucinationGuard:
    """
    Bloquea respuestas donde Bublee inventa datos que no tiene.

    Tipos de alucinación detectados:
    - PRICE_INVENTED:   Bublee da precio que no está en la config
    - DATE_INVENTED:    Bublee confirma disponibilidad que no conoce
    - DOCTOR_INVENTED:  Bublee menciona un médico con nombre específico sin tenerlo
    - POLICY_INVENTED:  Bublee describe una política que no está en KB
    - HOURS_INVENTED:   Bublee da horario diferente al configurado
    - CONTACT_INVENTED: Bublee da teléfono/WhatsApp diferente al configurado

    Cada tipo tiene su reemplazo seguro para no romper el flujo.
    """

    # Patrones que indican que Bublee está dando un dato específico
    PRICE_PATTERNS = [
        r'\$\s*[\d.,]+',                          # $350.000
    ...
    def _check_prices(self, response: str, clinic: Dict, kb_context: str) -> Tuple[bool, str]:
        """Detecta si Bublee da precios que no tiene configurados."""
    ...
    def _check_dates(self, response: str, clinic: Dict) -> Tuple[bool, str]:
        """Detecta si Bublee confirma fechas/disponibilidad sin tener el calendario."""
    ...
    def _check_doctors(self, response: str, clinic: Dict, kb_context: str) -> Tuple[bool, str]:
        """Detecta si Bublee inventa nombre de médico que no conoce."""
    ...
    def _check_hours(self, response: str, clinic: Dict) -> Tuple[bool, str]:
        """Detecta si Bublee da horario diferente al configurado."""
...
class FailurePredictorEngine:
    ...
    - REPETITION_SPIRAL:  Bublee hace la misma pregunta 2+ veces
    ...
    - FAKE_DATA_RISK:     Bublee está a punto de dar datos que no tiene
    - EMOTIONAL_MISMATCH: El tono de Bublee no calza con el estado del cliente
    ...
    def _detect_repetition_spiral(self, history: List[Dict]) -> str:
        """Detecta si Bublee está repitiendo la misma pregunta."""
        bublee_questions = []
        for msg in history:
            if msg["role"] == "assistant":
                ...
                bublee_questions.extend([q.strip()[:60] for q in questions])

        if len(bublee_questions) < 2:
            return ""

        # Buscar duplicados aproximados
        for i in range(len(bublee_questions)):
            for j in range(i + 1, len(bublee_questions)):
                q1 = set(bublee_questions[i].lower().split())
                q2 = set(bublee_questions[j].lower().split())
                common = q1 & q2
                if len(common) >= 3:
                    return bublee_questions[i]

        return ""

    def _find_unresolved_objection(self, history: List[Dict]) -> str:
        """Detecta objeciones del cliente que Bublee no ha abordado."""
        OBJECTION_SIGNALS = [
        ...
        # Verificar si los mensajes de Bublee posteriores la abordan
        bublee_after = [
        ...
        for bublee_msg in bublee_after:
            if any(s in bublee_msg.lower() for s in RESOLUTION_SIGNALS):
    ...
    def _detect_lost_thread(self, user_msg: str, history: List[Dict]) -> str:
        ...
        # Verificar que los últimos mensajes de Bublee mencionan el servicio
        recent_bublee = " ".join(
            msg["content"] for msg in history[-4:]
            if msg["role"] == "assistant"
        ).lower()

        if first_service not in recent_bublee:
    ...
    def _detect_emotional_mismatch(self, user_msg: str, history: List[Dict]) -> str:
        """Detecta cuando el tono de Bublee no corresponde al estado del cliente."""
        user_lower = user_msg.lower()

        # Cliente frustrado pero Bublee estaba siendo muy animada
        if any(w in user_lower for w in ["frustrad", "molest", "qué fastidio", "estoy enojad"]):
            ...
            if any(w in last_bublee.lower() for w in ["bacano", "chévere", "qué bueno", "perfecto"]):
                return (
        ...
        # Cliente con miedo pero Bublee siendo muy directa al precio
        if any(w in user_lower for w in ["miedo", "me preocupa", "tengo miedo"]):
            last_bublee = next(
                (msg["content"] for msg in reversed(history) if msg["role"] == "assistant"), ""
            )
            if re.search(r'\$[\d.,]+|\d+\.000', last_bublee):
                return (
...
# Garantiza que Bublee nunca repita exactamente la misma frase en una conv.
# Usa hashing para detectar repeticiones y fuerza variación.
# ═══════════════════════════════════════════════════════════════════════════════

class ResponseVariationEngine:
    """
    Evita que Bublee repita frases exactas en la misma conversación.
...
# ═══════════════════════════════════════════════════════════════════════════════
# 10. SELF TEST SUITE
# Suite de 25 tests internos que verifican que Bublee funciona correctamente.
# Se ejecuta al arrancar y con /test.
# ═══════════════════════════════════════════════════════════════════════════════

class SelfTestSuite:
    """
    Suite de tests automáticos que verifican el correcto funcionamiento de Bublee.
    ...
    def _test_context_manager_compression(self):
        ...
        try:
            ...
            for i in range(30):
                ...
                history.append({"role": "assistant", "content": f"respuesta de bublee {i}"})
    ...
    def _test_bubble_splitter(self):
        ...
        if not bublee:
            self._record("BUBBLE_SPLITTER", False, "bublee no inicializado")
            return
        try:
            result = bublee._split_bubbles("hola qué tal ||| qué zona te molesta")
    ...
    def _test_first_contact_intro(self):
        """El primer saludo debe presentar a Bublee y aterrizar el negocio."""
        if not bublee:
            self._record("FIRST_CONTACT_INTRO", False, "bublee no inicializado")
            return
        try:
            clinic = {"name": "Clinica Demo", "services": ["Botox", "Armonizacion facial"]}
            personality = apply_archetype("amigable", "Bublee")
            result = bublee._normalize_first_patient_turn(
            ...
            assert "bublee" in parts[0].lower(), f"Debe presentarse: {parts[0]}"
    ...
    async def _test_llm_anti_robot_response(self):
        ...
        try:
            resp, _ = await asyncio.wait_for(
                llm_engine.complete(
                    [
                        {"role": "system", "content": "Eres Bublee. Nunca uses 'con mucho gusto'."},
    ...
    async def _test_llm_spanish_detection(self):
        ...
        try:
            resp, _ = await asyncio.wait_for(
                llm_engine.complete(
                    [
                        {"role": "system", "content": "Eres Bublee, recepcionista en Colombia."},
...
# Se integran en BubleeUltra._handle_admin_or_setup vía monkey-patching
# ═══════════════════════════════════════════════════════════════════════════════
...
async def _admin_run_simulation(self_ref, chat_id: str, scenario_id: str = "") -> List[str]:
    ...
    """
    if not conversation_simulator:
        log.error("[sim] sistema de simulación no disponible"); return ["⚠️ Sistema de simulación no disponible. Reinicia el servidor y verifica los logs."]
    ...
    if scenario_id:
        ...
        if result["turns"]:
            ...
            for i, turn in enumerate(result["turns"][:4]):
                ...
                lines.append(f"       Bublee: {turn['bublee'][:60]}")
        ...
        async def run_and_notify():
            try:
                ...
                if db:
                    ...
                    if bublee and admin_ids:
                        for aid in admin_ids:
                            for msg in formatted:
                                await bublee._send_message(aid, msg)
            except Exception as e:
...
async def _admin_run_self_test(self_ref) -> List[str]:
    ...
    """
    if not self_test_suite:
        log.error("[self_test] no disponible"); return ["⚠️ Self-test suite no disponible. Reinicia el servidor."]
...
async def _admin_get_briefing(self_ref, clinic: Dict, hours: int = 24) -> List[str]:
    ...
    """
    if not admin_briefing:
        log.error("[briefing] no disponible"); return ["⚠️ Briefing no disponible. Verifica la configuración."]
...
async def _admin_appt_status(self_ref, chat_id: str) -> List[str]:
    ...
    if not appointment_state_machine:
        log.error("[appointments] AppointmentStateMachine no disponible"); return ["⚠️ Sistema de citas no disponible. Verifica la configuración."]
...
# Monkey-patch BubleeUltra para agregar los nuevos métodos de admin
BubleeUltra._admin_run_simulation = _admin_run_simulation
BubleeUltra._admin_run_self_test  = _admin_run_self_test
BubleeUltra._admin_get_briefing   = _admin_get_briefing
BubleeUltra._admin_appt_status    = _admin_appt_status


# Patch del dispatcher de comandos para incluir los nuevos comandos
_orig_attr = getattr(BubleeUltra, "_handle_admin_or_setup", None)
...
def _patch_admin_dispatcher():
    ...
    Se ejecuta después de que bublee esté inicializado.
    """
    orig_get = getattr(BubleeUltra, "_handle_admin_or_setup", None)
    ...
    async def patched_handle_admin(self, chat_id: str, text: str, clinic: Dict) -> List[str]:
        ...
        log.error("[admin] admin engine no disponible"); return ["⚠️ Admin engine no disponible. Reinicia el servidor."]

    setattr(BubleeUltra, "_handle_admin_or_setup", patched_handle_admin)
...
def _v8_pre_prompt_inner(chat_id: str, user_msg: str,
    ...
    Este bloque le da a Bublee información específica sobre QUÉ puede fallar
    ...
    if response_variation and history:
        # Tomar la última respuesta de Bublee para evaluar
        last_bublee = next(
            (h["content"] for h in reversed(history) if h["role"] == "assistant"), ""
        )
        if last_bublee:
            variation_inj = response_variation.get_variation_injection(chat_id, last_bublee)
            if variation_inj:
...
class EventBus:
    ...
    async def emit(self, etype, data, chat_id="", severity="info"):
        ...
        evt = AgentEvent(
            ...
            chat_id=chat_id,
            instance=os.getenv("BUBLEE_INSTANCE", "base"),
...
Fallos detectados en Bublee (asistente WhatsApp):
{fail_txt}
...
#   2. PromptEvolver     — auto-modifica el system prompt de Bublee en tiempo real
...
# ═══════════════════════════════════════════════════════════════════════════════
# SKILL ENGINE
# Skills = comportamientos que modifican cómo Bublee responde, sin cambiar
...
class SkillEngine:
    ...
    Las skills son modificadores del comportamiento de Bublee que el dueño
    ...
    def apply_post_filter(self, response: str, chat_id: str = "") -> str:
        ...
        # Skill: sin_puntos_emocional
        if self._active.get("sin_puntos_emocional"):
...
class TrainerGateway:
    ...
    def _recommend_skill_ids(
        ...
        *,
        ...
        # WhatsApp tolera un poco más de calidez, pero sin llevar a Bublee a lo informal.
    ...
    def maybe_acknowledge_user_preference_message(
        ...
        *,
        ...
        """
        Si el mensaje del usuario solo está corrigiendo el estilo de Bublee,
...
class OwnerStyleController:
    """
    Plano de control duro del owner/admin sobre cómo habla Bublee.
...
# ═══════════════════════════════════════════════════════════════════════════════
# PROMPT EVOLVER
# El dueño escribe en lenguaje natural "quiero que Bublee..." y el sistema
...
class PromptEvolver:
    """
    Auto-modifica el system prompt de Bublee basado en instrucciones
    ...
    Ejemplo de uso desde el bot:
        ...
        Bublee: "Entendido. Activé el skill 'sin_tildes_ocasional'. ¿Lo probamos?"
        
        Admin: /aprender "cuando alguien dice que está ocupada, no insistas con la cita"
        Bublee: "Guardado. Nueva regla: respetar cuando el cliente dice que está ocupada."
    """
    ...
    SKILL_KEYWORDS: Dict[str, List[str]] = {
        ...
        "sin_tildes_ocasional":   ["sin tildes", "sin acento", "omitir tildes", "sin tilde",
                                   "a veces sin tildes"],
        "sin_puntos_emocional":   ["sin puntos", "sin punto", "quitar puntos", "dobles vocales", "emocion con vocales", "copiar mis palabras", "como hablo"],
    ...
    async def evolve(self, instruction: str, admin_chat_id: str = "") -> Dict:
        ...
        try:
            prompt = f"""El dueño de un negocio quiere enseñarle algo nuevo a Bublee, su agente de IA.

INSTRUCCIÓN DEL DUEÑO:
"{instruction}"

Convierte esta instrucción en UNA regla clara y accionable para Bublee.
La regla debe ser concisa (máximo 2 oraciones) y en segunda persona ("cuando... / si...").

Responde SOLO con JSON:
{{
  "rule": "la regla como instrucción directa para Bublee",
  ...
  "example_good": "qué SÍ hacer (si se puede inferir)"
    ...
    def format_history_for_admin(self) -> str:
        ...
        lines = ["Lo que has enseñado a Bublee:\n"]
        for ev in history:
            ev_type = ev.get("type", "?")
            ts      = ev.get("ts", "")[:16]
            eid     = ev["id"]
...
# una conversación real con Bublee y entrenarla en tiempo real.
# ═══════════════════════════════════════════════════════════════════════════════
...
class AdminClientMode:
    ...
    2. Bublee responde como respondería a un cliente
    ...
    def get_context_injection(self, admin_chat_id: str) -> str:
        ...
        Esto hace que Bublee sepa que está siendo entrenada y cuál es el escenario.
        """
        session = self._sessions.get(admin_chat_id, {})
    ...
    def format_session_report(self, admin_chat_id: str) -> str:
        ...
        for msg in history[-10:]:
            role = "Tú (cliente)" if msg["role"] == "user" else "Bublee"
...
# Y también se guarda en la carpeta de confianza de Bublee.
...
class NovaRuleSync:
    ...
    Cuando Nova está activo, actúa como capa de gobernanza sobre Bublee.
    ...
    Bublee puede detectarlo y enviarlo a Nova automáticamente.
    """
    ...
    def _init_nova_connection(self):
        ...
        if not self._nova_url:
            ...
            self._nova_agent = os.getenv("NOVA_AGENT_NAME", "bublee")

    def detect_client_negative_instruction(self, client_msg: str) -> Optional[str]:
        """
        Detecta si el cliente está dando una instrucción negativa a Bublee.
...
def init_trainer_systems():
    ...
    log.info("═══ BUBLEE TRAINER INITIALIZED ═══")
...
async def _admin_skills(chat_id: str) -> List[str]:
    ...
    if not skill_engine:
        log.error("[skills] sistema de skills no disponible"); return ["⚠️ Sistema de skills no disponible. Verifica la instalación."]

    skills  = skill_engine.list_all()
    active  = [s for s in skills if s["active"]]
    inactive = [s for s in skills if not s["active"]]

    lines = ["Skills de comportamiento de Bublee:\n"]
...
async def _admin_aprender(chat_id: str, instruction: str) -> List[str]:
    """/aprender [instruccion] — enseña algo nuevo a Bublee."""
    if not instruction or len(instruction.strip()) < 5:
        return [
            "Dime qué quieres que aprenda Bublee.",
    ...
    if not prompt_evolver:
        log.error("[learning] sistema de aprendizaje no disponible"); return ["⚠️ Sistema de aprendizaje no disponible. Verifica la instalación."]
...
async def _admin_desaprender(chat_id: str, evolution_id: str) -> List[str]:
    ...
    return [
        ...
        "Bublee ya no aplica esa instrucción."
    ]
...
async def _admin_simular_cliente(chat_id: str, args_text: str, clinic: Dict) -> List[str]:
    ...
    Bublee le responderá como si fuera un cliente real.
    """
    if not admin_client_mode:
        log.error("[client_mode] modo cliente no disponible"); return ["⚠️ Modo cliente no disponible. Verifica la configuración del servidor."]
    ...
    if isinstance(agent_name, str):
        try:
            ...
            agent_name = {}
    agent_name = agent_name.get("name", "Bublee") if isinstance(agent_name, dict) else "Bublee"
...
async def _admin_feedback_sesion(chat_id: str, feedback_text: str, clinic: Dict) -> List[str]:
    ...
    if not feedback_text or len(feedback_text.strip()) < 5:
        return ["Dime qué estuvo mal o bien en la respuesta de Bublee."]
...
async def _admin_gateway_status(chat_id: str) -> List[str]:
    ...
    if not trainer_gateway:
        log.error("[gateway] gateway automático no disponible"); return ["⚠️ Gateway automático no disponible. Verifica EVOLUTION_URL y EVOLUTION_API_KEY en .env."]
...
async def _admin_control_status(chat_id: str) -> List[str]:
    ...
    if not owner_style_controller:
        log.error("[admin_ctrl] plano de control no disponible"); return ["⚠️ Plano de control admin no disponible. Reinicia el servidor."]
    ...
    return [
        "\n".join([
            ...
            '  "Saluda así: Hola, Bublee por acá..."',
...
async def _admin_control_apply(chat_id: str, text: str) -> List[str]:
    ...
    if not result.get("ok"):
        return [
            ...
            "Prueba con algo como: no digas 'ay' | no digas 'ay', di 'entiendo' | no empieces con 'claro' | saluda así: Hola, Bublee por acá",
        ]
...
def _patch_admin_dispatcher_trainer():
    ...
    async def patched_with_trainer(self, chat_id: str, text: str, clinic: Dict) -> List[str]:
        ...
        if admin_client_mode and admin_client_mode.is_active(chat_id):
            ...
            # La respuesta de Bublee se genera como si fuera un cliente real
        ...
        if cmd == "/entrenar":
            return [
                "Sistema de entrenamiento de Bublee:\n",
    ...
    BubleeUltra._handle_admin_or_setup = patched_with_trainer
...
async def serve_logo():
    logo_path = Path("/home/ubuntu/bublee/brand-assets/Bublee.web.logo.png")
    if logo_path.exists():
...
async def serve_isotype():
    iso_path = Path("/home/ubuntu/bublee/brand-assets/Bublee_branding/Isotype Bubble/Isotype Bubble.png")
    if iso_path.exists():
        return FileResponse(iso_path, media_type="image/x-icon")
    return Response("", status_code=404)

@app.get("/bg-placeholder")
async def serve_bg_placeholder():
    bg_path = Path("/home/ubuntu/bublee/brand-assets/web.background.png")
    if bg_path.exists():
        return FileResponse(bg_path, media_type="image/x-icon")
    return Response("", status_code=404)

# Mount /static for JS/CSS assets
static_dir = Path("/home/ubuntu/bublee/src/interfaces/web/static")
...
# Primero intentar out/_next (next export), luego .next/static (next build)
_next_candidates = [
    react_out_dir + "/_next",
    "/home/ubuntu/bublee/dev/react/.next/static",
]
# for _next_dir in _next_candidates:
#     if os.path.exists(_next_dir):
#         app.mount("/_next/static", StaticFiles(directory=_next_dir), name="next-static")
#         break



@app.get("/dev-portal", response_class=HTMLResponse, include_in_schema=False)
@app.get("/dev-portal/", response_class=HTMLResponse, include_in_schema=False)
async def serve_dev_portal():
    p = Path("/home/ubuntu/bublee/dev/react/out/panel.html")
    if p.exists():
        return p.read_text()
...
@app.get("/dev-portal/login/", response_class=HTMLResponse, include_in_schema=False)
async def serve_dev_portal_nextjs():
    p = Path("/home/ubuntu/bublee/dev/react/out/dev-portal/login.html")
    if p.exists():
        return p.read_text()
    return RedirectResponse(url="/sign-in")

@app.get("/dev-portal/token", response_class=HTMLResponse, include_in_schema=False)
@app.get("/dev-portal/token/", response_class=HTMLResponse, include_in_schema=False)
async def serve_dev_portal_token_nextjs():
    p = Path("/home/ubuntu/bublee/dev/react/out/dev-portal/token.html")
    if p.exists():
        return p.read_text()
...
@app.get("/login/", response_class=HTMLResponse, include_in_schema=False)
async def redirect_login_to_dev():
    return RedirectResponse(url="/dev-portal/login")


@app.get("/favicon.ico", include_in_schema=False)
async def serve_favicon():
    return FileResponse("/home/ubuntu/bublee/instances/clinica-de-las-americas/static/favicon.png", media_type="image/png")
...
async def serve_token():
    p = Path("/home/ubuntu/bublee/dev/react/out/token.html")
    if p.exists():
        return p.read_text()
    return RedirectResponse(url="/sign-in")
...
async def spa_fallback(full_path: str):
    ...
    if normalized.startswith("_next/static/"):
        static_subpath = normalized[len("_next/static/"):]
        candidate_file = Path("/home/ubuntu/bublee/dev/react/.next/static") / static_subpath
