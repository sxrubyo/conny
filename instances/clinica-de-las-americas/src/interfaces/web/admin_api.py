"""
Bublee Admin API — Multi-tenant runtime configuration endpoints.
...
log = logging.getLogger("bublee.admin_api")
...
async def teach_fact(
    ...
    """Admin teaches Bublee a new fact (question/answer pair)."""
    _verify_auth(x_admin_key)
...
def _get_db_path(instance_id: str) -> Path:
    ...
    instances_dir = Path("/home/ubuntu/bublee/instances")
    instance_db = instances_dir / instance_id / "bublee.db"
    if instance_db.exists():
        return instance_db
    
    base_dir = Path("/home/ubuntu/bublee")
    ultra_db = base_dir / "bublee_ultra.db"
    if ultra_db.exists():
        return ultra_db
        
    return base_dir / "bublee_ultra.db"
...
<head>
    ...
    <title>Bublee Dashboard — Live Streams</title>
    ...
    <style>
        ...
        * {{
        ...
        .bublee-row .bubble {{
            background-color: var(--bubble-bublee);
            color: var(--text-primary);
        ...
        /* Insights panel under Bublee bubbles */
        .insights-container {{
            width: 100%;
            display: flex;
            flex-direction: column;
...
<body class="dark-theme">
    ...
    <div class="sidebar">
        <div class="header">
            <div class="brand-section">
                ...
                <span class="brand-title">Bublee Live Streams</span>
            </div>
        ...
        <div id="chat-pane" class="chat-pane">
            <div class="splash-screen">
                ...
                <p style="max-width: 400px; line-height: 1.4; font-size: 0.9rem;">
                    Selecciona una conversación del panel izquierdo para monitorear las respuestas de Bublee en tiempo real, latencias y análisis cognitivo.
                </p>
            </div>
        </div>
    </div>
    ...
    <div id="analysis-modal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">🧠 Análisis Cognitivo de Bublee</h3>
                <button id="modal-close-btn" class="modal-close">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 12px; line-height: 1.4;">
                    Representa el razonamiento interno en tiempo real realizado por Bublee (confianza, intenciones, triggers de anti-loops y resguardos).
                </p>
    ...
    <script>
        ...
        themeBtn.addEventListener("click", () => {{
            ...
            localStorage.setItem("bublee_theme", document.body.classList.contains("light-theme") ? "light" : "dark");
        }});
        
        if (localStorage.getItem("bublee_theme") === "light") {{
        ...
        function initAuth() {{
            ...
            const storedKey = localStorage.getItem("bublee_admin_key");
            
            if (keyParam) {{
                adminKey = keyParam;
                localStorage.setItem("bublee_admin_key", keyParam);
                startDashboard();
            }} else if (storedKey) {{
                adminKey = storedKey;
                startDashboard();
        ...
        async function submitAuth() {{
            ...
            try {{
                ...
                if (res.ok) {{
                    adminKey = input;
                    localStorage.setItem("bublee_admin_key", input);
        ...
        async function fetchChats(isPoll = false) {{
            try {{
                ...
                if (res.status === 401) {{
                    ...
                    localStorage.removeItem("bublee_admin_key");
                    location.reload();
                    return;
                }}
                if (!res.ok) return;
        ...
        function renderMessages(messages, isPoll = false) {{
            ...
            messages.forEach((msg, idx) => {{
                ...
                if (msg.role === "user") {{
                    ...
                    // Split Bublee bubbles using " ||| " separator
                    const bubbles = msg.content.split(" ||| ");
                    bubbles.forEach((bubbleText, bIdx) => {{
                        html += `
                            <div class="message-row bublee-row">
                                <div class="bubble">
                    ...
                    const modelName = msg.model_used || "bublee-brain";
