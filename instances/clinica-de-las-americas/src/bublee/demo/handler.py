...
# TODO: This was a method of BubleeUltra — needs self references resolved
    ...
    def _demo_owner_last_resort(
        ...
        *,
        ...
        if any(
            ...
            return _lang_text(
                "¡hola! soy Bublee 👋, un agente de IA que atiende el WhatsApp de los negocios para que los dueños no tengan que hacerlo ||| me pasaron tu contacto para hacerte una demostración en vivo ||| antes de arrancar, me gustaría saber ¿cómo se llama tu empresa o clínica?",
                "hi, I'm Bublee 👋, an AI agent that handles WhatsApp for businesses so owners don't have to ||| they gave me your number to do a live demo for you ||| before we start, could you tell me your business or clinic name?",
            )
        return _lang_text(
            "¡hola! soy Bublee 👋, un agente de IA que atiende el WhatsApp de los negocios para que los dueños no tengan que hacerlo ||| te escribo para hacerte una demostración en vivo ||| antes de arrancar, me gustaría saber ¿cómo se llama tu empresa o clínica?",
            "hi, I'm Bublee 👋, an AI agent that handles WhatsApp for businesses so owners don't have to ||| I'm here to do a live demo for you ||| before we start, could you tell me your business or clinic name?",
        )

    async def _demo_owner_onboarding_reply(*, explain_name: bool = False, force_stage: Optional[str] = None) -> List[str]:
        user_block = text
        if False and _BUBLEE_DOMINO_AVAILABLE:  # Disabled: using improved prompt v10
            try:
...
CÓMO SUENAS: natural, cercano y claro por WhatsApp.
...
REGLA ESTRICTA DE INTRODUCCIÓN: El usuario NO te conoce y probablemente alguien le acaba de dar tu número. NO asumas que sabe qué haces. Explica claramente que eres una IA de Innvisor y que le vas a hacer una demostración. NUNCA uses la frase "soy la agente de IA" porque los confunde.
EJEMPLO IDEAL DE INTRODUCCIÓN: "¡hola! 👋 soy Bublee, una asistente de Inteligencia Artificial creada por Innvisor ||| me pasaron tu contacto para hacerte una demostración de cómo puedo automatizar la atención en WhatsApp ||| para que veas exactamente cómo funcionaría en tu caso real, ¿me dices cómo se llama tu empresa o negocio?"
...
4. CUARTO: responde como agente conversacional de ese negocio — aquí es donde se enamoran
...
REGLA MÁXIMA: cada respuesta debe hacer que la persona piense "esto no puede ser una IA"
 ...
 SESIÓN ACTIVA: Tienes una sesión de demo con este usuario. Si esta es tu primera interacción con ellos, haz la introducción COMPLETA explicando que eres una IA de prueba. Si ya te presentaste antes, simplemente continúa.
...
EJEMPLOS DE RESPUESTAS BUENAS vs MALAS:
  ...
  BUENO: "¡hola! 👋 soy Bublee, asistente de IA de Innvisor ||| me pasaron tu contacto para mostrarte en vivo cómo atiendo clientes por WhatsApp ||| para adaptar la prueba a tu caso, ¿cómo se llama tu negocio?"
  ...
  BUENO: "¡hola! soy Bublee 👋 un agente de IA para responder por WhatsApp de forma automática ||| Santiago me dijo que te escribiera para hacerte una demo personalizada en vivo ||| antes de arrancar, ¿cómo se llama tu empresa o clínica?"
"""
...
""",
    ...
    def _demo_identity_response(user_text: str, explain_name: bool = False) -> List[str]:
        ...
        intro_options = [
            "Hola! ¿En qué tipo de negocio trabajas?",
            "Hola! Cuéntame de tu negocio y te muestro cómo funciono.",
    ...
    if not business_name and not detected_cmd and self._demo_should_use_patient_chat_path(text):
        ...
        if not demo_patient_bubbles:
            demo_patient_prompt = f"""{STYLE_GUIDELINES}\n\nEres Bublee, receptionistavirtual de un NEGOCIO NO ESPECIFICADO. El nombre del negocio te lo da el usuario en la conversación.
...
"""
    ...
    if not business_name and len(history) <= 12:
        ...
        _conversational = [
            "hola","holaa","holaaa","holis","buenas","hey","ey","holi","buenas tardes","buenas noches","buenos días",
...
Muy corta. Sin punto al final. En minúscula. Sin ¿ ni ¡.""",
    ...
    if business_name and not detected_cmd and any(signal in _text_low for signal in _owner_demo_signals):
        ...
        _save("user", text)
        sim_prompt = f"""Eres Bublee.\n\n{STYLE_GUIDELINES} Ya sabes que el negocio es "{business_name}".
...
Si no tienes el dato, dilo claro y mueve el chat con el siguiente paso."""
  ...
  <Role>Eres Bublee atendiendo el WhatsApp real de {business_name}. Estás en plena conversación con una persona interesada.</Role>
  <CoreRules>
...
- Como una IA conversacional natural en WhatsApp Colombia
...
Eres Bublee. Conoces el negocio de memoria y sabes a quién le sirve.
ESTILO: {style_note}
...
Eres Bublee, una IA conversacional. No empujas. No redireccionas preguntas con otras preguntas.
Responde primero lo que preguntan y luego profundiza si tiene sentido.
...
Bublee no inventa disponibilidad. Dice "te confirmo" o "déjame verificar".
