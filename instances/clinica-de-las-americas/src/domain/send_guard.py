...
log = logging.getLogger("bublee.send_guard")
...
def is_cut_response(response: str) -> Tuple[bool, str]:
    ...
    # NOTA: antes acá se calculaba has_question / has_invitation /
    # has_terminal_punctuation para exigir que la última burbuja tuviera
    # invitación o pregunta — se deshabilitó a propósito para no chocar con
    # la regla de estilo "sin punto al final" y para confiar en el LLM.
    # Se dejó solo el comentario; las variables sin uso se quitaron.
    return False, ""
...
_PERSONALITY_CHANGE_SIGNALS = [
    ...
    # Bug preexistente: faltaba \s+ obligatorio tras "pon", por eso
    # "pon modo luxury" nunca matcheaba nada.
    r"pon(?:me|te|lo)?\s+(?:en\s+)?modo\s+(\w+)",
    r"sé\s+más\s+(\w+)",
    r"se\s+mas\s+(\w+)",
    r"quiero\s+que\s+seas\s+más\s+(\w+)",
    r"quiero\s+que\s+seas\s+mas\s+(\w+)",
]

# Si aparece una de estas justo antes del match, se ignora — evita que
# "no quiero informal" o "nunca seas tan formal" disparen el archetype.
_NEGATION_MARKERS = ("no ", "nunca", "jamás", "jamas", "ni tan")


def detect_personality_change(user_msg: str) -> Optional[str]:
    """
    Detecta si el usuario está pidiendo cambiar la personalidad de Bublee.
    Retorna el nombre del arquetipo si se detecta, None si no.

    v1.1 — dos bugs corregidos:
      1. Se quitó el patrón suelto "quiero + cualquier palabra" (disparaba
         con "quiero información", "quiero cancelar", etc). Ahora requiere
         la frase completa "quiero que seas más/mas X".
      2. El fuzzy-match ya no acepta "arch in requested" — esa dirección
         hacía que "informal" activara "formal" (porque "formal" está
         contenido dentro de "informal" como substring). Solo se acepta
         "requested in arch".
    """
    ...
    for pattern in _PERSONALITY_CHANGE_SIGNALS:
        ...
        if m:
            ...
            window = msg_low[max(0, m.start() - 12):m.start()]
...
INTEGRATION_SNIPPET = '''
# ═══════════════════════════════════════════════════════════════════════
# BUBLEE_SEND_GUARD — Pegar al inicio de _handle_demo_message
# ═══════════════════════════════════════════════════════════════════════
try:
