"""bublee_commands.py — Slash command system for users and admins."""
...
log = logging.getLogger("bublee.commands")
...
ADMIN_COMMANDS = {
    ...
    "/aprendizaje": "Resumen LLM de qué le falta aprender a Bublee",
}
...
class CommandHandler:
    ...
    async def _handle_admin(self, cmd: str, args: str, chat_id: str,
        ...
        if cmd == "/pausa":
            self._paused = True
            return ["⏸ Bublee pausada. Mensajes no serán respondidos hasta /reanudar"]

        if cmd == "/reanudar":
            self._paused = False
            return ["▶ Bublee activa de nuevo."]
        ...
        if cmd == "/aprendizaje":
            try:
                from src.core.admin_engines import SelfImprovementEngine
                ...
                if gap_count == 0:
                    return [summary or "Sin gaps registrados todavía."]
                return [f"📊 {gap_count} gaps registrados (últimos 7 días)\n\n{summary}"]
            except Exception as e:
                log.error(f"[commands] /aprendizaje error: {e}")
                return [f"No pude generar el análisis ahora mismo: {e}"]

        if cmd in ("/formal", "/amigable", "/luxury", "/directa", "/energica", "/empatica", "/experta", "/juvenil"):
            tone_map = {
                "/formal": "formal",
                "/amigable": "casual",
                "/luxury": "luxury",
            ...
            name_map = {
                ...
                "casual": "amigable (cercana)",
                "luxury": "luxury (premium)",
                "directa": "directa (al grano)",
                "energica": "enérgica (entusiasta)",
                "empatica": "empática (escucha activa)",
                "experta": "experta (técnica)",
                "juvenil": "juvenil (fresca)",
            }
            friendly_name = name_map.get(tone, tone)
            return [
                f"✅ Cambié mi personalidad a {friendly_name} para hablar con los pacientes.",
                "¿Te gustaría que también use este tono contigo aquí, o seguimos hablando normal?"
            ]
