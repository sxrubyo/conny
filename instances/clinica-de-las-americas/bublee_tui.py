#!/usr/bin/env python3
"""
bublee_tui — Interfaz interactiva de Bublee con flechas ↑↓

Reemplaza el texto de ayuda estático por un menú navegable.
Se activa cuando el usuario corre 'bublee' sin argumentos.

Instalar:
  Copia este archivo a ~/bublee/bublee_tui.py
  El parche en bublee_sync_fix.py lo activa automáticamente.

Uso directo:
  python3 bublee_tui.py
"""
...
# ── Config (mismos defaults que bublee_cli.py) ─────────────────────────────
VERSION       = "8.0.2"
...
CLI_SCRIPT    = os.getenv("BUBLEE_CLI",   str(Path(__file__).resolve().parent / "bublee_cli.py"))

# ── Colores curses (índices de par) ─────────────────────────────────────────
CP_LOGO   = 1   # Lavanda — letras BUBLEE
...
def _load_instances() -> List[Instance]:
    ...
    # Base instance
    base_env_path = Path(BUBLEE_DIR) / ".env"
...
def _run_bublee(*args: str) -> None:
    """Llamar a bublee_cli.py con argumentos."""
    curses.endwin()
    print()
    try:
...
def _execute_menu_action(action_id: str, instances: List[Instance]) -> None:
    ...
    if needs_instance:
        ...
        if inst is None:
            return
        _run_bublee(action_id, inst.name)
    else:
        _run_bublee(action_id)


# ── Logo — idéntico al banner de `bublee init` ──────────────────────────────
#
# Cada entrada: (texto, color_pair, bold)
# CP_LOGO  → letras BUBLEE      (lavanda/púrpura)
...
LOGO_LINES = [
    ...
    (f"  ✦ Bublee v{VERSION}  ·  20 sectores",                             CP_TITLE, True),
...
def _draw_logo(win, row: int, cols: int, compact: bool = False) -> int:
    ...
    if compact or cols < 64:
        ...
        # Modo completo: idéntico a `bublee init`
...
def _tui_main(stdscr) -> None:
    ...
    curses.init_pair(CP_LOGO,  183, -1)   # Lavanda (letras BUBLEE)
...
def run_tui() -> None:
    """
    Lanza el TUI interactivo. Llamar desde bublee_cli.py main()
...
def _simple_help() -> None:
  ...
  Comandos esenciales:
    bublee new           Crear instancia para un cliente
    bublee sync          Sincronizar updates a todas las instancias
    bublee list          Ver todas las instancias
    bublee dashboard     Panel en tiempo real
    bublee health        Health check rápido
    bublee logs [n]      Logs en vivo
    bublee restart [n]   Reiniciar instancia
