#!/usr/bin/env python3
"""bublee — AI receptionist platform."""
...
BOOT_FILE = Path.home() / ".bublee" / ".boot_shown"
...
def cmd_help(args=""):
    ...
    if _should_boot():
        try:
            from bublee_worm import boot_sequence
    ...
    # Live status
    procs = _pm2()
    online = [p for p in procs if "bublee" in p.get("name","") and p.get("pm2_env",{}).get("status")=="online"]
    if online:
        for p in online:
            name = p["name"].replace("bublee-","").replace("bublee","main")
        ...
        con.print(f"  {ICON_OFFLINE} [dim]No active Bublee instances found[/dim]")
    con.print(SEP)
    ...
    con.print(f"  [dim]docs: github.com/sxrubyo/bublee[/dim]")
    con.print()
...
def cmd_status(args=""):
    ...
    for p in _pm2():
        if "bublee" not in p.get("name",""): continue
...
def cmd_list(args=""):
    idir = Path("/home/ubuntu/bublee/instances")
...
def cmd_doctor(args=""):
    ...
    con.print(f"\n  [dim]full report: bublee doctor --verbose[/dim]\n")

def cmd_new(args=""): _py("bublee_init.py")
def cmd_chat(args=""): _py("bublee_studio.py", "--instance", args.strip() or "default")
def cmd_persona(args=""): _py("bublee_persona_cli.py", *(args.split() if args else ["list"]))
def cmd_logs(args=""): _sh("pm2","logs",args.strip() or "bublee","--lines","30","--nostream")
def cmd_sync(args=""): _py("bublee_cli.py","sync")
def cmd_demo(args=""): _py("bublee_cli.py","demo",args or "")
def cmd_modelo(args=""): _py("bublee_cli.py","modelo",args or "")
def cmd_restart(args=""): _sh("pm2","restart",args.strip() or "bublee")
def cmd_stop(args=""): _sh("pm2","stop",args.strip() or "bublee")
def cmd_backup(args=""): _py("bublee_cli.py","backup",args or "")
def cmd_bridge(args=""): _py("bublee_cli.py","bridge",args or "")
def cmd_pair(args=""): _py("bublee_cli.py","pair",args or "")
def cmd_reporte(args=""): _py("bublee_weekly_report.py",args or "default")
def cmd_gaps(args=""):
...
def cmd_aprender(args=""):
    if not args or ("→" not in args and "->" not in args):
        con.print(f'  [dim]uso: bublee aprender "pregunta" → "respuesta"[/dim]'); return
    sep="→" if "→" in args else "->"
...
def cmd_config(args=""):
    env=Path("/home/ubuntu/bublee/.env")
...
def route(cmd, args=""):
    if cmd in ("--version","-v"): con.print(f"  bublee v{VERSION}"); return
    fn = CMDS.get(cmd.lower())
    if fn: fn(args)
    else: _py("bublee_cli.py", cmd, args)


# ─── Onboarding ──────────────────────────────────────────────────────────────

def first_run():
    return not Path(os.path.expanduser("~/.bublee/initialized")).exists()

def onboard():
    con.print(); con.print(LOGO_FULL)
    con.print(f"\n  [bold]First time setup.[/bold] [dim]~3 minutes.[/dim]\n")
    cmd_new()
    Path(os.path.expanduser("~/.bublee")).mkdir(parents=True,exist_ok=True)
    Path(os.path.expanduser("~/.bublee/initialized")).write_text(time.strftime("%Y-%m-%d"))
...
def _py(*a):
    try: subprocess.run([sys.executable]+[str(x) for x in a],cwd="/home/ubuntu/bublee")
