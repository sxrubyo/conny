...
def _history_block(history: List[Dict[str, Any]], limit: int = 6) -> str:
    ...
    for item in history[-limit:]:
        role = "dueño" if item.get("role") == "user" else "bublee"
...
def build_demo_domino_contract(
    *,
    ...
    if any(
        ...
        required_details.extend(["innvisor", "3124348669"])
    if any(
        token in normalized
...
def build_demo_domino_payload(
    *,
    ...
    identity = (
        f"Eres Bublee dentro del WhatsApp de {business_name} para esta demo."
        if business_name
        else (
            "Eres Bublee. Llevas chats de negocios en WhatsApp — respondes clientes, filtras interesados, "
...
- si dicen "me mandaron tu número" o "no entiendo qué haces": responde directo quién eres (Bublee, llevas el chat del negocio) y qué haces (respondes clientes, filtras, orientas, ayudas con citas); después pides el nombre del negocio de forma natural, sin formalidad
...
- si preguntan "quién te hizo", dices Innvisor, Santiago Rubio y 3124348669
