...
@lru_cache(maxsize=4)
def _bb_personality_catalog_for_path(bublee_dir: str) -> Dict[str, Dict[str, Any]]:
    bublee_path = Path(bublee_dir) / "bublee.py"
    fallback = {
        "amigable": {
    ...
    try:
        module = ast.parse(bublee_path.read_text(encoding="utf-8"))
...
def bb_personality_catalog(ctx: BBContext) -> Dict[str, Dict[str, Any]]:
    return _bb_personality_catalog_for_path(ctx.bublee_dir)
...
def bb_prompt_block(ctx: BBContext, label: str, initial: str = "") -> str:
    ...
    if editor and shutil.which(editor):
        if ctx.confirm(f"¿Abrir {editor} para editar {label.lower()}?", default=True):
            fd, tmp_path = tempfile.mkstemp(prefix="bublee-bb-", suffix=".txt")
            os.close(fd)
...
def bb_show_agent_summary(ctx: BBContext, inst: Any, persona: Optional[Dict[str, Any]] = None) -> None:
    ...
    ctx.kv("Agente", persona.get("name", "Bublee"))
...
def cmd_bb(ctx: BBContext, args: Any) -> None:
    ...
    if not action:
        ...
        ctx.section("Black Boss", "Capa operativa rápida para Bublee")
        shortcuts = [
            ("bublee bb config [n]", "Crear y ajustar el agente de una instancia"),
            ("bublee bb chat [n]", "Entrar al chat operativo"),
            ("bublee bb doctor", "Diagnóstico completo"),
            ("bublee bb sync", "Clonar runtime exacto a todas las instancias"),
            ("bublee bb new", "Crear nueva instancia"),
            ("bublee bb guide", "Abrir guía operativa"),
        ]
        for cmd_text, desc in shortcuts:
            print(f"  {ctx.q(ctx.colors.CYN, cmd_text):<30} {ctx.q(ctx.colors.G1, desc)}")
        ctx.nl()
        ctx.info("Usa el patrón: bublee bb <acción> [instancia]")
        return
    ...
    if not handler:
        ...
        ctx.info("Prueba con: bublee bb config | chat | doctor | sync | new | guide")
        return
...
def cmd_bb_config(ctx: BBContext, args: Any) -> None:
    inst = ctx.pick_instance(args, "¿Cuál agente Bublee quieres ajustar?")
    if not inst:
        return
    ...
    while True:
        ...
        if choice == 0:
            new_name = ctx.prompt("Nombre visible del agente", default=persona.get("name", "Bublee"))
