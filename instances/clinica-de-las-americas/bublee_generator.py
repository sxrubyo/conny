"""
Módulo de generación LLM para Bublee Ultra.
...
class GeneratorManager:
    """
    Gestor de generación LLM para Bublee Ultra.
    ...
    def _get_demo_engine(self, demo_model_pref: str = "auto", bmodel_key: str = ""):
        ...
        if provider == "groq" and Config.GROQ_API_KEY:
            ...
            # Antes solo probaba 3 de las 7 keys que trae la cascada de
            # clinica.env. getattr con default porque Config puede venir de
            # bublee_config.py (no incluido aquí) y no hay certeza de que
            # ya declare _4 a _7 como atributos.
            _gemini_candidates = [
                getattr(Config, "GEMINI_API_KEY", ""),
                getattr(Config, "GEMINI_API_KEY_2", ""),
                getattr(Config, "GEMINI_API_KEY_3", ""),
                getattr(Config, "GEMINI_API_KEY_4", ""),
                getattr(Config, "GEMINI_API_KEY_5", ""),
                getattr(Config, "GEMINI_API_KEY_6", ""),
                getattr(Config, "GEMINI_API_KEY_7", ""),
            ]
            key = next((k for k in _gemini_candidates if k), None)
            if key:
    ...
    async def _llm(
        ...
        """
        import logging
        log = logging.getLogger("bublee_generator")
