"""
bublee_voice.py — Humanization engine v2.
...
log = logging.getLogger("bublee.voice")
...
ROBOT_PATTERNS = [
    ...
    (re.compile(r"^(Hola|Buenos días|Buenas tardes)[,!]?\s*(Soy|Me llamo)\s*Bublee(?:,\s*(?:tu|la)\s+(?:asistente|asesora)\s+virtual)?[.!?]?(?:\s+|$)", re.I), ""),
...
class BubleeVoice:
    ...
    def humanize(self, text: str, persona_override: Optional[Dict] = None) -> str:
        ...
        result = re.sub(r'^Bublee[,:.]?\s*', '', result, flags=re.I)
