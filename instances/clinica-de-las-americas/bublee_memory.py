#!/usr/bin/env python3
"""
bublee_memory.py — Persistent memory system for Bublee multi-tenant instances.
...
log = logging.getLogger("bublee.memory")
...
class BubleeMemory:
    ...
    def __init__(self, instance_id: str = "default"):
        ...
        paths_to_try = [
            Path(f"/home/ubuntu/bublee/instances/{instance_id}"),
            Path(f"/home/ubuntu/bublee/instances/{instance_id}"),
    ...
    def load_context(self) -> str:
        """Load everything Bublee needs at the start of every conversation."""
        sections = []

        # 1. Cargar IDENTITY.md de la instancia
        identity_file = self.base / "identity" / "IDENTITY.md"
        if identity_file.exists():
            try:
                sections.append(f"## IDENTIDAD Y CONFIGURACIÓN (IDENTITY.md)\n{identity_file.read_text(encoding='utf-8').strip()}")
            except Exception as e:
                log.warning(f"[memory] error al leer IDENTITY.md: {e}")

        # 2. Cargar SOUL.md de la instancia
        soul_file = self.base / "soul" / "SOUL.md"
        if soul_file.exists():
            try:
                sections.append(f"## EVOLUCIÓN Y APRENDIZAJE (SOUL.md)\n{soul_file.read_text(encoding='utf-8').strip()}")
            except Exception as e:
                log.warning(f"[memory] error al leer SOUL.md: {e}")

        # 3. Cargar ROLE.md de la instancia
        role_file = self.base / "ROLE.md"
        if role_file.exists():
            try:
                sections.append(f"## ROL OPERATIVO (ROLE.md)\n{role_file.read_text(encoding='utf-8').strip()}")
            except Exception as e:
                log.warning(f"[memory] error al leer ROLE.md: {e}")
    ...
    def save_escalation(
        ...
        """Save an admin escalation so Bublee learns it."""
        now = datetime.now()
...
_memory_cache: Dict[str, "BubleeMemory"] = {}


def get_memory(instance_id: str = "default") -> "BubleeMemory":
    if instance_id not in _memory_cache:
        _memory_cache[instance_id] = BubleeMemory(instance_id)
