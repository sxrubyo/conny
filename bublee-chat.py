#!/usr/bin/env python3
"""
bublee-chat — Interfaz de lenguaje natural para administrar Bublee.

Uso:
    python3 bublee-chat.py                  → conecta a instancia base (puerto 8001)
    python3 bublee-chat.py clinica-bella    → conecta a instancia específica
    python3 bublee-chat.py --port 8005      → puerto específico

Ejemplo de conversación:
    Tu:     "Bublee tenemos nuevo cliente, la clínica se llama Estética Sofía,
             búscala en Google. El admin se llama Carolina, su número es 3124567890.
             Dame el token y me lo envío."
    Bublee: "Listo Santiago. Busqué Estética Sofía...
...
def print_typing(name: str = "Bublee"):
...
TEMPLATE_DIR  = "/home/ubuntu/bublee"
...
async def llm_interpret(
    ...
    # Antes solo probaba GEMINI_API_KEY, _2 y _3 — clinica.env trae cascada
    # hasta _7 justamente para esquivar rate-limit de free tier. Si las
    # primeras 3 estaban ocupadas, este CLI se quedaba sin Gemini aunque
    # hubiera 4 keys más disponibles y sin usar.
    gemini_keys = []
    if env.get("GEMINI_API_KEY"):
        gemini_keys.append(env["GEMINI_API_KEY"])
    for idx in range(2, 11):
        k = env.get(f"GEMINI_API_KEY_{idx}")
        if k:
            gemini_keys.append(k)
    for gk in gemini_keys:
        providers.append(("gemini", "native", gk, "gemini-2.0-flash"))
    ...
    hist_text = ""
    for h in history[-6:]:
        role = "Santiago" if h["role"] == "user" else "Bublee"
        hist_text += f"{role}: {h['content']}\n"

    system = f"""Eres Bublee, asistente virtual de administración de clínicas estéticas.
...
async def repl(instance: dict):
    ...
    # Header
    print(f"\n{P}{'═'*54}{NC}")
    print(f"{P}  BUBLEE CHAT{NC} — {W}{name}{NC}")
...
def main():
    ...
    if instance is None:
        ...
        print(f"  Crea una con: ./bublee-cli nuevo-cliente\n")
        sys.exit(1)
...
# python3 bublee-chat.py          → instancia base
# python3 bublee-chat.py nombre   → instancia específica
