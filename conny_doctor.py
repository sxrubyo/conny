#!/usr/bin/env python3
"""conny_doctor.py — Health check + auto-heal for Conny instances."""
...
class ConnyDoctor:
    ...
    async def _check_persona(self):
        persona_paths = [
            Path(f"/home/ubuntu/conny-instances/{self.instance_id}/personas/persona.yaml"),
            Path(f"/home/ubuntu/conny/personas/{self.instance_id}/persona.yaml"),
            Path(f"/home/ubuntu/conny/personas/{self.instance_id}/runtime_override.json"),
        ]
    ...
    def _get_port(self) -> int:
        ...
        env_path = Path(f"/home/ubuntu/conny-instances/{self.instance_id}/.env")
        if env_path.exists():
            for line in env_path.read_text().splitlines():
                if line.startswith("PORT="):
                    return int(line.split("=")[1].strip())
        # Default: main instance on 8001
        if self.instance_id in ("default", "conny"):
            return 8001
        return 8003
    ...
    async def auto_heal(self):
        ...
        for check in self.checks:
            ...
            if "PM2" in check.name and "not found" not in check.message:
                try:
                    subprocess.run(["pm2", "restart", f"conny-{self.instance_id}"], capture_output=True, timeout=10)
                    healed.append(f"Restarted PM2: conny-{self.instance_id}")
                except:
                    pass
        if healed:
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(prog="conny doctor", description="Health check for Conny instances")
    parser.add_argument("instance", nargs="?", default="conny", help="Instance ID to check")
