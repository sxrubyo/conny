...
════════════════════════════════════════════════════════════════════════════════
PITCH INTELIGENTE — Black One / Bublee v1.0
════════════════════════════════════════════════════════════════════════════════

SOLUCIONA:
  1. Bublee sin pitch cuando alguien pregunta "qué haces / cuánto cuestas"
...
CÓMO USAR en bublee.py (al final del archivo, antes del bloque __main__):

    try:
        from bublee_pitch_upgrade import (
...
log = logging.getLogger("bublee.pitch_upgrade")
...
#    alguien que recibió el número de Bublee para evaluarla como producto.
# ════════════════════════════════════════════════════════════════════════════════
...
def is_prospect_confused(user_msg: str, history: List[Dict[str, Any]]) -> bool:
    ...
    evaluando si Bublee sirve para su negocio — no un cliente final.
    """
...
def build_prospect_pitch_system_prompt(business_name: str = "") -> str:
    ...
    qué hace Bublee, cuánto cuesta, qué haría por su negocio, etc.

    Es un pitch de ventas inteligente — no robótico, no una lista de features.
    Bublee habla como la persona que LLEVARÍA el chat del negocio.
    """
    biz_ctx = f"El prospecto es de: {business_name}." if business_name else ""

    return f"""Eres Bublee, una recepcionista virtual creada por Black One — una empresa colombiana de software y gobernanza de agentes de IA.

{biz_ctx}

CONTEXTO DE ESTA CONVERSACIÓN:
Alguien recibió tu número porque quiere evaluar si Bublee sirve para su negocio.
...
QUÉ HACE BUBLEE (habla de esto con naturalidad, no como lista):
...
- Escala al humano (admin o asesor) cuando hay algo que Bublee no puede resolver
...
BUENO: "hola, soy Bublee — básicamente soy la que llevaría el WhatsApp de tu negocio ||| respondo clientes, agendo citas, hago seguimiento y escalo cuando hay algo que necesita un humano ||| ¿de qué tipo de negocio eres para mostrarte cómo quedaría?"
...
def patch_owner_demo_prompt() -> bool:
    ...
    if bublee_module is None:
        for name, mod in sys.modules.items():
            if name.startswith("bublee") and hasattr(mod, "generator"):
                bublee_module = mod
                break

    if bublee_module is None:
        log.warning("[pitch_upgrade] módulo bublee no encontrado — patch no aplicado")
        return False
...
def patch_creator_identity() -> bool:
    ...
    for attr in dir(bublee_module):
        val = getattr(bublee_module, attr, None)
        if isinstance(val, str):
            for variant in _BLACKBOSS_VARIANTS:
                if variant in val:
                    setattr(bublee_module, attr, val.replace(variant, CREATOR_NAME))
...
#
#   En bublee.py, dentro de _handle_demo_message, ANTES del bloque de LLM:
#
#       try:
#           from bublee_pitch_upgrade import (
#               is_prospect_confused,
...
def get_integration_snippet() -> str:
    """Retorna el snippet de integración listo para pegar en bublee.py."""
    return '''
...
try:
    ...
    # 2. ¿El prospecto está confundido sobre qué es Bublee? → Pitch
    if is_prospect_confused(text, history):
...
try:
    from bublee_pitch_upgrade import fix_creator_in_response
