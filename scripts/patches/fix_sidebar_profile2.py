...
# Add state for profile inside Sidebar
if 'const [profile, setProfile]' not in content:
    state_code = """
  const [profile, setProfile] = useState<any>({ name: "Santiago", email: "Santi21435@gmail.com", avatar: "" });
...
"""
    content = re.sub(r'(export function Sidebar\(\{.*?\}: SidebarProps\) \{)', r'\1\n' + state_code, content)

# Replace the hardcoded bottom left avatar
...
content = re.sub(r'<img\s*src="https://images\.unsplash\.com/photo-1531427186611-ecfd\s*6d936c79\?w=900&auto=format&fit=crop&q=60"[^>]*>', replacement, content)

# Replace hardcoded Santiago text
