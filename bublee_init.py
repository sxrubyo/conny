#!/usr/bin/env python3
"""bublee init — Infrastructure Provisioning Wizard v2."""
...
CURRENT_LANG = os.environ.get("BUBLEE_INIT_LANG", "en").lower()

TRANSLATIONS = {
    "en": {
        "Selección de idioma": "Language selection",
        "Idioma preferido para Bublee": "Preferred language for Bublee",
        ...
        "¿Cómo quieres exponer la página web de Bublee?": "How do you want to expose Bublee's web dashboard?",
...
from bublee_tui_select import select_menu as _orig_select_menu, confirm as _orig_confirm, text_input as _orig_text_input
try:
    from bublee_runtime_ops import mirror_instance_env_to_base, set_active_instance, start_localhost_run_tunnel
except Exception:
...
BUBLEE_HOME = Path(os.environ.get("BUBLEE_HOME", str(Path.home() / ".bublee")))
INSTANCES_DIR = Path(os.environ.get("INSTANCES_DIR", str(BUBLEE_HOME / "instances")))
BUBLEE_DIR = Path(os.environ.get("BUBLEE_DIR", os.path.dirname(os.path.abspath(__file__))))
...
def _persist_language(lang: str) -> None:
    try:
        workspace_config_path = Path(os.environ.get("BUBLEE_WORKSPACE_CONFIG", str(BUBLEE_HOME / "config.json")))
...
def _configure_dashboard(port: int) -> dict:
    ...
    choice = select_menu(options, title="¿Cómo quieres exponer la página web de Bublee?")
    if choice == 0:
        return {
            "host": "127.0.0.1",
...
def run_wizard():
    ...
    print()
    print(f"  {C_PRIMARY}{B1}Bublee CLI {VERSION}{R} · {C_ACCENT}Autonomous Dynamic Receptionist{R} · {C_MUTED}⏱ {_t('Tiempo estimado: ~3 min')}{R}")
    ...
    i = select_menu([l[1] for l in LANGUAGES], title="Idioma preferido para Bublee")
    lang = LANGUAGES[i][0]
    global CURRENT_LANG
    CURRENT_LANG = lang
  ...
  {C_SUCCESS}{B1}🚀 {_t('Infraestructura Desplegada Exitosamente')}{R}
    ...
    {C_MUTED}{_t('Control:')}{R}     {C_PRIMARY}bublee status {instance_id}{R}
    {C_MUTED}{_t('Check:')}{R}       {C_PRIMARY}bublee doctor{R}
    {C_MUTED}{_t('Lanzar:')}{R}      {C_PRIMARY}pm2 start {INSTANCES_DIR / instance_id}/run.sh --name bublee-{instance_id}{R}
""")
...
def _create(name, iid, sector, channel, provider, model_id, tone, secrets_map, lang, port, gateway, dashboard):
    ...
    webhook_secret = f"bublee_{iid}_{secrets.token_hex(6)}"
    ...
    sys_prompt = f"""Eres Bublee, recepcionista autónoma de {name} (Sector: {sector}).
Tu tono de voz es {tone}. Responde por {channel}.
Idioma principal: {lang}
"""
    (idir / "bublee_system_prompt.txt").write_text(sys_prompt)
...
""")
    ...
    core = ["bublee.py", "src/core/admin_engines.py", "src/core/production_monitor.py", "bublee_config.py",
            "bublee_utils.py", "bublee_commands.py", "bublee_learning.py", "bublee_voice.py",
            "bublee_uncertainty.py", "bublee_memory_engine.py", "bublee_admin_api.py",
            "bublee_cron.py", "bublee_nova_proxy.py", "bublee_smart_features.py",
            "bublee_web_search.py", "bublee_google_auth.py", "run.sh", "requirements.txt",
            "bublee_design.py", "bublee_i18n.py", "bublee_cli_bb.py"]
    ...
    (idir / "bublee.state.json").write_text(json.dumps(state, indent=2))
