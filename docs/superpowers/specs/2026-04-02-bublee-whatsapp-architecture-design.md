# Bublee WhatsApp Architecture Redesign

Date: 2026-04-02
Owner: Santiago
Scope: Bublee production architecture for WhatsApp-first conversational sales and support

## Why This Exists

Bublee is no longer competing against simple n8n bots or prompt wrappers.
...
The current Bublee runtime mixes too many concerns in one place, so good model output gets flattened into repetitive, rigid, scripted behavior.
...
Bublee cannot win by being "another chat bot".
Bublee has to win by being:
...
Gemini 2.5 Flash should be Bublee's real-time production default for:
...
Bublee is powerful, but structurally overloaded.
...
- `bublee.py` `ResponseGenerator`
- `bublee.py` `BubleeUltra.process_message`
- `bublee.py` demo path
- `bublee.py` `ConversationSimulator`
- `bublee.py` `TrainerGateway`
- `bublee.py` `OwnerStyleController`
- `bublee.py` `PromptEvolver`
...
Bublee has one logic for normal users and another for demo mode. That creates two personalities and two different failure surfaces.
...
Bublee remembers some facts and preferences, but not yet a strong structured view of:
...
Bublee must behave like a real business operator, not like a generic AI.
...
- `personas/bublee/base/*.yaml`
- `personas/bublee/overlays/*.yaml`
- `personas/clients/*.yaml`

Each Bublee persona must include:
...
- `bublee_lab/scenarios/*.yaml`
- `bublee_lab/personality_matrix.py`
- `bublee_lab/scoring.py`
- `bublee_lab/runner.py`
...
Bublee must behave like a WhatsApp-native operator.
...
Keep Bublee production working by routing only:
...
out of `bublee.py` and into the new core.
...
If admin rules keep generating text instead of constraining it, Bublee will keep sounding rigid.
Mitigation: enforce policy-only control semantics.

## Recommendation

Do not keep growing `bublee.py` as the primary innovation surface.
...
That is the path to make Bublee feel less like "code" and more like a real operator.
