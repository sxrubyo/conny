# Conny WhatsApp Architecture Redesign

Date: 2026-04-02
Owner: Santiago
Scope: Conny production architecture for WhatsApp-first conversational sales and support

## Why This Exists

Conny is no longer competing against simple n8n bots or prompt wrappers.
...
The current Conny runtime mixes too many concerns in one place, so good model output gets flattened into repetitive, rigid, scripted behavior.
...
Conny cannot win by being "another chat bot".
Conny has to win by being:
...
Gemini 2.5 Flash should be Conny's real-time production default for:
...
Conny is powerful, but structurally overloaded.
...
- `conny.py` `ResponseGenerator`
- `conny.py` `ConnyUltra.process_message`
- `conny.py` demo path
- `conny.py` `ConversationSimulator`
- `conny.py` `TrainerGateway`
- `conny.py` `OwnerStyleController`
- `conny.py` `PromptEvolver`
...
Conny has one logic for normal users and another for demo mode. That creates two personalities and two different failure surfaces.
...
Conny remembers some facts and preferences, but not yet a strong structured view of:
...
Conny must behave like a real business operator, not like a generic AI.
...
- `conny_core/conversation_engine.py`
...
- `conny_core/persona_registry.py`
- `personas/conny/base/*.yaml`
- `personas/conny/overlays/*.yaml`
- `personas/clients/*.yaml`

Each Conny persona must include:
...
- `conny_lab/scenarios/*.yaml`
- `conny_lab/personality_matrix.py`
- `conny_lab/scoring.py`
- `conny_lab/runner.py`
...
Conny must behave like a WhatsApp-native operator.
...
Keep Conny production working by routing only:
...
out of `conny.py` and into the new core.
...
If admin rules keep generating text instead of constraining it, Conny will keep sounding rigid.
Mitigation: enforce policy-only control semantics.

## Recommendation

Do not keep growing `conny.py` as the primary innovation surface.
...
That is the path to make Conny feel less like "code" and more like a real operator.
