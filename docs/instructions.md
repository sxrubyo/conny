# Bublee v10.0 — System Instructions & Agent Programming Capabilities

## Architecture

```
Port Map:
  8001 → bublee (demo mode, shared telegram router)
  8003 → bublee-clinica-de-las-americas (production)
  8002 → whatsapp-bridge (Baileys)
  9001 → nova-core (governance, optional)

PM2 processes:
  bublee                         → /home/ubuntu/bublee/run.sh
  bublee-clinica-de-las-americas → /home/ubuntu/bublee/instances/clinica-de-las-americas/run.sh
  whatsapp-bridge                 → Baileys bridge
  nova-core                       → Nova governance server

Key modules:
  bublee.py              → monolith engine (1.3MB, all core logic)
  bublee_admin_api.py    → FastAPI /admin endpoints (persona, model, teach, gaps, chats, dashboard)
  bublee_memory_engine.py→ episodic + semantic + procedural memory with TF-IDF recall
  bublee_voice.py        → humanization post-processor (robot pattern removal)
  bublee_uncertainty.py  → confidence scoring + admin alerts
  bublee_nova_proxy.py   → transparent LLM proxy (memory + governance + voice)
  bublee_brain_v10.py    → conversation intelligence (LLM-first, anti-loop)
  bublee_domino.py       → response quality control
  nova_bridge.py          → Nova governance client (validate before send)
  bublee_cron.py         → APScheduler (weekly memory consolidation, Sun 3am)
  bublee_send_guard.py   → quality control guard rails protecting against fragment ejections
```
...
3. **Validate exhaustively.** Run `pytest tests/` + `pm2 logs bublee --lines 20` after changes.
...
- Telegram: shared router on main instance, routing by `chat_id` via `bublee pair`.
- WhatsApp: one bridge session per instance, never share `auth_info_*` folder.
- Core code sync: `bublee_core/`, `bublee_agents/`, brain, domino — shared across instances.
- Client data: NEVER sync databases, conversations, or credentials.

## Response Rules (enforced by bublee_voice.py & bublee_send_guard.py)
...
- "Hola! Soy Bublee, tu asistente virtual"
...
- **Guard Rails (bublee_send_guard.py):** If the LLM response contains multiple bubbles (`|||`), short or severe fragments (e.g. "ok") at the end must be popped while keeping preceding rich bubbles, instead of discarding the entire response.
...
- **`$frontend-skill`**: Standard for beautiful user interfaces. Emphasizes custom HSL color systems, glassmorphism, responsive panels, typography, and polished micro-animations. Avoids Tailwind unless explicitly requested.
  - *Trigger:* When iterating on or designing new views for the Bublee Admin Dashboard.
- **`$video-to-website`**: Dynamic scroll-driven animated sites.
  - *Trigger:* When building landing pages or product showcases for Bublee.
...
- **`$systematic-debugging`**: Root-cause analysis before any code changes. Isolates errors systematically across Bublee's voice filters, domino checks, and LLM proxies.
...
- **`$memory-management`** & **`$agentdb-vector-search`**: Manages semantic lookup and HNSW fast searches inside Bublee's memory engines.
...
pm2 logs bublee --lines 50 --nostream | grep -i error
...
# Run tests
cd /home/ubuntu/bublee && .venv/bin/python -m pytest tests/ -v
```
...
5. Propagate runtime changes to all sub-instances with `bublee sync`.
6. `pm2 restart bublee` → verify with health endpoint and dashboard access.
