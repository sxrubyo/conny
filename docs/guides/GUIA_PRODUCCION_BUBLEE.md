# Bublee en Producción

Guía corta para operar Bublee esta noche sin mezclar promesas con realidad.
...
- Bublee conserva el archivo original y el texto extraído cuando aplica
- la carpeta se copia con `bublee new`, `bublee clone` y `bublee sync`
...
- El enrutado correcto se define con `bublee pair`.

Comandos útiles:

```bash
bublee pair list
bublee pair default clinica-de-las-americas
bublee pair clinica-de-las-americas 123456789
bublee pair detach 123456789
```
...
- si el admin pide cambio de tono, Bublee debe ajustarse
- si algo no se entiende, debe pedir una aclaración corta

## Burbujas y buffering

Bublee no responde una burbuja por una burbuja cuando el contexto llega partido.
...
`instance.json`, Bublee ahora rellena automáticamente:
...
- no asumir que Bublee ve todo tu historial de Gmail si no existe integración explícita
