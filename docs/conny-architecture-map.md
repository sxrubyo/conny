# Conny Architecture Map

## Qué es Conny

Conny es un runtime base para recepcionistas IA orientadas a WhatsApp y Telegram. El repo público contiene el core reusable: conversación, memoria corta, personalidad, canales, API, CLI y sincronización hacia instancias de negocio. El estado vivo de cada negocio queda fuera del repo.
...
- `personas/conny/base/default.yaml`
  Contrato base de voz.
- `personas/conny/base/estetica_whatsapp.yaml`
  Override específico para estética/WhatsApp.
- `conny_core/prompt_ops.py`
...
- `conny.py`: ~28.5k líneas
- `conny_cli.py`: ~11k líneas
...
- mover builders auxiliares de identidad/contexto que todavía viven dentro de `conny.py`

### Fase 2

- mover browser admin de conversaciones a `conny_admin/conversation_review_ops.py`
- separar auth/admin lifecycle de `conny.py`

### Fase 3

- partir `conny_cli.py` en:
  - `conny_cli_instances.py`
  - `conny_cli_runtime.py`
  - `conny_cli_common.py`
...
```bash
python3 conny_cli.py --help
python3 conny_cli.py guide
python3 conny_cli.py bb config
```
...
- `conny.py` todavía mezcla HTTP, DB, LLM, prompts, canales y helpers de negocio.
- `conny_cli.py` todavía mezcla UX, PM2, bridge, sync, config y entrenamiento.
