...
<head>
    ...
    <style>
        /* BASE THEME (Dark/Professional by default since video is background) */
        :root {
            --bg-void: transparent; /* Video is behind */
            --bg-surface: rgba(17, 17, 24, 0.6); /* Semi-transparent for glass effect */
            ...
            --gradient-card: linear-gradient(145deg, rgba(26,26,36,0.6) 0%, rgba(17,17,24,0.6) 100%);
            ...
            --invert-logo: 1; /* Keep logo white */
        }

        /* Light theme overrides (optional, but for video dark is best) */
        :root.light-theme {
        ...
        * {
        ...
        body {
            background-color: #05050A; /* Fallback */
        ...
        .video-overlay {
            ...
            width: 100vw;
            height: 100vh;
            background: rgba(5, 5, 10, 0.75); /* Darken professionally */
            z-index: -1;
            pointer-events: none;
        }
        ...
        .hero h1 {
            ...
            margin-bottom: 32px;
            text-shadow: 0 4px 32px rgba(0,0,0,0.5); /* Strong shadow for video contrast */
        }
...
<body class="dark-theme">
    ...
    <script>
        ...
        video.addEventListener('loadedmetadata', function() {
            // Once we know the video duration, set up the ScrollTrigger
            let tlVideo = gsap.timeline({
                scrollTrigger: {
                    trigger: "body",
                    start: "top top",
                    end: "bottom bottom",
                    scrub: 1.5, // 1.5s smoothing effect so it feels fluid
                }
            });
            // Animate video's currentTime from 0 to its full duration over the scroll length
            tlVideo.fromTo(video, { currentTime: 0 }, { currentTime: video.duration, ease: "none" });
        });
        // Optional fallback: force metadata load by a gentle play/pause if stuck (browser policies)
