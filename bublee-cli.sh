#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# bublee-cli — Deploy y gestión de instancias Bublee
#
# Uso:
#   ./bublee-cli nuevo-cliente            → crea nueva instancia interactivamente
#   ./bublee-cli nuevo-cliente mi-clinica → nombre directo
#   ./bublee-cli listar                   → ver todas las instancias activas
#   ./bublee-cli estado mi-clinica        → ver estado de una instancia
#   ./bublee-cli detener mi-clinica       → detener instancia
#   ./bublee-cli reiniciar mi-clinica     → reiniciar instancia
#   ./bublee-cli eliminar mi-clinica      → eliminar instancia completamente
#   ./bublee-cli logs mi-clinica          → ver logs en vivo
# ═══════════════════════════════════════════════════════════════════════════════
...
if [ -t 0 ] || [[ "$COMMAND" != "nuevo-cliente" && "$COMMAND" != "new" && "$COMMAND" != "nuevo" ]]; then
    exec node "$SCRIPT_DIR/npm/bublee.js" "$@"
fi
# ── Configuración ──────────────────────────────────────────────────────────────
BUBLEE_TEMPLATE="$SCRIPT_DIR"                                               # instancia base/template
INSTANCES_DIR="${BUBLEE_INSTANCES_DIR:-$(dirname "$SCRIPT_DIR")/bublee-instances}" # donde viven los clientes
...
instance_db()   { echo "$INSTANCES_DIR/$1/bublee.db"; }
pm2_name()      { echo "bublee-$1"; }
...
cmd_nuevo_cliente() {
    ...
    if [ -d "$DIR" ]; then
        ...
        info "Usa: ./bublee-cli estado $NAME"
        exit 1
    fi
    ...
    # ── Master API Key ────────────────────────────────────────────────────────
    MASTER_KEY="bublee_$(openssl rand -hex 8 2>/dev/null || echo "${NAME}_$(date +%s)")"
    ...
    # 2. Copiar archivos de Bublee (sin DB, sin .env — sesión nueva limpia)
    for f in bublee.py search.py knowledge_base.py requirements.txt; do
        if [ -f "$BUBLEE_TEMPLATE/$f" ]; then
            cp "$BUBLEE_TEMPLATE/$f" "$DIR/$f"
        fi
    done
    success "Archivos copiados"

    # 3. Copiar .env base y personalizarlo
    cp "$BUBLEE_TEMPLATE/.env" "$DIR/.env.base" 2>/dev/null || true
    ...
    if [ -f "$BUBLEE_TEMPLATE/.env" ]; then
        GROQ_KEY=$(grep "^GROQ_API_KEY=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY=$(grep "^GEMINI_API_KEY=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_2=$(grep "^GEMINI_API_KEY_2=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_3=$(grep "^GEMINI_API_KEY_3=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_4=$(grep "^GEMINI_API_KEY_4=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_5=$(grep "^GEMINI_API_KEY_5=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_6=$(grep "^GEMINI_API_KEY_6=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_7=$(grep "^GEMINI_API_KEY_7=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        OPENROUTER_KEY=$(grep "^OPENROUTER_API_KEY=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        OPENAI_KEY=$(grep "^OPENAI_API_KEY=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        SERP_KEY=$(grep "^SERP_API_KEY=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
        BRAVE_KEY=$(grep "^BRAVE_API_KEY=" "$BUBLEE_TEMPLATE/.env" | cut -d= -f2-)
    fi

    # Escribir .env limpio para esta instancia
    cat > "$DIR/.env" << ENVEOF
# ═══════════════════════════════════════════════════════════════════════════════
# Bublee — Instancia: $NAME
...
HOST=0.0.0.0
BASE_URL=$BASE_URL
WEBHOOK_SECRET=bublee_${NAME}_$(openssl rand -hex 4 2>/dev/null || echo "secret")

# Base de datos — aislada por cliente
DB_PATH=$DIR/bublee.db
...
ENVEOF
    ...
    pm2 start "$DIR/bublee.py" \
        --name "$PM2_NAME" \
        --interpreter "python3" \
        --restart-delay=3000 \
        --max-restarts=10 \
        --log "$DIR/logs/bublee.log" \
    ...
    echo    "  ./bublee-cli logs $NAME       → logs en vivo"
    echo    "  ./bublee-cli estado $NAME     → estado"
    echo    "  ./bublee-cli detener $NAME    → detener"
    echo ""
...
cmd_listar() {
    header "Instancias Bublee activas"
    ...
    if [ ! -d "$INSTANCES_DIR" ] || [ -z "$(ls -A "$INSTANCES_DIR" 2>/dev/null)" ]; then
        ...
        echo "  Crea una con: ./bublee-cli nuevo-cliente"
        echo ""
        return
    fi
    ...
    echo ""
    echo "  Para gestionar: ./bublee-cli [estado|detener|reiniciar|logs|eliminar] [nombre]"
    echo ""
}
...
cmd_logs() {
    ...
    pm2 logs "$(pm2_name "$name")" 2>/dev/null || \
        tail -f "$(instance_dir "$name")/logs/bublee.log" 2>/dev/null || \
...
case "$COMMAND" in
    "chat"|"bublee-chat")
        ...
        CHAT_SCRIPT="$(dirname "$0")/bublee-chat.py"
        if [ ! -f "$CHAT_SCRIPT" ]; then
            error "bublee-chat.py no encontrado en $(dirname "$0")"
            exit 1
        fi
    ...
    *)
        echo ""
        echo -e "${PURPLE}  bublee-cli${NC} — Gestión de instancias Bublee"
        echo ""
        echo -e "  ${CYAN}Comandos:${NC}"
        echo    "  chat [nombre]             Hablar con Bublee en lenguaje natural"
        ...
        echo    "  ./bublee-cli chat                    → hablar con la instancia base"
        echo    "  ./bublee-cli chat clinica-bella      → hablar con instancia específica"
        echo    "  ./bublee-cli nuevo-cliente           → crear nuevo cliente"
        echo    "  ./bublee-cli listar"
        echo ""
        ;;
esac
