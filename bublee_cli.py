...
Comandos:
  bublee install           Instalar CLI globalmente (hazlo primero)
  bublee init              Primer setup guiado del producto
  bublee new               Crear instancia para un cliente (wizard)
  bublee list              Ver todas las instancias
  bublee dashboard         Panel en tiempo real
  bublee chat [n]          Generar tokens, probar bot, ver webhook
  bublee sync              Clonar runtime compartido actualizado a todos los clientes
  bublee sync-check        Vista previa de qué archivos se sincronizarían (dry-run)
  bublee sync-manifest     Ver / editar el manifiesto de archivos sincronizables
  bublee fix [n]           Reparar instancias con bug de --cwd (bot no responde)
  bublee status [n]        Estado detallado
  bublee health            Health check rápido de todas
  bublee logs [n]          Logs en tiempo real
  bublee restart [n]       Reiniciar (o 'all')
  bublee webhooks [n]      Ver/reconfigurar webhook de Telegram
  bublee mcp               Inicia el servidor Model Context Protocols [n]
  bublee config [n]        Hub de configuración o edición de instancia
  bublee guide             Guía completa de operación
  bublee doctor            Diagnóstico del sistema
  bublee upgrade           Actualizar + propagar a todos los clientes
  bublee metrics [n]       Métricas de uso
  bublee stats [n]         Estadísticas de conversaciones
  bublee export [n]        Exportar a JSON/CSV
  bublee import [file]     Importar configuración
  bublee test [n]          Probar respuestas
  bublee stop [n]          Detener
  bublee delete [n]        Eliminar
  bublee clone [n]         Clonar instancia
  bublee reset [n]         Resetear sesión
  bublee backup [n]        Crear snapshot
  bublee restore [file]    Restaurar snapshot
  bublee scale [n] [num]   Escalar workers
  bublee audit             Auditoría de seguridad
  bublee benchmark         Test de rendimiento
  bublee secure            Guía de seguridad
  bublee rotate-keys       Rotar secrets
  bublee billing           Uso de APIs y costos
  bublee omni [sub]        Monitoreo central
  bublee nova [sub]        Motor Nova
  bublee gateway [n]       Gateway automático de skills y reglas
  bublee control [n]       Control duro del admin sobre frases, saludo y trato

V8.0 — NUEVOS COMANDOS:
  bublee modelo [n]        Cambiar modelo LLM en caliente (sin reiniciar)
  bublee simular [n]       Simular 10 conversaciones — detecta frases de bot
  bublee v8 [n]            Estado de los 15 sistemas V8
  bublee quality [n]       Score de humanidad de últimas respuestas
  bublee briefing [n]      Briefing diario: leads calientes + acción recomendada
  bublee cost [n]          Estimación de costo mensual del LLM
  bublee latency [n]       Test de latencia HTTP + LLM
  bublee diff [n1] [n2]    Comparar configuración entre instancias
  bublee watchdog          Monitor continuo con auto-restart inteligente
  bublee warmup [n]        Precalentar LLM antes de lanzar
  bublee campana [n]       Campañas de seguimiento y reactivación
  bublee env-check [n]     Verificar .env contra mejores prácticas V8
  bublee rollforward [n]   Aplicar bublee.py a instancias
  bublee dashboard-v8      Dashboard enriquecido con métricas V8

LOG ENGINE — Análisis inteligente de errores:
  bublee logs [n]           Stream en vivo con colores + alerta de errores
  bublee logs [n] --scan    Escanear logs: detecta errores con diagnóstico
  bublee logs [n] --fix     Detectar errores + aplicar fixes automáticos
  bublee logs [n] --errors  Mostrar solo líneas de error
  bublee logfix [n]         Atajo: logs --fix
  bublee logscan [n]        Atajo: logs --scan
"""
...
try:
    from bublee_i18n import get_i18n, set_language, SUPPORTED_LANGUAGES, LANGUAGE_MENU, detect_user_language
    _I18N = get_i18n()
except ImportError:
    class FakeI18n:
...
if [ -x "$INSTANCE_DIR/.venv/bin/python" ]; then
  exec "$INSTANCE_DIR/.venv/bin/python" bublee.py
fi

if [ -x "$HOME/.bublee/runtime/bin/python" ]; then
  exec "$HOME/.bublee/runtime/bin/python" "$INSTANCE_DIR/bublee.py"
fi

if command -v python3 >/dev/null 2>&1; then
  exec python3 "$INSTANCE_DIR/bublee.py"
fi

exec python "$INSTANCE_DIR/bublee.py"
"""
...
VERSION = "9.8.0"
BUBLEE_HOME = os.getenv("BUBLEE_HOME", str(Path.home() / ".bublee"))
BUBLEE_DIR = os.getenv("BUBLEE_DIR", str(Path(__file__).resolve().parent))
INSTANCES_DIR = _default_instances_dir()
NOVA_DIR = os.getenv("NOVA_DIR", "/home/ubuntu/nova-os")
BACKUP_DIR = Path(os.getenv("BUBLEE_BACKUPS", Path.home() / "bublee-backups"))
CACHE_DIR = Path(BUBLEE_HOME) / "cache"
...
# IMPORTANTE: el Caddyfile vive SIEMPRE en xus-https, nunca en la carpeta de Bublee.
...
SHARED_TELEGRAM_ROUTES = Path(
    os.getenv("BUBLEE_SHARED_TELEGRAM_ROUTES", f"{BUBLEE_HOME}/shared_telegram_routes.json")
)
WORKSPACE_CONFIG_PATH = Path(os.getenv("BUBLEE_WORKSPACE_CONFIG", f"{BUBLEE_HOME}/config.json"))
...
#   4. Manifiesto           — ~/.bublee/sync_manifest.json (lista extra del usuario)
#
...
SYNC_RUNTIME_FILES = [
    ...
    "run.sh",
    "bublee_doctor.py",
    "bublee_runtime_ops.py",
    "bublee_ultra_config.py",
    "src/core/globals.py",
    "src/core/runtime.py",
    ...
    "bublee_brain_v10.py",
    "bublee_domino.py",
    "bublee_memory_engine.py",
    "bublee_uncertainty.py",
    "bublee_voice.py",
    "bublee_voice_engine.py",
    "bublee_nova_proxy.py",
    "bublee_cron.py",
    "bublee_learning.py",
    "bublee_commands.py",
    "bublee_admin_api.py",
    ...
    "bublee_audio.py",
    "bublee_i18n.py",
    "bublee_send_guard.py",
    "bublee_nuke_robot_phrases.py",
    "bublee_pitch_upgrade.py",
    "bublee_memory.py",
    "search.py",
    "knowledge_base.py",
    "brand_assets.py",
...
_SYNC_EXCLUDE_NAMES: set = {
    ...
    "bublee_pairing.py",
    "bublee_patch.py",
    "bublee_sync_fix.py",   # helper temporal de fixes, no es runtime
}
...
_SYNC_MANIFEST_PATH = Path(BUBLEE_HOME) / "sync_manifest.json"


def _load_sync_manifest() -> dict:
    """Carga el manifiesto desde ~/.bublee/sync_manifest.json."""
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTORES - La clave para escalar Bublee
# ══════════════════════════════════════════════════════════════════════════════
@dataclass
class Sector:
    id: str
...
def print_logo(compact=False, sector=None):
    ...
    logo_path = Path(os.environ.get("BUBLEE_DIR", os.path.dirname(os.path.abspath(__file__)))) / "brand-assets" / "bublee-logo.png"
    if logo_path.exists():
        try:
    ...
    try:
        from src.bublee.channels.logo_art import LOGO_ART_LINES
    except ImportError:
        try:
            from bublee.channels.logo_art import LOGO_ART_LINES
        except ImportError:
            LOGO_ART_LINES = ["Bublee"]
    ...
    text_lines = [
        "",
        "",
        f"  {C.P1}{C.BOLD}Bublee CLI {VERSION}{C.R}",
...
def ensure_workspace_files() -> None:
    Path(BUBLEE_HOME).mkdir(parents=True, exist_ok=True)
...
def runtime_defaults() -> Dict[str, Any]:
    config = load_workspace_config()
    legacy_env = load_env(f"{BUBLEE_DIR}/.env")
...
def workspace_summary(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    section("Workspace Bublee", "Producto limpio y configurable")
    ...
    kv("Agente", cfg.get("agent", {}).get("display_name", "Bublee"))
    nl()
...
def edit_workspace_config() -> Dict[str, Any]:
    ...
    agent_cfg["display_name"] = prompt("Nombre base del agente", default=agent_cfg.get("display_name", "Bublee")).strip() or "Bublee"
...
def _pm2_start_runtime(pm2_name: str, inst_dir: str, log_path: str, error_path: str) -> subprocess.CompletedProcess:
    """Arrancar Bublee siempre a través de run.sh para respetar VENV/runtime real."""
...
class Instance:
    ...
    def pm2_name(self) -> str:
        ...
        return "bublee" if self.is_base else f"bublee-{self.name}"

    @property
    def db_path(self) -> str:
        return self.env.get("DB_PATH", f"{self.dir}/bublee.db")
...
def get_instances() -> List[Instance]:
    ...
    # Base instance
    base_env = load_env(f"{BUBLEE_DIR}/.env")
    if base_env:
...
def cmd_init(args):
    """Onboarding guiado del producto Bublee."""
    import subprocess
    import sys
    subprocess.call([sys.executable, str(Path(os.environ.get("BUBLEE_DIR", os.path.dirname(os.path.abspath(__file__)))) / "bublee_init.py")])
    return
    print_logo()
    ensure_workspace_files()
    section("Bublee Init", "Primer setup guiado del producto")
    info("Este init no usa configuraciones del autor. Crea tu workspace limpio en ~/.bublee.")
    nl()

    info("Sistema:")
    kv("OS", platform.system())
    kv("Python", sys.version.split()[0])
    kv("Bublee home", BUBLEE_HOME)
    ...
    nl()
    info("Bublee ya puede nacer como producto abierto sin estado previo.")
    info("Cuando hables con Bublee desde Telegram como admin podrás afinarla con:")
    ...
    info("Siguiente paso: bublee new")
    info("Luego: bublee config o bublee bb config para ajustar prompt y personalidad")
    nl()
...
def cmd_new(args):
    ...
    if os.path.isdir(inst_dir):
        ...
        info(f"Usa 'bublee status {name}' para ver su estado")
        return
    ...
    if platform_choice == "telegram":
        ...
        if shared_base_token and confirm("¿Usar el token compartido de Bublee base para Telegram?"):
    ...
    with Spinner(f"Creando '{name}'...") as sp:
        ...
        _clone_runtime_entries(BUBLEE_DIR, inst_dir)
        
        # .env
        env_content = f"""# Bublee — {name}
# Sector: {sector.name}
...
HOST=0.0.0.0
BASE_URL={base_url}
WEBHOOK_SECRET=bublee_{name}_{webhook_secret}

# DB
DB_PATH={inst_dir}/bublee.db
...
"""
    ...
    if agent_seed and any(str(agent_seed.get(key, "")).strip() for key in ("display_name", "role", "prompt_master")):
        ...
        _bb_apply_persona(
            seeded_inst,
            {
                "name": agent_seed.get("display_name") or "Bublee",
    ...
    kv("Master Key", mask_secret(master_key))
    nl()

    webhook_url_real = f"{base_url}/webhook/bublee_{name}_{webhook_secret}"
    if telegram_shared:
        ...
        print(f"       {q(C.CYN, 'El webhook real lo gestiona Bublee base (/telegram/shared/...)')}")
    ...
    webhook_path = f"webhook/bublee_{name}_{webhook_secret}"
    if telegram_shared:
    ...
    info(f"Genera el token con: bublee chat {name}")
    nl()

    # ── Auto-generar token de activacion y mostrarlo ──────────────────────────
    time.sleep(3)  # dar tiempo a que Bublee arranque
    h_final = health(port)
    if h_final and master_key:
        try:
            ...
            info(f"Genera el token con: bublee token {name}")
    else:
        info(f"Genera el token con: bublee token {name}")
...
def cmd_template(args):
    ...
    if sector_id and sector_id in SECTORS:
        ...
        hr()
        info("Usa: bublee template <sector_id>")
        info("Ejemplo: bublee template dental")
        nl()
...
def cmd_status(args):
    ...
    for inst in instances:
        ...
        kv("PM2", f"bublee-{inst.name}" if not inst.is_base else "bublee")
        nl()
...
_LOG_ERROR_PATTERNS = [
    ...
    (
        ...
        "bublee config {name}  → cambia TELEGRAM_TOKEN\nLuego: bublee restart {name}",
    ),
    (
    ...
    (
        ...
        "bublee webhooks {name}  → reconfigurar webhook",
    ),
    (
        re.compile(r"TimeoutError|asyncio.*timeout|timed out", re.I),
        "MEDIO", "Timeout",
        "Una llamada al LLM o API tardó demasiado.",
        "Normal si es puntual. Si es frecuente: bublee latency {name}",
    ),
    (
        ...
        "bublee doctor  → verificar espacio\nbublee restart {name}",
    ),
    (
        ...
        "bublee sync  → clona el runtime compartido actualizado\nbublee restart {name}",
    ),
    (
        ...
        "bublee config {name}  → agregar API key",
    ),
    (
    ...
    (
        ...
        "El motor Nova no responde — Bublee funciona sin él.",
        "bublee nova status  → verificar Nova (opcional)",
    ),
]
...
def _llm_diagnose_unknown(unmatched_lines: List[str], env: Dict[str, str]) -> Optional[Dict]:
    """
    Fallback para líneas que PARECEN error pero no matchearon ninguno de
    los 14 _LOG_ERROR_PATTERNS conocidos. Antes esas líneas quedaban
    invisibles: el doctor podía reportar "sin errores" aunque hubiera algo
    raro en el log, simplemente porque no estaba en la lista de patrones.

    Se llama UNA sola vez por escaneo (no por línea), y solo cuando hay
    líneas sin clasificar — en el caso común, donde los 14 patrones ya
    cubren todo, esto no agrega ninguna llamada ni latencia extra.
    """
    if not _HTTPX or not unmatched_lines:
        return None
    ...
    sample = "\n".join(unmatched_lines[-15:])[:3000]
    prompt = (
        "Estas son líneas de log de un bot de WhatsApp en Python que NO "
        "coincidieron con ningún patrón de error conocido de una lista fija. "
        "En español, en máximo 2 frases muy cortas: qué está pasando "
        "probablemente y qué acción tomarías. Sin relleno, directo.\n\n" + sample
    )
    try:
        r = _httpx.post(
        ...
        return {
            "severity":  "MEDIO",
            "title":     "Error no reconocido (analizado por LLM)",
            "diagnosis": text,
            "fix":       "bublee logs {name} --errors  → ver líneas completas",
            "context":   unmatched_lines[-1][:120],
...
def _analyze_errors(lines: List[str], inst_name: str, env: Optional[Dict[str, str]] = None) -> List[Dict]:
    """
    Analiza líneas de log y devuelve lista de errores detectados con su fix.
    Deduplica — no reporta el mismo patrón dos veces.

    Si se pasa `env` (con alguna GEMINI_API_KEY disponible), las líneas que
    parecen error pero no matchean ninguno de los patrones conocidos se
    mandan una vez a _llm_diagnose_unknown en vez de desaparecer en silencio.
    """
    found = []
    seen_patterns = set()
    ...
    if env:
        generic_error_lines = [
            ...
            and l not in matched_lines
        ]
        if generic_error_lines:
            llm_entry = _llm_diagnose_unknown(generic_error_lines, env)
            if llm_entry:
                llm_entry["fix"] = llm_entry["fix"].replace("{name}", inst_name)
                found.append(llm_entry)
...
def _auto_fix(errors: List[Dict], inst: Any) -> int:
    ...
    if needs_restart:
        pm2_name = "bublee" if inst.is_base else f"bublee-{inst.name}"
...
def cmd_logs(args):
    ...
    bublee logs [nombre]              — últimas 50 líneas + stream en vivo
    bublee logs [nombre] --lines 200  — más líneas históricas
    bublee logs [nombre] --errors     — solo líneas de error
    bublee logs [nombre] --fix        — analiza + aplica fixes automáticos
    bublee logs [nombre] --scan       — escaneo de errores sin stream
    """
    name        = getattr(args, 'name', '')        or ''
    ...
    if mode in ("scan", "fix", "errors"):
        ...
        _env_for_diag = dict(load_env(f"{inst.dir}/.env"))
        errors_found = _analyze_errors(recent, inst.name, env=_env_for_diag)
            ...
            if errors_found:
                ...
                if crit:
                    print(f"  {q(C.RED, '⚠')}  {q(C.RED, f'{len(crit)} error(es) CRÍTICO(s) detectado(s)')}  "
                          f"{q(C.G3, '→ bublee logs ' + inst.name + ' --fix')}")
                elif high:
                    print(f"  {q(C.YLW, '!')}  {q(C.YLW, f'{len(high)} advertencia(s) de nivel ALTO')}  "
                          f"{q(C.G3, '→ bublee logs ' + inst.name + ' --scan')}")
                nl()
                    ...
                    if re.search(r"ERROR|CRITICAL|Traceback", line, re.I):
                        ...
                        if error_counter == 3:
                            print(f"\n  {q(C.RED, '⚠')}  {q(C.YLW, '3 errores seguidos — ejecuta: ')} "
                                  f"{q(C.CYN, 'bublee logs ' + inst.name + ' --fix')}\n")
                            error_counter = 0
                    else:
...
def cmd_config(args):
    ...
    name = getattr(args, 'name', '') or ''
    try:
        import bublee_ultra_config
        bublee_ultra_config.run_ultra_config(name)
    except Exception as e:
...
def cmd_export(args):
    ...
    if fmt_idx == 0:  # JSON completo
        output = BACKUP_DIR / f"bublee_{inst.name}_{timestamp}.json"
...
def cmd_test(args):
    ...
    print_logo(compact=True)
    section(f"Test — {inst.label}", "Prueba respuestas de Bublee")
    ...
    while True:
        ...
        try:
            ...
            nl()
            print(f"  {q(C.P2, 'ᗧ')}  {q(C.W, 'Bublee:', bold=True)}")
...
def cmd_stop(args):
    ...
    if not name:
        fail("Especifica la instancia: bublee stop <nombre>")
        return
...
def cmd_delete(args):
    ...
    if not name:
        fail("Especifica la instancia: bublee delete <nombre>")
        return
...
def cmd_clone(args):
    ...
    with Spinner(f"Clonando '{source.label}' → '{dest_name}'...") as sp:
        ...
        missing_base_entries = [
            rel_path for rel_path in _runtime_sync_entries(BUBLEE_DIR)
            if rel_path not in clone_entries
        ]
        if missing_base_entries:
            _clone_runtime_entries(BUBLEE_DIR, dest_dir, missing_base_entries)
        ...
        src_env["WEBHOOK_SECRET"] = f"bublee_{dest_slug}_{_s.token_hex(6)}"
        src_env["DB_PATH"] = f"{dest_dir}/bublee.db"
        ...
        env_lines = [f"# Bublee — {dest_slug}  (clonado de {source.name})"]
...
def cmd_reset(args):
    ...
    with Spinner("Borrando sesión...") as sp:
        for db_file in [
            f"{inst.dir}/bublee.db",
            f"{inst.dir}/vectors.db",
            f"{inst.dir}/bublee.db-wal",
            f"{inst.dir}/bublee.db-shm"
        ]:
...
def cmd_zero(args):
    ...
    print(f"       {q(C.GRN, '✓')}  Archivos de código (bublee.py, etc.)")
    ...
    with Spinner(f"Creando backup de seguridad...") as sp:
        try:
            ...
            with _tar.open(backup_out, "w:gz") as tar:
                for f in [".env", "bublee.db", "instance.json", "vectors.db",
                          "bublee.db-wal", "bublee.db-shm"]:
                    p = f"{inst.dir}/{f}"
                    if os.path.exists(p):
                        tar.add(p, arcname=f)
    ...
    db_files = [
        f"{inst.dir}/bublee.db",
        f"{inst.dir}/bublee_ultra.db",
        f"{inst.dir}/vectors.db",
        f"{inst.dir}/bublee.db-wal",
        f"{inst.dir}/bublee.db-shm",
        f"{inst.dir}/bublee.db-journal",
    ]
    ...
    with Spinner("Aplicando nueva configuración...") as sp:
        ...
        nuevo_wh_secret  = f"bublee_{inst.name}_{_s.token_hex(8)}"
    ...
    with Spinner("Arrancando instancia limpia...") as sp:
        ...
        subprocess.run([
            ...
            "--restart-delay", "3000",
            "--max-restarts",  "10",
            "--log",           f"{inst.dir}/logs/bublee.log",
    ...
    if nuevo_token and nuevo_token != tg_actual:
        ...
        with Spinner("Registrando webhook en Telegram...") as sp:
            try:
                ...
                warn(f"Registra manualmente: bublee webhooks {inst.name}")
    elif nuevo_token:
    ...
    kv("Master Key",       mask_secret(nuevo_master))
    ...
    nl()
    info(f"Genera un token de activación con:  bublee chat {inst.name}")
    nl()
...
def cmd_backup(args):
    ...
    BACKUP_DIR.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = BACKUP_DIR / f"bublee_{inst.name}_{timestamp}.tar.gz"
    ...
    nl()
    info(f"Restaurar con: bublee restore {output.name}")
...
def cmd_restore(args):
    ...
    if port:
        ...
        update_env_key(env_path, "DB_PATH", f"{dest_dir}/bublee.db")
...
def cmd_scale(args):
    ...
    inst = find_instance(name) if name else None
    if not inst:
        fail("Especifica la instancia: bublee scale <nombre> <workers>")
        info("Ejemplo: bublee scale clinica-bella 4")
        return
...
def cmd_doctor(args):
    ...
    # Archivos
    info("Archivos core:")
    for f in ["bublee.py", "search.py", "knowledge_base.py", "nova_bridge.py"]:
        p = f"{BUBLEE_DIR}/{f}"
        if os.path.exists(p):
            ok(f"  {f}")
        else:
    ...
    if issues:
        ...
        if log_issues:
            ...
            for li in log_issues:
                ...
                dim(f"  bublee logs {inst_n} --fix")
            nl()
        info("Corrige los problemas y ejecuta 'bublee doctor' nuevamente")
    else:
...
def cmd_audit(args):
    ...
    gitignore_path = f"{BUBLEE_DIR}/.gitignore"
    ...
    if issues:
        ...
        for i in issues:
            dim(f"  · {i}")
        nl()
        info("Ejecuta 'bublee secure' para ver recomendaciones")
    else:
...
def cmd_secure(args):
    ...
    steps = [
        ...
        ("Backup antes de cambios",
         "bublee backup <instancia>",
         "Siempre tener un snapshot"),
        
        ("Usuario no-root",
         "sudo -u bublee pm2 start bublee.py",
...
def cmd_rotate_keys(args):
    ...
    for inst in instances:
        ...
        new_ws = f"bublee_{inst.name}_{_s.token_hex(6)}"
...
def cmd_upgrade(args):
    ...
    with Spinner("Validando sintaxis...") as sp:
        r = subprocess.run(
            [sys.executable, "-m", "py_compile", f"{BUBLEE_DIR}/bublee.py"],
            capture_output=True
        )
        if r.returncode == 0:
            ...
            subprocess.run(
                ["git", "checkout", "HEAD~1", "--", f"{BUBLEE_DIR}/bublee.py"],
                cwd=BUBLEE_DIR,
                capture_output=True
            )
            return
    ...
    if clientes and confirm(f"¿Propagar actualización a {len(clientes)} instancia(s) de clientes?"):
        ...
        info(f"Recuerda propagar a tus {len(clientes)} clientes con: bublee sync")

    nl()
    ok("Bublee actualizada")
...
def cmd_chat(args):
    """
    Chat interactivo con cualquier instancia de Bublee.
    Genera tokens de activación y permite hablar directamente via API.
    Detecta instancias tanto en /bublee como en /bublee/instances/.
    """
    print_logo(compact=True)
    name = getattr(args, 'name', '') or ''
    ...
    if not h:
        ...
        nl()
        info(f"Para levantarla: bublee restart {inst.name}")
        return
    ...
    options = [
        ...
        "Enviar mensaje de prueba a Bublee",
    ...
    # 2. Enviar mensaje de prueba
        ...
        while True:
            ...
            try:
                with Spinner("Bublee piensa...") as sp:
                    if _HTTPX:
                        ...
                        if code == 200:
                            ...
                            sp.stop(ok=False)
                            fail("404 — el endpoint /test no existe en bublee.py")
                            warn("Sube el bublee.py actualizado y reinicia:")
                            info(f"  bublee restart {inst.name}")
                            break
                        elif code == 401:
                            sp.stop(ok=False)
                ...
                nl()
                print(f"  {q(C.P3, 'Bublee:', bold=True)}")
                ...
                err = str(e)
                if "404" in err:
                    fail("404 — /test no existe en bublee.py")
                    warn("Sube bublee.py actualizado y reinicia la instancia")
                    break
...
def cmd_non_interactive(args):
    ...
    if not h:
        fail(f"'{inst.label}' no está online")
        info(f"Levántala con: bublee restart {inst.name}")
        return 1
...
def cmd_omni(args):
    """Bublee Omni — monitoreo central."""
    script = f"{BUBLEE_DIR}/bublee-omni.py"
    
    if not os.path.exists(script):
        fail("bublee-omni.py no encontrado")
        return
            ...
            if _HTTPX:
                ...
                if r.status_code == 200:
                    ...
                    if tid:
                        ...
                        pm2_name = "bublee" if inst_dir == BUBLEE_DIR else f"bublee-{name}"
                        pm2("restart", pm2_name)
...
def cmd_help(args):
    ...
    commands = [
        ...
        ("Mantenimiento", [
            ("sync", "Propagar bublee.py a todos los clientes"),
    ...
    for category, cmds in commands:
        ...
        for cmd_, desc in cmds:
            print(f"    {q(C.G2, f'bublee {cmd_:<18}')}  {q(C.G3, desc)}")
        nl()
    ...
    info("Usa lenguaje natural: bublee 'crear instancia para veterinaria'")
    nl()

def cmd_install(args):
    """
    Instala una copia completa de Bublee o el CLI global.

    Uso:
      bublee install              → Instala el CLI globalmente
      bublee install <nombre>     → Duplica todo Bublee en bublee-<nombre>/
    """
    ...
    if not name:
        ...
        target = "/usr/local/bin/bublee"
        target_alt = os.path.expanduser("~/.local/bin/bublee")
        ...
        try:
            ...
            nl()
            info("Ahora puedes ejecutar 'bublee' desde cualquier directorio.")
            info("Prueba con:  bublee list")
            nl()
            return
        ...
        try:
            ...
            if os.path.expanduser("~/.local/bin") not in path_dirs:
                ...
                info("Ya puedes ejecutar 'bublee' desde cualquier directorio.")
            nl()
            return
        except Exception as e:
        ...
        nl()
        print(f"  {q(C.G3, 'Opción 1 (requiere sudo):')}  {q(C.CYN, f'sudo ln -sf {this} /usr/local/bin/bublee')}")
        print(f"  {q(C.G3, 'Opción 2 (usuario):')}        {q(C.CYN, f'ln -sf {this} ~/.local/bin/bublee')}")
        nl()
        return
    ...
    home = Path(BUBLEE_DIR).resolve().parent
    dest = home / f"bublee-{slug}"

    if dest.exists():
        fail(f"Ya existe: {dest}")
        info("Usa otro nombre o borra la copia existente")
        return

    print_logo(compact=True)
    section(f"Instalar copia: bublee-{slug}", "Duplicando plataforma completa")

    info(f"Origen:  {BUBLEE_DIR}")
    info(f"Destino: {dest}")
    nl()

    with Spinner("Copiando archivos...") as sp:
        shutil.copytree(BUBLEE_DIR, dest, symlinks=True,
                        ignore=shutil.ignore_patterns('__pycache__', '*.pyc', '.git',
                                                       'node_modules', 'logs', '*.db',
                                                       '*.db-shm', '*.db-wal', '.env',
                                                       'soul', 'teachings', 'memory_store',
                                                       'knowledge_gaps', 'brand_assets', 'brand-assets'))
        sp.finish(f"{sum(f.stat().st_size for f in dest.rglob('*') if f.is_file()) // 1024} KB copiados")

    import uuid
    env_dest = dest / ".env"
    env_dest.write_text(f"# Bublee — {slug}\n"
                        f"INSTANCE_NAME={slug}\n"
                        f"PORT={next_port()}\n"
                        f"MASTER_API_KEY={uuid.uuid4().hex[:24]}\n"
                        f"WEBHOOK_SECRET={uuid.uuid4().hex[:16]}\n"
                        f"BUBLEE_DIR={dest}\n")
    ok(".env generado con tokens únicos")
    nl()

    ok(f"Bublee-{slug} creado en {dest}")
    info("Siguiente paso: configura la copia con:")
    info(f"  cd {dest} && bublee init")
    nl()
    info("Esto generará: agencia, tokens, bot de Telegram, LLM, etc.")
...
def cmd_demo(args):
    ...
    y cualquier persona que le escriba recibe a Bublee funcionando
    como agente de IA operativo — sin tokens, sin setup, sin esperas.
    ...
    Uso:
      bublee demo              → wizard: activar demo (muestra status primero)
      bublee demo status       → ver estado de todas las instancias
      bublee demo off          → desactivar demo mode
    """
    print_logo(compact=True)
    ...
    if sub == "on":
        sub = "" # On is the default action for wizard
        if name == "on":
            name = ""
    elif name == "on":
        name = ""
    ...
    if not sub:
        ...
        if not activate:
            nl()
            print(f"  Comandos: {q(C.CYN, 'bublee demo status')} · {q(C.CYN, 'bublee demo off')}")
            nl()
            return
        ...
        print(f"       Cualquier persona que le escriba recibe a Bublee.")
    ...
    if name:
        ...
        if not inst:
            ...
            info("Usa 'bublee list' para ver los nombres disponibles")
            return
        info(f"Usando instancia: {inst.label} (debug name={name}, len={len(instances)}, inst.name={inst.name})")
    ...
    steps = [
        ...
        "Muestra cómo Bublee responde en tiempo real",
        ...
        f"Para ver sesiones activas:  bublee demo status",
        f"Para apagar la demo:        bublee demo off",
    ]
    ...
    print(f"  {q(C.G3, 'Tip:')} {q(C.G2, 'Si alguien pregunta por el bot, Bublee les dará tu contacto')}")
    nl()
...
def cmd_rollback(args):
    """
    Rollback de bublee.py a una versión anterior.
    Crea backup de la versión actual y restaura la elegida.
    """
    import glob
    section("Rollback de Bublee", "Restaura una versión anterior de bublee.py")

    backups = sorted(
        glob.glob(f"{BUBLEE_DIR}/bublee.py.bak.*"),
        reverse=True
    )

    if not backups:
        warn("No hay backups disponibles.")
        info(f"Los backups se crean automáticamente en {BUBLEE_DIR}/bublee.py.bak.YYYYMMDD_HHMMSS")
        info("Sube una versión anterior manualmente o usa: cp bubleev6.py bublee.py")
        return
    ...
    current  = f"{BUBLEE_DIR}/bublee.py"
    ...
    if confirm("¿Sincronizar rollback a todas las instancias?"):
        ...
        for inst in clientes:
            with Spinner(f"Sincronizando {inst.label}...") as sp:
                try:
                    shutil.copy2(current, f"{inst.dir}/bublee.py")
                    sp.finish(f"{inst.label} — copiado")
                except Exception as e:
                    sp.finish(f"{inst.label} — ERROR: {e}", ok=False)

    # Reiniciar
    if confirm("¿Reiniciar bublee ahora?"):
        with Spinner("Reiniciando...") as sp:
            pm2("restart", "bublee")
            time.sleep(3)
            h = health(8001)
            sp.finish("Online ✓" if h else "Reiniciando...", ok=bool(h))
        if h:
            ok("Rollback completado y bublee online")
    nl()
...
def cmd_sync(args):
    ...
    TODAS las instancias en bublee/instances/.
    ...
    if not clientes:
        warn("No hay instancias de clientes en bublee/instances/")
        info("Crea una con: bublee new")
        return
    ...
    sync_entries  = _runtime_sync_entries(BUBLEE_DIR)
    auto_detected = _autodetect_sync_files(BUBLEE_DIR)

    # ── Mostrar archivos fijos ─────────────────────────────────────────────────
    info("Archivos fijos (siempre sincronizados):")
    for rel_path in [*SYNC_RUNTIME_FILES, *SYNC_RUNTIME_DIRS]:
        src = Path(BUBLEE_DIR) / rel_path
        if src.is_file():
    ...
    if dry_run:
        ...
        for inst in clientes:
            ...
            nl()
        info("Ejecuta 'bublee sync' para aplicar los cambios")
        nl()
        return
    ...
    _bak   = f"{BUBLEE_DIR}/bublee.py.bak.{_stamp}"
    try:
        shutil.copy2(f"{BUBLEE_DIR}/bublee.py", _bak)
        ok(f"Backup creado: bublee.py.bak.{_stamp}  (usa 'bublee rollback' para revertir)")
    ...
    for inst in clientes:
        ...
        with Spinner(f"Sincronizando {inst.label}...") as sp:
            copied = 0
            try:
                copied = len(_clone_runtime_entries(BUBLEE_DIR, inst.dir, sync_entries))
    ...
    if no_restart:
        ...
        info("  bublee restart all")
    if errors:
        warn(f"Fallaron: {', '.join(errors)}")
        info("Revisa sus logs: bublee logs <nombre>")
    nl()
...
def cmd_sync_check(args):
    ...
    Equivalente a 'bublee sync --dry-run'.
    """
...
def cmd_sync_manifest(args):
    ...
    Sub-comandos:
      bublee sync-manifest           — Ver estado actual
      bublee sync-manifest add <f>   — Añadir archivo al sync
      bublee sync-manifest remove <f>/ bublee sync-manifest rm <f>
                                      — Excluir archivo del sync
      bublee sync-manifest reset     — Borrar manifiesto (vuelve a defaults)
    """
    print_logo(compact=True)
    ...
    if sub in ("add", "agregar"):
        target = name or sub  # bublee sync-manifest add smart_handoff.py
        if not target or target in ("add", "agregar"):
            fail("Especifica el archivo: bublee sync-manifest add <archivo.py>")
            return
        ...
        info("Se incluirá en el próximo 'bublee sync'")
        return
    ...
    if sub in ("remove", "rm", "excluir", "quitar"):
        ...
        if not target or target in ("remove", "rm", "excluir", "quitar"):
            fail("Especifica el archivo: bublee sync-manifest rm <archivo.py>")
            return
        if target in include:
    ...
    # ── show (default) ────────────────────────────────────────────────────────
    root = Path(BUBLEE_DIR)
    auto = _autodetect_sync_files(BUBLEE_DIR)
    ...
    nl()
    info("Comandos:")
    dim("  bublee sync-manifest add <archivo>   — añadir al sync")
    dim("  bublee sync-manifest rm  <archivo>   — excluir del sync")
    dim("  bublee sync-manifest reset            — volver a defaults")
    dim("  bublee sync-check                     — preview sin ejecutar")
    nl()


def cmd_base(args):
    """
    Gestiona la instancia BASE de Bublee — /home/ubuntu/bublee.
    Es el directorio donde desarrollas; las instancias de clientes
    se sincronizan desde aquí con 'bublee sync'.

    Sub-comandos:
      bublee base              — Estado actual (PM2 + health)
      bublee base start        — Registrar en PM2 y arrancar
      bublee base restart      — Reiniciar
      bublee base stop         — Detener
      bublee base logs         — Logs en vivo
      bublee base register     — Re-registrar en PM2 (si ya existía)
    """
    print_logo(compact=True)
    ...
    if not inst:
        ...
        info(f"Directorio base buscado: {BUBLEE_DIR}")
        return
    ...
    if sub in ("", "status", "estado"):
        ...
        if proc:
            ...
            info("Registra con: bublee base start")
        ...
        dim("  2. bublee base restart   ← prueba en la base")
        dim("  3. bublee sync           ← propaga a clientes")
        nl()
        return
    ...
    if sub in ("start", "register", "iniciar", "registrar"):
        ...
        dim("  bublee base restart    — reiniciar tras un cambio")
        dim("  bublee base logs       — ver logs en vivo")
        dim("  bublee sync            — propagar a clientes")
        nl()
        return
    ...
    if sub in ("restart", "reiniciar", "r"):
        ...
        dim("  bublee sync   — propaga los cambios a todas las instancias")
        nl()
        return
...
def cmd_fix(args):
    ...
    info("Si alguna aún no responde en Telegram: bublee logs <nombre>")
    nl()
...
def cmd_estilo(args):
    ...
    info("Bublee leerá los últimos 50 mensajes del admin y aprenderá su forma de escribir")
...
def _admin_cmd(inst, master_key, cmd_text, user_id=None):
    """Enviar un comando al endpoint /test de Bublee como si fuera el admin."""
    base_url = f"http://localhost:{inst.port}"
    ...
    if not user_id:
        try:
            ...
            conn = _sq.connect(f"{inst.dir}/bublee.db")
        ...
        if missing:
            ...
            if confirm("¿Agregar las rutas faltantes ahora?", default=True):
                for m in missing:
                    ...
                    wpath = f"webhook/bublee_{m.name}_{ws}" if ws else f"webhook/bublee_{m.name}"
...
def cmd_token(args):
    ...
    Uso:
      bublee token                    → genera para la instancia base
      bublee token --admin            → genera token Bublee Pro Admin
      bublee token 1                  → genera para la instancia 1
      bublee token "Clinica Demo"     → genera con ese nombre
      bublee token all                → genera para todas las instancias
    """
    ...
    def _gen_token(inst, label_override=""):
        ...
        def _print_token(token, expires, offline=False):
            kind = "Bublee Pro Admin" if admin_mode else "activación"
...
def cmd_activar(args):
    ...
    Uso:
      bublee activar                  → activa la instancia base
      bublee activar 1                → activa instancia 1
    """
    name = getattr(args, 'name', '') or getattr(args, 'subcommand', '') or ''
    inst = find_instance(name) if name else None
    if not inst:
    ...
    if not os.path.exists(db_path):
        ...
        info("¿Está corriendo? bublee restart " + inst.name)
        return
    ...
    try:
        ...
        nl()
        info("Reiniciando Bublee para aplicar...")
        pm2("restart", inst.pm2_name)
        time.sleep(2)
        h = health(inst.port)
        if h.get("status") == "online":
            ok(f"Bublee online en :{inst.port}")
        else:
...
def cmd_bridge(args):
    ...
    Uso:
      bublee bridge                   → estado del bridge
      bublee bridge restart           → reiniciar bridge
      bublee bridge qr                → mostrar QR para conectar
      bublee bridge fix               → fijar a qué instancia apunta el bridge
      bublee bridge fix 1             → fijar al bridge para la instancia 1
    """
    sub  = getattr(args, 'subcommand', '') or ''
    name = getattr(args, 'name', '')       or ''
    ...
    if target_inst:
        ...
        info("  Usa: bublee bridge fix  para seleccionar la instancia correcta")
    ...
    if sub in ("restart", "reiniciar"):
        ...
        if not instances:
            fail("No hay instancias Bublee"); return
        ...
        if demo:
            ok("DEMO_MODE=true — Bublee responderá a TODOS sin pedir token")
        else:
            if not inst.is_active:
                warn("La instancia no está activada — usa: bublee activar")
            else:
        ...
        info("Escríbele a Bublee por WhatsApp para probar")
        ...
        try:
            ...
            if result.get("ok"):
                ...
                info("Espera ~20s y verifica con: bublee bridge")
            else:
...
def cmd_tokens(args):
    ...
    Uso:
      bublee tokens                   → listar tokens de la instancia base
      bublee tokens 1                 → listar tokens de instancia 1
    """
    name = getattr(args, 'name', '') or getattr(args, 'subcommand', '') or ''
    inst = find_instance(name) if name else None
    if not inst:
    ...
    try:
        ...
        if not tokens:
            ...
            info("Crea uno con: bublee token")
            return
...
def cmd_whatsapp_cli(args):
    ...
    Uso:
      bublee whatsapp             → estado WhatsApp de todas las instancias
      bublee whatsapp 1           → estado de instancia 1
      bublee whatsapp connect 1   → conectar WhatsApp a instancia 1
      bublee whatsapp disconnect  → desconectar
      bublee whatsapp qr          → mostrar QR de reconexión
    """
    sub  = getattr(args, 'subcommand', '') or ''
    name = getattr(args, 'name', '')       or ''
    ...
    try:
        ...
        if wa_status != "open":
            warn("WhatsApp no conectado — usa: bublee whatsapp qr")
    except Exception:
        wa_status = "offline"
    ...
    nl()
    if not sub:
        info("Subcomandos: bublee whatsapp qr | connect | disconnect | status")
        return
    ...
    if sub == "qr":
        ...
        info("Cuando aparezca el QR corre: bublee whatsapp qr")
...
def _fix_bridge_webhook(inst: Instance):
    ...
    BRIDGE_ENV = "/home/ubuntu/whatsapp-bridge/.env"
    secret = inst.webhook_secret or "bublee_2026"
...
def cmd_info(args):
    ...
    Uso:
      bublee info                 → info de la instancia base
      bublee info 1               → info de instancia 1
    """
    name = getattr(args, 'name', '') or getattr(args, 'subcommand', '') or ''
    inst = find_instance(name) if name else None
    if not inst:
    ...
    if h and inst.master_key:
        try:
            ...
            if tokens:
                ...
                warn("Sin tokens activos — crea uno con: bublee token")
        except Exception:
            pass
    nl()
    ...
    # Acciones rápidas
    info("Acciones:")
    print(f"    bublee token {inst.name}       — generar token de activacion")
    print(f"    bublee activar {inst.name}     — activar instancia")
    print(f"    bublee bridge fix              — arreglar bridge WhatsApp")
    print(f"    bublee restart {inst.name}     — reiniciar")
    print(f"    bublee logs {inst.name}        — ver logs")
    nl()
...
def cmd_pair(args):
    ...
    Usos:
      bublee pair list
      bublee pair default <instancia>
      bublee pair detach <chat_id>
      bublee pair <instancia> <chat_id>
    """
    ...
    if sub == "default":
        if not name:
            fail("Uso: bublee pair default <instancia>")
            return
        inst = find_instance(name)
        if not inst:
    ...
    if sub in ("detach", "rm", "remove"):
        if not name:
            fail("Uso: bublee pair detach <chat_id>")
            return
    ...
    if not inst or not name:
        fail("Uso: bublee pair <instancia> <chat_id>")
        return
...
def cmd_bb_config(args):
    """Configura el agente Bublee de una instancia: nombre, prompt, personalidad y skills."""
    return _cmd_bb_config_impl(_bb_context(), args)


def cmd_mcp(args):
    """Inicia el servidor Model Context Protocol de Bublee."""
    info("Iniciando servidor MCP de Bublee V3 en stdio...")
    try:
...
def cmd_auth(args):
    ...
    if not token:
        token = prompt("Ingresa tu token de acceso (generado con bublee token)")
    
    if not token:
        fail("No se proporcionó ningún token.")
        return

    auth_file = Path(BUBLEE_HOME) / "auth.json"
    auth_data = {}
    if auth_file.exists():
        try:
...
def cmd_portal(args):
    ...
    if not auth_file.exists():
        warn("No estás autenticado. Ejecuta 'bublee auth' primero.")
    
    info("Abriendo el portal web...")
    try:
        __import__('webbrowser').open("https://dashboard.bublee.ai/")
        ok("Portal abierto en tu navegador.")
    except:
        info("Visita: https://dashboard.bublee.ai/")
...
def cmd_sync_web(args):
    ...
    auth_file = Path(BUBLEE_HOME) / "auth.json"
    if not auth_file.exists():
        fail("No estás autenticado. Necesitas un token web (bublee auth).")
        return
        
    inst = _pick_instance(args, "¿Qué instancia deseas sincronizar con la web?")
    if not inst: return
    
    with Spinner("Sincronizando estado con Bublee Web Services...") as sp:
...
def cmd_dev_account(args):
    ...
    if not os.path.exists(db_path):
        db_path = "/home/ubuntu/bublee/bublee.db"
    ...
    try:
        ...
        info("Ya puedes entrar en la web mediante el botón 'API Access / Bublee Dev'.")
    except Exception as e:
        fail(f"Error al crear cuenta localmente: {e}")


def cmd_x(args):
    """Duplicar la arquitectura modular para un nuevo desarrollador (OVNI)."""
    print_logo(compact=True)
    section("Arquitectura Modular OVNI", "Despliegue de entorno para nuevo desarrollador")
    
    dev_name = prompt("Nombre del desarrollador (ej. dev2, analista)")
    if not dev_name:
        return
        
    dev_slug = slug(dev_name)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    dest_dir = f"/home/ubuntu/bublee-{dev_slug}"
    
    if os.path.exists(dest_dir):
        fail(f"El directorio {dest_dir} ya existe.")
        return
        
    base_port = next_port()
    
    with Spinner(f"Desplegando ecosistema OVNI en {dest_dir}...") as sp:
        # Copiar carpeta entera excluyendo pesados
        cmd = f"rsync -a --exclude '.git' --exclude '.venv' --exclude '__pycache__' --exclude 'node_modules' --exclude 'bublee_ultra.db' --exclude 'bublee/instances' {base_dir}/ {dest_dir}/"
        os.system(cmd)
        ...
        if os.path.exists(env_path):
            ...
            env_content = re.sub(r"PORT=\d+", f"PORT={base_port}", env_content)
            with open(env_path, 'w') as envf:
                envf.write(env_content)
                
        # Modificar ecosystem.config.js para que use nombres únicos
        eco_path = f"{dest_dir}/ecosystem.config.js"
        if os.path.exists(eco_path):
            with open(eco_path, 'r') as ecof:
                eco_content = ecof.read()
            eco_content = eco_content.replace('name: "bublee"', f'name: "bublee-{dev_slug}"')
            eco_content = eco_content.replace('/home/ubuntu/bublee', dest_dir)
            with open(eco_path, 'w') as ecof:
                ecof.write(eco_content)
                
        sp.finish(f"Entorno modular '{dev_name}' desplegado.")
        
    nl()
    ok(f"El nuevo entorno está listo en: {dest_dir}")
    info(f"Puerto asignado: {base_port}")
    info(f"Para iniciar el entorno, ejecuta:")
    info(f"  cd {dest_dir}")
    info(f"  python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt")
    info(f"  pm2 start ecosystem.config.js")
...
def cmd_interactive(args):
    """Modo interactivo — shell de Bublee."""
    print_logo(compact=True)
    section("Modo Interactivo", "Escribe comandos sin 'bublee'. 'exit' para salir.")
    ...
    while True:
        try:
            sys.stdout.write(f"\n  {q(C.P2, 'ᗧ', bold=True)} {q(C.G2, 'bublee')} {q(C.P3, '›')} ")
            sys.stdout.flush()
            line = input("").strip()
        except (EOFError, KeyboardInterrupt):
            nl()
...
# ALERTAS Y MONITOREO
# ══════════════════════════════════════════════════════════════════════════════
ALERTS_FILE = Path.home() / ".bublee" / "alerts.json"
...
def cmd_alerts(args):
    ...
    if sub == "add":
        ...
        if not alerts:
            ...
            info("Crea una con: bublee alerts add")
            return
...
def cmd_webhooks(args):
    ...
    if platform == "telegram":
        ...
        webhook_secret = ev.get("WEBHOOK_SECRET", "bublee_2026")
        ...
        kv("Verify Token", ev.get("WA_VERIFY_TOKEN", "bublee_verify"))
...
def cmd_search(args):
    ...
    if choice in [0, 2]:
        ...
        instances = get_instances()
        for inst in instances:
            log_path = f"{inst.dir}/logs/bublee.log"
...
def cmd_batch(args):
    ...
    if operation == "restart":
        ...
        for inst in selected:
            ...
            with tarfile.open(output, "w:gz") as tar:
                for f in [".env", "bublee.db", "instance.json"]:
                    p = f"{inst.dir}/{f}"
                    if os.path.exists(p):
                        tar.add(p, arcname=f)
...
# QUICK ACTIONS
# ══════════════════════════════════════════════════════════════════════════════
QUICK_ACTIONS_FILE = Path.home() / ".bublee" / "quick_actions.json"
...
def cmd_quick(args):
    ...
    if sub == "add":
        ...
        if not actions:
            ...
            info("Crea una con: bublee quick add")
            return
        ...
        info("Ejecutar: bublee quick run <nombre>")
        info("Crear: bublee quick add")
...
def cmd_debug(args):
    ...
    kv("CWD", os.getcwd())
    kv("BUBLEE_DIR", BUBLEE_DIR)
    kv("INSTANCES_DIR", INSTANCES_DIR)
    nl()
    
    # Variables de entorno
    info("Variables de entorno relevantes:")
    env_vars = ["BUBLEE_DIR", "INSTANCES_DIR", "NOVA_DIR", "TERM", "SHELL", "USER", "HOME"]
    for var in env_vars:
...
# ══════════════════════════════════════════════════════════════════════════════
PLUGINS_DIR = Path.home() / ".bublee" / "plugins"
...
def cmd_plugins(args):
    ...
    if sub == "list":
        ...
        if not plugins:
            ...
            info("Los plugins van en: ~/.bublee/plugins/")
            return
...
def cmd_cron(args):
    ...
    # Mostrar tareas bublee existentes
    try:
        result = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        current = result.stdout if result.returncode == 0 else ""
        
        bublee_tasks = [l for l in current.split('\n') if 'bublee' in l.lower()]
        
        if bublee_tasks:
            info("Tareas de Bublee en cron:")
            for t in bublee_tasks:
                print(f"    {q(C.G1, t)}")
        else:
            info("No hay tareas de Bublee en cron")
    except Exception:
    ...
    templates = [
        ("Backup diario (3am)", "0 3 * * * /usr/local/bin/bublee backup base"),
        ("Health check (cada 5min)", "*/5 * * * * /usr/local/bin/bublee health --quiet"),
        ("Rotate keys (mensual)", "0 0 1 * * /usr/local/bin/bublee rotate-keys --auto"),
        ("Limpieza logs (semanal)", "0 4 * * 0 /usr/local/bin/bublee cleanup logs"),
    ]
...
def cmd_guide(args):
    """
    Guía completa paso a paso para operar Bublee desde la terminal.
    ...
    print(f"          {q(C.CYN, 'bublee zero <nombre>')}  ← borra datos, permite renombrar")

    hr()

    # ── SECCIÓN 2: Ver estado de las instancias ────────────────────────────────
    print(f"\n  {q(C.P1, '2.', bold=True)}  {q(C.W, 'VER ESTADO', bold=True)}")
    print(f"\n  {q(C.G3, 'Ver todas las instancias:')}     {q(C.CYN, 'bublee list')}")
    print(f"  {q(C.G3, 'Dashboard en tiempo real:')}     {q(C.CYN, 'bublee dashboard')}")
    print(f"  {q(C.G3, 'Health check rápido:')}          {q(C.CYN, 'bublee health')}")
    print(f"  {q(C.G3, 'Estado detallado de uno:')}      {q(C.CYN, 'bublee status <nombre>')}")
    print(f"\n  Ejemplo: {q(C.CYN, 'bublee status clinica-perez')}")

    hr()

    # ── SECCIÓN 3: Levantar / Reiniciar ───────────────────────────────────────
    print(f"\n  {q(C.P1, '3.', bold=True)}  {q(C.W, 'LEVANTAR Y REINICIAR', bold=True)}")
    print(f"\n  {q(C.G3, 'Reiniciar una instancia:')}      {q(C.CYN, 'bublee restart <nombre>')}")
    print(f"  {q(C.G3, 'Reiniciar TODAS:')}               {q(C.CYN, 'bublee restart all')}")
    print(f"  {q(C.G3, 'Detener una:')}                   {q(C.CYN, 'bublee stop <nombre>')}")
    print(f"\n  {q(C.G3, 'Si una instancia no responde, prueba en este orden:')}")
    print(f"    {q(C.YLW, '1)')} {q(C.CYN, 'bublee health')}  → ¿está online?")
    print(f"    {q(C.YLW, '2)')} {q(C.CYN, 'bublee logs <nombre>')}  → ¿qué error muestra?")
    print(f"    {q(C.YLW, '3)')} {q(C.CYN, 'bublee restart <nombre>')}  → reiniciarla")
    print(f"    {q(C.YLW, '4)')} {q(C.CYN, 'bublee webhooks <nombre>')}  → verificar webhook en Telegram")

    hr()

    # ── SECCIÓN 4: Logs ───────────────────────────────────────────────────────
    print(f"\n  {q(C.P1, '4.', bold=True)}  {q(C.W, 'VER LOGS (para detectar errores)', bold=True)}")
    print(f"\n  {q(C.G3, 'Logs en tiempo real:')}          {q(C.CYN, 'bublee logs <nombre>')}")
    print(f"  {q(C.G3, 'Logs directos de PM2:')}          {q(C.CYN, 'pm2 logs bublee-<nombre>')}")
    ...
    print(f"\n  {q(C.G3, 'Verificar y reconfigurar:')}    {q(C.CYN, 'bublee webhooks <nombre>')}")
    ...
    print(f"\n  {q(C.G3, 'Encontrar el SECRET:')}  {q(C.CYN, 'bublee config <nombre>')}")
    ...
    print(f"\n  {q(C.G3, 'Asistente de config:')}          {q(C.CYN, 'bublee config <nombre>')}")
    print(f"  {q(C.G3, 'Editar .env directamente:')}      {q(C.CYN, 'nano /home/ubuntu/bublee/instances/<nombre>/.env')}")
    print(f"\n  {q(C.YLW, 'Después de editar el .env siempre reinicia:')}  {q(C.CYN, 'bublee restart <nombre>')}")

    hr()

    # ── SECCIÓN 7: Diagnóstico ────────────────────────────────────────────────
    print(f"\n  {q(C.P1, '7.', bold=True)}  {q(C.W, 'DIAGNÓSTICO COMPLETO DEL SISTEMA', bold=True)}")
    print(f"\n  {q(C.CYN, 'bublee doctor')}   → revisa dependencias, puertos, configuración")
    print(f"  {q(C.CYN, 'bublee debug')}    → info técnica detallada de todas las instancias")
    print(f"\n  {q(C.G3, 'Comandos PM2 directos (cuando bublee CLI no basta):')}")
    print(f"    {q(C.CYN, 'pm2 list')}              → ver todos los procesos")
    print(f"    {q(C.CYN, 'pm2 restart all')}       → reiniciar todos")
    print(f"    {q(C.CYN, 'pm2 delete bublee-X')}  → borrar proceso específico")
    ...
    problems = [
        (
            ...
            [
                ...
                f"Ejecuta: {q(C.CYN, 'bublee webhooks <nombre>')} → opción 1 para ver la URL",
        ...
        (
            ...
            [
                "El proceso arrancó pero bublee.py falló internamente",
                f"Ejecuta: {q(C.CYN, 'bublee logs <nombre>')} y busca el error",
                "Causas comunes: falta una API key, .env incorrecto, puerto ocupado",
                f"Solución rápida: {q(C.CYN, 'bublee restart <nombre>')}",
            ]
        ),
        (
            ...
            [
                ...
                f"Para liberar un puerto muerto: {q(C.CYN, 'pm2 delete bublee-<nombre>')}",
            ]
        ),
        (
            ...
            [
                ...
                f"Solución: {q(C.CYN, 'pm2 delete bublee-<nombre>')} → luego {q(C.CYN, 'bublee restart <nombre>')}",
    ...
    quick_ref = [
        ("Instalar CLI globalmente",     "bublee install"),
        ("Nueva instancia",              "bublee new"),
        ("Ver todas",                    "bublee list"),
        ("Dashboard live",               "bublee dashboard"),
        ("Health check",                 "bublee health"),
        ("Status de una",                "bublee status <nombre>"),
        ("Generar token + probar bot",   "bublee chat <nombre>"),
        ("Logs en vivo",                 "bublee logs <nombre>"),
        ("Reiniciar una",                "bublee restart <nombre>"),
        ("Reiniciar todas",              "bublee restart all"),
        ("Reparar bug de --cwd",         "bublee fix <nombre>"),
        ("Clonar runtime compartido a todos",  "bublee sync"),
        ("♻  Entregar a nuevo cliente",  "bublee zero <nombre>"),
        ("Parar una",                    "bublee stop <nombre>"),
        ("Editar configuración",         "bublee config <nombre>"),
        ("Ver/fijar webhook",            "bublee webhooks <nombre>"),
        ("Diagnóstico completo",         "bublee doctor"),
        ("Actualizar + propagar",        "bublee upgrade"),
        ("Esta guía",                    "bublee guide"),
        ("",                             ""),
        ("Pipeline de leads",            "bublee pipeline [instancia]"),
        ("Por qué se van clientes",      "bublee perdidos [instancia]"),
        ("Coach de ventas",              "bublee coach [instancia]"),
        ("Clonar estilo del dueño",      "bublee estilo [instancia]"),
        ("Reactivar inactivos",          "bublee reactivar [instancia]"),
        ("Reporte ahora",               "bublee reporte [instancia]"),
        ("Mensaje masivo",               "bublee broadcast [instancia]"),
        ("Instagram DMs",               "bublee instagram [instancia]"),
        ("Pagos desde chat",            "bublee pagos [instancia]"),
    ]
    ...
    nl()
    info("Tip: escribe 'bublee i' para modo interactivo con autocompletado")
    nl()
...
def cmd_help_extended(args):
    ...
    command_groups = [
        ...
        ("V8.0 — Calidad & Modelo", [
            ...
            ("rollforward [n]",   "Aplicar bublee.py a todas las instancias"),
    ...
    for category, cmds in command_groups:
        print(f"  {q(C.P2, category, bold=True)}")
        for cmd_, desc in cmds:
            print(f"    {q(C.G2, f'bublee {cmd_:<20}')} {q(C.G3, desc)}")
        nl()
    ...
    shortcuts = [
        ("bublee i", "Modo interactivo"),
        ("bublee l", "Listar instancias"),
        ("bublee s", "Status"),
        ("bublee r all", "Reiniciar todo"),
    ]
    for short, desc in shortcuts:
        print(f"    {q(C.CYN, short)}  →  {q(C.G3, desc)}")
    nl()
    
    info("Lenguaje natural: bublee 'crear veterinaria para clínica peludo'")
    nl()
...
def cmd_modelo(args):
    ...
    # Subcomando directo (bublee modelo [n] gemini-flash)
...
def _apply_model_change(inst: "Instance", alias: str):
    ...
    if alias == "reset":
        with Spinner(f"Restaurando modelo original...") as sp:
            ...
            if r.get("ok") or "reset" in str(r).lower():
                ...
                info("¿La instancia está corriendo bublee.py?")
        return
    ...
    nl()
    ok(f"Para verificar: bublee v8 {inst.name}")
...
SIMULATION_SCENARIOS = {
    ...
    "conversacion_larga":"Conversación larga — ¿Bublee mantiene coherencia?",
}
...
def cmd_simular(args):
    ...
    if not h:
        ...
        info(f"Arráncala con: bublee restart {inst.name}")
        return
    ...
    if ping.get("error") or not ping.get("v8"):
        ...
        info(f"Verifica: grep -c 'v8/ping' /home/ubuntu/bublee/bublee.py")
        info(f"Si es 0: sube el bublee.py con V8 y reinicia")
        return
    ...
    if scenario_id == "all":
        ...
        if r.get("error"):
            ...
            info(f"Verifica: bublee v8 {inst.name}")
            info(f"Si V8 no está activo: bublee restart {inst.name}")
            return
    ...
    nl()
    info(f"Para ver el pipeline de leads: bublee pipeline {inst.name}")
...
def _print_simulation_report(report: dict):
    ...
    if failed == 0 and warned == 0:
        ...
        info("Bublee responde de forma humana y sin frases de bot")
...
def _print_scenario_result(r: dict):
    ...
    if turns:
        ...
        for i, t in enumerate(turns):
            ...
            print(f"         {q(C.G3, 'Bublee:')} {q(color, t.get('bublee', '')[:60])}")
            if t.get("failures"):
...
def cmd_v8(args):
    ...
    with Spinner("Verificando endpoints V8...") as sp:
        ...
        sp.finish(
            ...
            else "Endpoints V8 no encontrados — ¿el bublee.py tiene V8?",
            ok=ping_ok
        )
    ...
    if not ping_ok:
        ...
        dim(f"   grep -c 'v8/ping' /home/ubuntu/bublee/bublee.py")
        dim(f"   (debe retornar 1 o más)")
        dim(f"2. Si el resultado es 0: el bublee.py no tiene V8 — sube el nuevo archivo")
        dim(f"3. Si tiene V8: revisa los logs: bublee logs {inst.name}")
        dim(f"4. Reinicia: bublee restart {inst.name}")
        nl()
    ...
    if v8_status.get("error"):
        ...
        dim("1. El proceso bublee.py no cargó los sistemas V8 — revisa los logs")
        dim("   bublee logs " + inst.name)
        dim("2. La versión de bublee.py no tiene V8 — verifica con 'cat bublee.py | head -20'")
        dim("3. Reinicia la instancia: bublee restart " + inst.name)
        nl()
        info("Estado del servidor básico:")
        kv("Status",          h.get("status", "?"))
        kv("Versión bublee",  h.get("version", "no reportada"))
        kv("Clinic",          h.get("clinic", "?"))
    ...
    if v8_init_status == "v8_not_initialized":
        ...
        for note in v8_status.get("init_notes", []):
            dim(f"  • {note}")
        nl()
        warn("Solución: reinicia la instancia → bublee restart " + inst.name)
        nl()
    ...
    nl()
    info(f"Simular conversaciones: bublee simular {inst.name}")
    info(f"Correr tests automáticos: bublee test {inst.name}")
    info(f"Cambiar modelo: bublee modelo {inst.name}")
...
def cmd_quality(args):
    """
    Analiza la humanidad de las últimas respuestas de Bublee.
    ...
    try:
        ...
        # bublee usa "ts" como columna de timestamp en conversations
    ...
    if bot_hits:
        ...
        if avg_score < 0.75:
            ...
            info("Sube el nivel: bublee filtro 3 " + inst.name)
        else:
    ...
    nl()
    info(f"Para mejorar: bublee modelo {inst.name}  (cambiar a modelo más natural)")
    info(f"Para simular: bublee simular {inst.name}  (probar escenarios)")
...
def _generate_local_briefing(inst: "Instance", hours: int = 24):
    ...
    try:
        ...
        # bublee usa 'ts' como columna de timestamp
        try:
        ...
        if new_convs > 0 and new_apts == 0:
            ...
            info(f"Revisa: bublee quality {inst.name}")
        elif stuck > 2:
            warn(f"Hay {stuck} conversaciones sin respuesta por más de 12h")
            info(f"Revisa: bublee status {inst.name}")
        elif new_apts > 0:
...
def cmd_cost(args):
    ...
    if cost_per_month > 50:
        ...
        info(f"Para cambiar: bublee modelo {inst.name}")
    else:
        ...
        if confirm("¿Medir latencia del LLM (más lento, ~5-20s)?"):
            ...
            if llm_latencies:
                ...
                if avg_llm > 10000:
                    ...
                    info(f"Gemini Flash o Llama-70B son más rápidos: bublee modelo {inst.name}")
...
def cmd_watchdog(args):
    ...
    if sub == "stop":
        ...
        pid_file = Path.home() / ".bublee" / "watchdog.pid"
        if pid_file.exists():
            try:
    ...
    if sub == "status":
        ...
        if pid_file.exists():
            ...
            info("Arranca con: bublee watchdog start")
        return
    ...
    if background:
        ...
        try:
            ...
            if pid > 0:
                ...
                info(f"Ver estado: bublee watchdog status")
                info(f"Detener: bublee watchdog stop")
                return
...
def cmd_warmup(args):
    ...
    for i, user_msg in enumerate(warmup_prompts):
        with Spinner(f"Warmup {i+1}/{len(warmup_prompts)}: '{user_msg}'") as sp:
            start = time.time()
            # Usar el endpoint de test que existe en bublee
            r = inst.api_call(
...
def cmd_campana(args):
    ...
    if sub == "list" or not sub:
        ...
        if r.get("error"):
            ...
            info("¿Los sistemas V8 están activos? bublee v8 " + inst.name)
            return
        ...
        info(f"  bublee campana activate {inst.name}")
        ...
        if r.get("ok") or "reactivar" in str(r).lower():
            ok("Campaña de reactivación enviada a Bublee")
            info("Bublee procesará los follow-ups de forma escalonada")
        else:
            # Fallback directo a API de Bublee
...
def cmd_rollforward(args):
    """
    Aplica bublee.py a todas las instancias (o a una específica).
    Actualiza el archivo sin perder configuración (.env y DB se conservan).
    """
    print_logo(compact=True)
    section("Roll Forward — Aplicar V8 a instancias",
            "Actualiza bublee.py → bublee.py en todas las instancias")

    # Verificar que existe bublee.py en el directorio base
    # Buscar el bublee.py actualizado — puede estar en varios lugares
    candidates = [
        f"{BUBLEE_DIR}/bublee.py",
        "./bublee.py",
        os.path.expanduser("~/bublee/bublee.py"),
        os.path.expanduser("~/bublee.py"),
    ]
    v8_source = next((p for p in candidates if os.path.exists(p)), None)
    if not v8_source:
        fail("No se encontró bublee.py")
        info(f"Directorio base buscado: {BUBLEE_DIR}/bublee.py")
        info("Asegúrate de que el bublee.py con V8 esté en el directorio base")
        return
    ...
    for inst in targets:
        with Spinner(f"Actualizando {inst.label}...") as sp:
            try:
                # Backup del bublee.py actual
                current_bublee = f"{inst.dir}/bublee.py"
                if os.path.exists(current_bublee):
                    backup_path = f"{inst.dir}/bublee_backup_{int(time.time())}.py"
                    shutil.copy(current_bublee, backup_path)

                # Copiar v8
                shutil.copy(v8_source, current_bublee)
    ...
    if success_count > 0:
        info("Verifica con: bublee v8 <instancia>")
        info("Prueba con: bublee simular <instancia>")
...
def cmd_dashboard_v8(args):
    ...
    def draw():
        ...
        print(f"  {q(C.P1, '✦', bold=True)}  "
              f"{q(C.W, 'Bublee V8 Dashboard', bold=True)}  "
              f"{q(C.G3, now)}  "
        ...
        hr()
        print(f"  {q(C.G3, 'Actualiza cada 3s  ·  modelo: bublee modelo <n>  ·  v8: bublee v8 <n>')}")
...
ROUTES["log"]          = cmd_logs
ROUTES["logfix"]       = cmd_logs   # bublee logfix [n] → directo al --fix
ROUTES["logscan"]      = cmd_logs   # bublee logscan [n] → directo al --scan
...
_v8_section = (
    "V8.0 — Calidad & Modelo", [
        ...
        ("campana [n]",      "Campañas de seguimiento y reactivación"),
        ("env-check [n]",    "Verificar .env contra mejores prácticas V8"),
        ("rollforward [n]",  "Aplicar bublee.py a instancias"),
        ("dashboard-v8",     "Dashboard enriquecido con métricas V8"),
...
def cmd_trainer_skills(args):
    """Ver y gestionar skills de comportamiento de Bublee."""
    print_logo(compact=True)
    ...
    if sub in ("on", "off", "activar", "desactivar"):
        # bublee skills [on|off] [skill_id]
        skill_id = getattr(args, "name", "") or ""
        if not skill_id:
            fail("Formato: bublee skill [skill_id] on/off")
            return
    ...
    if r.get("error"):
        ...
        info("¿El trainer está activo? Verifica: bublee v8 " + inst.name)
        return
    ...
    nl()
    info("Para activar:  bublee skill <instancia> on  <skill_id>")
    info("Para desactivar: bublee skill <instancia> off <skill_id>")
...
def cmd_trainer_gateway(args):
    ...
    section(f"Gateway automático — {inst.label}",
            "Bublee decide skills y preferencias sin obligarte a configurarlas a mano")
    ...
    nl()
    info("Comandos:")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name:<24}')} {q(C.G2, 'Ver estado')}")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name} on')} {q(C.G2, 'Activar todo el gateway')}")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name} off')} {q(C.G2, 'Desactivar todo el gateway')}")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name} admin-on')} {q(C.G2, 'Activar solo admin')}")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name} admin-off')} {q(C.G2, 'Desactivar solo admin')}")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name} user-on')} {q(C.G2, 'Activar solo usuario')}")
    print(f"    {q(C.CYN, f'bublee gateway {inst.name} user-off')} {q(C.G2, 'Desactivar solo usuario')}")
...
def cmd_trainer_control(args):
    ...
    info("Ejemplos:")
    example_commands = [
        (f'bublee control {inst.name} "no digas ay"', "Prohibir una frase"),
        (f'bublee control {inst.name} "no digas ay, di entiendo"', "Forzar reemplazo"),
        (f'bublee control {inst.name} "no uses claro al inicio"', "Bloquear arranque"),
        (f'bublee control {inst.name} "saluda así: Hola, Bublee por acá"', "Fijar saludo base"),
        (f'bublee control {inst.name} "segunda burbuja: Estoy lista para ayudarle con la instancia"', "Fijar segunda burbuja"),
        (f'bublee control {inst.name} "tercera burbuja: Si quiere, lo dejamos listo ahora"', "Fijar tercera burbuja"),
        (f'bublee control {inst.name} "cierra así: Si quiere, lo dejamos listo ahora"', "Fijar cierre"),
        (f'bublee control {inst.name} "si no entiendes di: Perdón, ayúdeme con un poco más de contexto"', "Fijar fallback"),
        (f'bublee control {inst.name} "responde en 2 burbujas"', "Limitar burbujas"),
        (f'bublee control {inst.name} "conmigo háblame de usted y más directo"', "Ajustar trato admin"),
    ]
    for command, description in example_commands:
        print(f"    {q(C.CYN, command)} {q(C.G2, description)}")
    reset_admin_command = f'bublee control {inst.name} "reset admin"'
    print(f"    {q(C.CYN, reset_admin_command)} {q(C.G2, 'Reiniciar solo admin')}")


def cmd_trainer_learn(args):
    """Enseñar algo nuevo a Bublee en lenguaje natural."""
    print_logo(compact=True)
    ...
    if not instruction:
        instruction = prompt("¿Qué quieres que aprenda Bublee?",
    ...
    if r.get("ok"):
        ...
        if r.get("evolution_id"):
            ...
            info(f"Para revertir: bublee desaprender {inst.name} {r['evolution_id']}")
    else:
        fail(f"Error: {r.get('error', 'sin respuesta')}")


def cmd_trainer_unlearn(args):
    """Revertir algo que Bublee aprendió."""
    print_logo(compact=True)
...
def cmd_trainer_info(args):
    ...
    section(f"Bublee Trainer — {inst.label}")
    ...
    commands = [
        ("bublee skills <n>",          "Ver/gestionar skills de comportamiento"),
        ("bublee skill <n> on <id>",   "Activar un skill"),
        ("bublee skill <n> off <id>",  "Desactivar un skill"),
        ("bublee gateway <n>",         "Ver estado del gateway automático"),
        ("bublee control <n>",         "Ver control duro del admin"),
        ("bublee aprender <n>",        "Enseñar algo en lenguaje natural"),
        ("bublee desaprender <n>",     "Revertir algo aprendido"),
        ("bublee simular-cliente <n>", "Simular conversación como cliente"),
    ]
...
def cmd_trainer_simulate_client(args):
    ...
    section(f"Simular Cliente — {inst.label}",
            "Te conviertes en cliente para entrenar a Bublee")
    ...
    while True:
        ...
        with Spinner("Bublee responde...") as sp:
            ...
            webhook_secret = inst.env.get("WEBHOOK_SECRET", "bublee_ultra_5")
            r_msg = inst.api_call(
                "POST",
    ...
    nl()
    info("Para ver las reglas aprendidas: bublee config " + inst.name)
...
# ══════════════════════════════════════════════════════════════════════════════
# bublee watch — Live observatory feed in terminal
# ══════════════════════════════════════════════════════════════════════════════

def cmd_watch(args):
    """
    Live feed en tiempo real de todo lo que hace Bublee.
    ...
    if ping.get("error"):
        ...
        info("Asegúrate de que bublee.py tiene el Observatory integrado")
        return
...
def cmd_diagnose(args):
    ...
    if confirm("¿Pedir diagnóstico al agente IA?"):
        ...
        info(f"Para ver el stream en vivo: bublee watch {inst.name}")
...
def cmd_obs_stats(args):
    ...
    if stats.get("error"):
        fail("Observatory no disponible")
        info("Verifica que el observatory esté inicializado: bublee v8 " + inst.name)
        return
    ...
    info(f"Stream en vivo: bublee watch {inst.name}")
...
def _cmd_language(args):
    ...
    print()
    print(f"  Usage: bublee lang <code>")
    print(f"  Example: bublee lang en")
    print()
...
def main():
    parser = argparse.ArgumentParser(prog="bublee", add_help=False)
    ...
    parser.add_argument("--admin", action="store_true", help="Genera token Bublee Pro Admin")
    ...
    if cmd == "" and not args.help:
        ...
        if not workspace_is_configured() or not get_instances():
            cmd_init(args)
            return
        modern_entrypoint = Path(os.environ.get("BUBLEE_DIR", os.path.dirname(os.path.abspath(__file__)))) / "bublee_app.py"
    ...
    if cmd in ROUTES:
        ...
        if not nl_route(full_text):
            ...
            nl()
            info("'bublee help' para ver comandos")
            info("'bublee i' para modo interactivo")
