...
def _text_overlap(a: str, b: str) -> float:
    ...
    contención (intersección / el más corto de los dos) en vez de Jaccard
    puro. Pensada para preguntas cortas fraseadas distinto — dos preguntas
    sobre lo mismo ("tienen convenio con Sura?" / "aceptan Sura?") comparten
    pocas palabras del total, pero comparten la palabra clave que importa.
    """
    def _keywords(s):
        s = (s or "").lower()
...
class SelfImprovementEngine:
    """
    Analiza los knowledge gaps que bublee_commands.py ya registra en
    knowledge_gaps/*.jsonl (los mismos que muestra /gaps en crudo) y devuelve
    un resumen accionable: qué patrones de pregunta se repiten y qué info
    debería completar el dueño para cerrarlos.
    """
    ...
    async def analyze_performance(self, days: int = 7) -> Dict[str, Any]:
        ...
        for f in sorted(gaps_dir.glob("*.jsonl")):
            try:
                ...
                pass  # nombre de archivo no-fecha: se lee igual
            try:
        ...
        # Agrupar por similitud simple para encontrar patrones repetidos
        ...
        try:
            ...
            log.warning(f"[admin] SelfImprovementEngine LLM error: {e}")
