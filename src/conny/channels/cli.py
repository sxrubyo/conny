...
Comandos:
  conny install           Instalar CLI globalmente (hazlo primero)
  conny init              Primer setup guiado del producto
  conny new               Crear instancia para un cliente (wizard)
  conny list              Ver todas las instancias
  conny dashboard         Panel en tiempo real
  conny chat [n]          Generar tokens, probar bot, ver webhook
  conny sync              Clonar runtime compartido actualizado a todos los clientes
  conny sync-check        Vista previa de qué archivos se sincronizarían (dry-run)
  conny sync-manifest     Ver / editar el manifiesto de archivos sincronizables
  conny fix [n]           Reparar instancias con bug de --cwd (bot no responde)
  conny status [n]        Estado detallado
  conny health            Health check rápido de todas
  ...
  conny upgrade           Actualizar + propagar a todos los clientes
  conny metrics [n]       Métricas de uso
  conny stats [n]         Estadísticas de conversaciones
  conny export [n]        Exportar a JSON/CSV
  conny import [file]     Importar configuración
  conny test [n]          Probar respuestas
  conny stop [n]          Detener
  conny delete [n]        Eliminar
  conny clone [n]         Clonar instancia
  conny reset [n]         Resetear sesión
  conny backup [n]        Crear snapshot
  conny restore [file]    Restaurar snapshot
  conny scale [n] [num]   Escalar workers
  conny audit             Auditoría de seguridad
  conny benchmark         Test de rendimiento
  conny secure            Guía de seguridad
  conny rotate-keys       Rotar secrets
  conny billing           Uso de APIs y costos
  conny omni [sub]        Monitoreo central
  conny nova [sub]        Motor Nova
  conny gateway [n]       Gateway automático de skills y reglas
  conny control [n]       Control duro del admin sobre frases, saludo y trato

V8.0 — NUEVOS COMANDOS:
  conny modelo [n]        Cambiar modelo LLM en caliente (sin reiniciar)
  conny simular [n]       Simular 10 conversaciones — detecta frases de bot
  conny v8 [n]            Estado de los 15 sistemas V8
  conny quality [n]       Score de humanidad de últimas respuestas
  conny briefing [n]      Briefing diario: leads calientes + acción recomendada
  conny cost [n]          Estimación de costo mensual del LLM
  conny latency [n]       Test de latencia HTTP + LLM
  conny diff [n1] [n2]    Comparar configuración entre instancias
  conny watchdog          Monitor continuo con auto-restart inteligente
  conny warmup [n]        Precalentar LLM antes de lanzar
  conny campana [n]       Campañas de seguimiento y reactivación
  conny env-check [n]     Verificar .env contra mejores prácticas V8
  conny rollforward [n]   Aplicar conny.py a instancias
  conny dashboard-v8      Dashboard enriquecido con métricas V8

LOG ENGINE — Análisis inteligente de errores:
  conny logs [n]           Stream en vivo con colores + alerta de errores
  conny logs [n] --scan    Escanear logs: detecta errores con diagnóstico
  conny logs [n] --fix     Detectar errores + aplicar fixes automáticos
  conny logs [n] --errors  Mostrar solo líneas de error
  conny logfix [n]         Atajo: logs --fix
  conny logscan [n]        Atajo: logs --scan
"""
...
try:
    from conny_i18n import get_i18n, set_language, SUPPORTED_LANGUAGES, LANGUAGE_MENU, detect_user_language
    _I18N = get_i18n()
except ImportError:
    class FakeI18n:
...
# IMPORTANTE: el Caddyfile vive SIEMPRE en xus-https, nunca en la carpeta de Conny.
...
SHARED_TELEGRAM_ROUTES = Path(
    os.getenv("CONNY_SHARED_TELEGRAM_ROUTES", f"{CONNY_HOME}/shared_telegram_routes.json")
)
WORKSPACE_CONFIG_PATH = Path(os.getenv("CONNY_WORKSPACE_CONFIG", f"{CONNY_HOME}/config.json"))
...
#   4. Manifiesto           — ~/.conny/sync_manifest.json (lista extra del usuario)
#
...
_SYNC_EXCLUDE_NAMES: set = {
    ...
    "conny_pairing.py",
    "conny_patch.py",
    "conny_sync_fix.py",   # helper temporal de fixes, no es runtime
}
...
_SYNC_MANIFEST_PATH = Path(CONNY_HOME) / "sync_manifest.json"


def _load_sync_manifest() -> dict:
    """Carga el manifiesto desde ~/.conny/sync_manifest.json."""
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTORES - La clave para escalar Conny
# ══════════════════════════════════════════════════════════════════════════════
@dataclass
class Sector:
    id: str
...
def ensure_workspace_files() -> None:
    Path(CONNY_HOME).mkdir(parents=True, exist_ok=True)
...
def runtime_defaults() -> Dict[str, Any]:
    config = load_workspace_config()
    legacy_env = load_env(f"{CONNY_DIR}/.env")
...
def workspace_summary(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    section("Workspace Conny", "Producto limpio y configurable")
    ...
    kv("Agente", cfg.get("agent", {}).get("display_name", "Conny"))
    nl()
...
class Instance:
    ...
    def db_path(self) -> str:
        return self.env.get("DB_PATH", f"{self.dir}/conny.db")
...
def get_instances() -> List[Instance]:
    ...
    # Base instance
    base_env = load_env(f"{CONNY_DIR}/.env")
    if base_env:
...
def cmd_init(args):
    ...
    info("Este init no usa configuraciones del autor. Crea tu workspace limpio en ~/.conny.")
    nl()

    info("Sistema:")
    kv("OS", platform.system())
    kv("Python", sys.version.split()[0])
    kv("Conny home", CONNY_HOME)
    ...
    nl()
    info("Conny ya puede nacer como producto abierto sin estado previo.")
    info("Cuando hables con Conny desde Telegram como admin podrás afinarla con:")
    ...
    info("Siguiente paso: conny new")
    info("Luego: conny config o conny bb config para ajustar prompt y personalidad")
    nl()
...
def cmd_new(args):
    ...
    if os.path.isdir(inst_dir):
        ...
        info(f"Usa 'conny status {name}' para ver su estado")
        return
    ...
    if platform_choice == "telegram":
        ...
        if shared_base_token and confirm("¿Usar el token compartido de Conny base para Telegram?"):
    ...
    with Spinner(f"Creando '{name}'...") as sp:
        ...
        _clone_runtime_entries(CONNY_DIR, inst_dir)
        
        # .env
        env_content = f"""# Conny — {name}
# Sector: {sector.name}
...
HOST=0.0.0.0
BASE_URL={base_url}
WEBHOOK_SECRET=conny_{name}_{webhook_secret}

# DB
DB_PATH={inst_dir}/conny.db
...
"""
    ...
    if agent_seed and any(str(agent_seed.get(key, "")).strip() for key in ("display_name", "role", "prompt_master")):
        ...
        _bb_apply_persona(
            seeded_inst,
            {
                "name": agent_seed.get("display_name") or "Conny",
    ...
    webhook_url_real = f"{base_url}/webhook/conny_{name}_{webhook_secret}"
    if telegram_shared:
        ...
        print(f"       {q(C.CYN, 'El webhook real lo gestiona Conny base (/telegram/shared/...)')}")
    ...
    webhook_path = f"webhook/conny_{name}_{webhook_secret}"
    if telegram_shared:
    ...
    info(f"Genera el token con: conny chat {name}")
    nl()

    # ── Auto-generar token de activacion y mostrarlo ──────────────────────────
    time.sleep(3)  # dar tiempo a que Conny arranque
    h_final = health(port)
    if h_final and master_key:
        try:
            ...
            info(f"Genera el token con: conny token {name}")
    else:
        info(f"Genera el token con: conny token {name}")
...
def cmd_template(args):
    ...
    if sector_id and sector_id in SECTORS:
        ...
        hr()
        info("Usa: conny template <sector_id>")
        info("Ejemplo: conny template dental")
        nl()
...
def cmd_status(args):
    ...
    for inst in instances:
        ...
        kv("PM2", f"conny-{inst.name}" if not inst.is_base else "conny")
        nl()
...
_LOG_ERROR_PATTERNS = [
    ...
    (
        ...
        "conny config {name}  → cambia TELEGRAM_TOKEN\nLuego: conny restart {name}",
    ),
    (
    ...
    (
        ...
        "conny doctor  → verificar espacio\nconny restart {name}",
    ),
    (
        ...
        "conny sync  → clona el runtime compartido actualizado\nconny restart {name}",
    ),
    (
        ...
        "conny config {name}  → agregar API key",
    ),
    (
    ...
    (
        ...
        "El motor Nova no responde — Conny funciona sin él.",
        "conny nova status  → verificar Nova (opcional)",
    ),
]
...
def cmd_logs(args):
    ...
    conny logs [nombre]              — últimas 50 líneas + stream en vivo
    conny logs [nombre] --lines 200  — más líneas históricas
    conny logs [nombre] --errors     — solo líneas de error
    conny logs [nombre] --fix        — analiza + aplica fixes automáticos
    conny logs [nombre] --scan       — escaneo de errores sin stream
    """
    name        = getattr(args, 'name', '')        or ''
            ...
            if errors_found:
                ...
                if crit:
                    print(f"  {q(C.RED, '⚠')}  {q(C.RED, f'{len(crit)} error(es) CRÍTICO(s) detectado(s)')}  "
                          f"{q(C.G3, '→ conny logs ' + inst.name + ' --fix')}")
                elif high:
                    print(f"  {q(C.YLW, '!')}  {q(C.YLW, f'{len(high)} advertencia(s) de nivel ALTO')}  "
                          f"{q(C.G3, '→ conny logs ' + inst.name + ' --scan')}")
                nl()
                    ...
                    if re.search(r"ERROR|CRITICAL|Traceback", line, re.I):
                        ...
                        if error_counter == 3:
                            print(f"\n  {q(C.RED, '⚠')}  {q(C.YLW, '3 errores seguidos — ejecuta: ')} "
                                  f"{q(C.CYN, 'conny logs ' + inst.name + ' --fix')}\n")
                            error_counter = 0
                    else:
...
def cmd_export(args):
    ...
    if fmt_idx == 0:  # JSON completo
        output = BACKUP_DIR / f"conny_{inst.name}_{timestamp}.json"
...
def cmd_test(args):
    ...
    print_logo(compact=True)
    section(f"Test — {inst.label}", "Prueba respuestas de Conny")
    ...
    while True:
        ...
        try:
            ...
            nl()
            print(f"  {q(C.P2, 'ᗧ')}  {q(C.W, 'Conny:', bold=True)}")
...
def cmd_stop(args):
    ...
    if not name:
        fail("Especifica la instancia: conny stop <nombre>")
        return
...
def cmd_delete(args):
    ...
    if not name:
        fail("Especifica la instancia: conny delete <nombre>")
        return
...
def cmd_clone(args):
    ...
    with Spinner(f"Clonando '{source.label}' → '{dest_name}'...") as sp:
        ...
        missing_base_entries = [
            rel_path for rel_path in _runtime_sync_entries(CONNY_DIR)
            if rel_path not in clone_entries
        ]
        if missing_base_entries:
            _clone_runtime_entries(CONNY_DIR, dest_dir, missing_base_entries)
        ...
        src_env["WEBHOOK_SECRET"] = f"conny_{dest_slug}_{_s.token_hex(6)}"
        src_env["DB_PATH"] = f"{dest_dir}/conny.db"
        ...
        env_lines = [f"# Conny — {dest_slug}  (clonado de {source.name})"]
...
def cmd_reset(args):
    ...
    with Spinner("Borrando sesión...") as sp:
        for db_file in [
            ...
            f"{inst.dir}/conny.db-wal",
            f"{inst.dir}/conny.db-shm"
        ]:
...
def cmd_zero(args):
    ...
    print(f"       {q(C.GRN, '✓')}  Archivos de código (conny.py, etc.)")
    ...
    with Spinner(f"Creando backup de seguridad...") as sp:
        try:
            ...
            with _tar.open(backup_out, "w:gz") as tar:
                for f in [".env", "conny.db", "instance.json", "vectors.db",
                          "conny.db-wal", "conny.db-shm"]:
                    p = f"{inst.dir}/{f}"
                    if os.path.exists(p):
                        tar.add(p, arcname=f)
    ...
    db_files = [
        f"{inst.dir}/conny.db",
        f"{inst.dir}/conny_ultra.db",
        f"{inst.dir}/vectors.db",
        f"{inst.dir}/conny.db-wal",
        f"{inst.dir}/conny.db-shm",
        f"{inst.dir}/conny.db-journal",
    ]
    ...
    with Spinner("Aplicando nueva configuración...") as sp:
        ...
        nuevo_wh_secret  = f"conny_{inst.name}_{_s.token_hex(8)}"
    ...
    if nuevo_token and nuevo_token != tg_actual:
        ...
        with Spinner("Registrando webhook en Telegram...") as sp:
            try:
                ...
                warn(f"Registra manualmente: conny webhooks {inst.name}")
    elif nuevo_token:
    ...
    nl()
    info(f"Genera un token de activación con:  conny chat {inst.name}")
    nl()
...
def cmd_backup(args):
    ...
    BACKUP_DIR.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = BACKUP_DIR / f"conny_{inst.name}_{timestamp}.tar.gz"
    ...
    nl()
    info(f"Restaurar con: conny restore {output.name}")
...
def cmd_restore(args):
    ...
    if port:
        ...
        update_env_key(env_path, "DB_PATH", f"{dest_dir}/conny.db")
...
def cmd_scale(args):
    ...
    inst = find_instance(name) if name else None
    if not inst:
        fail("Especifica la instancia: conny scale <nombre> <workers>")
        info("Ejemplo: conny scale clinica-bella 4")
        return
...
def cmd_doctor(args):
    ...
    # Archivos
    info("Archivos core:")
    for f in ["conny.py", "search.py", "knowledge_base.py", "nova_bridge.py"]:
        p = f"{CONNY_DIR}/{f}"
        if os.path.exists(p):
            ok(f"  {f}")
        else:
    ...
    if issues:
        ...
        if log_issues:
            ...
            for li in log_issues:
                ...
                dim(f"  conny logs {inst_n} --fix")
            nl()
        info("Corrige los problemas y ejecuta 'conny doctor' nuevamente")
    else:
...
def cmd_audit(args):
    ...
    gitignore_path = f"{CONNY_DIR}/.gitignore"
    ...
    if issues:
        ...
        for i in issues:
            dim(f"  · {i}")
        nl()
        info("Ejecuta 'conny secure' para ver recomendaciones")
    else:
...
def cmd_secure(args):
    ...
    steps = [
        ...
        ("Backup antes de cambios",
         "conny backup <instancia>",
         "Siempre tener un snapshot"),
        
        ("Usuario no-root",
         "sudo -u conny pm2 start conny.py",
...
def cmd_rotate_keys(args):
    ...
    for inst in instances:
        ...
        new_ws = f"conny_{inst.name}_{_s.token_hex(6)}"
...
def cmd_upgrade(args):
    ...
    with Spinner("Validando sintaxis...") as sp:
        r = subprocess.run(
            [sys.executable, "-m", "py_compile", f"{CONNY_DIR}/conny.py"],
            capture_output=True
        )
        if r.returncode == 0:
            ...
            subprocess.run(
                ["git", "checkout", "HEAD~1", "--", f"{CONNY_DIR}/conny.py"],
                cwd=CONNY_DIR,
                capture_output=True
            )
            return
    ...
    if clientes and confirm(f"¿Propagar actualización a {len(clientes)} instancia(s) de clientes?"):
        ...
        info(f"Recuerda propagar a tus {len(clientes)} clientes con: conny sync")

    nl()
    ok("Conny actualizada")
...
def cmd_chat(args):
    """
    Chat interactivo con cualquier instancia de Conny.
    Genera tokens de activación y permite hablar directamente via API.
    Detecta instancias tanto en /conny como en /conny-instances/.
    """
    print_logo(compact=True)
    name = getattr(args, 'name', '') or ''
    ...
    if not h:
        ...
        nl()
        info(f"Para levantarla: conny restart {inst.name}")
        return
    ...
    options = [
        ...
        "Enviar mensaje de prueba a Conny",
    ...
    # 2. Enviar mensaje de prueba
        ...
        while True:
            ...
            try:
                with Spinner("Conny piensa...") as sp:
                    if _HTTPX:
                        ...
                        if code == 200:
                            ...
                            sp.stop(ok=False)
                            fail("404 — el endpoint /test no existe en conny.py")
                            warn("Sube el conny.py actualizado y reinicia:")
                            info(f"  conny restart {inst.name}")
                            break
                        elif code == 401:
                            sp.stop(ok=False)
                ...
                nl()
                print(f"  {q(C.P3, 'Conny:', bold=True)}")
                ...
                err = str(e)
                if "404" in err:
                    fail("404 — /test no existe en conny.py")
                    warn("Sube conny.py actualizado y reinicia la instancia")
                    break
...
def cmd_non_interactive(args):
    ...
    if not h:
        fail(f"'{inst.label}' no está online")
        info(f"Levántala con: conny restart {inst.name}")
        return 1
...
def cmd_omni(args):
    """Conny Omni — monitoreo central."""
    script = f"{CONNY_DIR}/conny-omni.py"
    
    if not os.path.exists(script):
        fail("conny-omni.py no encontrado")
        return
            ...
            if _HTTPX:
                ...
                if r.status_code == 200:
                    ...
                    if tid:
                        ...
                        pm2_name = "conny" if inst_dir == CONNY_DIR else f"conny-{name}"
                        pm2("restart", pm2_name)
...
def cmd_help(args):
    ...
    commands = [
        ...
        ("Mantenimiento", [
            ("sync", "Propagar conny.py a todos los clientes"),
    ...
    for category, cmds in commands:
        ...
        for cmd_, desc in cmds:
            print(f"    {q(C.G2, f'conny {cmd_:<18}')}  {q(C.G3, desc)}")
        nl()
    ...
    info("Usa lenguaje natural: conny 'crear instancia para veterinaria'")
    nl()

def cmd_install(args):
    """
    Instala el CLI de Conny globalmente para poder ejecutar
    'conny' desde cualquier directorio sin importar dónde
    esté guardado conny_cli.py.
    """
    print_logo(compact=True)
    section("Instalar CLI globalmente")

    this = os.path.abspath(__file__)
    target = "/usr/local/bin/conny"
    target_alt = os.path.expanduser("~/.local/bin/conny")
    ...
    try:
        ...
        nl()
        info("Ahora puedes ejecutar 'conny' desde cualquier directorio.")
        info("Prueba con:  conny list")
        nl()
        return
    ...
    try:
        ...
        if os.path.expanduser("~/.local/bin") not in path_dirs:
            ...
            info("Ya puedes ejecutar 'conny' desde cualquier directorio.")
        nl()
        return
    except Exception as e:
    ...
    nl()
    print(f"  {q(C.G3, 'Opción 1 (requiere sudo):')}  {q(C.CYN, f'sudo ln -sf {this} /usr/local/bin/conny')}")
    print(f"  {q(C.G3, 'Opción 2 (usuario):')}        {q(C.CYN, f'ln -sf {this} ~/.local/bin/conny')}")
    nl()
...
def cmd_demo(args):
    ...
    y cualquier persona que le escriba recibe a Conny funcionando
    ...
    Uso:
      conny demo              → wizard: activar demo (muestra status primero)
      conny demo status       → ver estado de todas las instancias
      conny demo off          → desactivar demo mode
    """
    print_logo(compact=True)
    ...
    if not sub:
        ...
        if not activate:
            nl()
            print(f"  Comandos: {q(C.CYN, 'conny demo status')} · {q(C.CYN, 'conny demo off')}")
            nl()
            return
        ...
        print(f"       Cualquier persona que le escriba recibe a Conny.")
    ...
    if name:
        ...
        if not inst:
            ...
            info("Usa 'conny list' para ver los nombres disponibles")
            return
    ...
    steps = [
        ...
        "Muestra cómo Conny responde en tiempo real",
        ...
        f"Para ver sesiones activas:  conny demo status",
        f"Para apagar la demo:        conny demo off",
    ]
    ...
    print(f"  {q(C.G3, 'Tip:')} {q(C.G2, 'Si alguien pregunta por el bot, Conny les dará tu contacto')}")
    nl()
...
def cmd_rollback(args):
    """
    Rollback de conny.py a una versión anterior.
    Crea backup de la versión actual y restaura la elegida.
    """
    import glob
    section("Rollback de Conny", "Restaura una versión anterior de conny.py")

    backups = sorted(
        glob.glob(f"{CONNY_DIR}/conny.py.bak.*"),
        reverse=True
    )

    if not backups:
        warn("No hay backups disponibles.")
        info(f"Los backups se crean automáticamente en {CONNY_DIR}/conny.py.bak.YYYYMMDD_HHMMSS")
        info("Sube una versión anterior manualmente o usa: cp connyv6.py conny.py")
        return
    ...
    if confirm("¿Sincronizar rollback a todas las instancias?"):
        ...
        for inst in clientes:
            with Spinner(f"Sincronizando {inst.label}...") as sp:
                try:
                    shutil.copy2(current, f"{inst.dir}/conny.py")
                    sp.finish(f"{inst.label} — copiado")
                except Exception as e:
                    sp.finish(f"{inst.label} — ERROR: {e}", ok=False)

    # Reiniciar
    if confirm("¿Reiniciar conny ahora?"):
        with Spinner("Reiniciando...") as sp:
            pm2("restart", "conny")
            time.sleep(3)
            h = health(8001)
            sp.finish("Online ✓" if h else "Reiniciando...", ok=bool(h))
        if h:
            ok("Rollback completado y conny online")
    nl()
...
def cmd_sync(args):
    ...
    TODAS las instancias en conny-instances/.
    ...
    if not clientes:
        warn("No hay instancias de clientes en conny-instances/")
        info("Crea una con: conny new")
        return
    ...
    sync_entries  = _runtime_sync_entries(CONNY_DIR)
    auto_detected = _autodetect_sync_files(CONNY_DIR)

    # ── Mostrar archivos fijos ─────────────────────────────────────────────────
    info("Archivos fijos (siempre sincronizados):")
    for rel_path in [*SYNC_RUNTIME_FILES, *SYNC_RUNTIME_DIRS]:
        src = Path(CONNY_DIR) / rel_path
        if src.is_file():
    ...
    if dry_run:
        ...
        for inst in clientes:
            ...
            nl()
        info("Ejecuta 'conny sync' para aplicar los cambios")
        nl()
        return
    ...
    _bak   = f"{CONNY_DIR}/conny.py.bak.{_stamp}"
    try:
        shutil.copy2(f"{CONNY_DIR}/conny.py", _bak)
        ok(f"Backup creado: conny.py.bak.{_stamp}  (usa 'conny rollback' para revertir)")
    ...
    for inst in clientes:
        ...
        with Spinner(f"Sincronizando {inst.label}...") as sp:
            copied = 0
            try:
                copied = len(_clone_runtime_entries(CONNY_DIR, inst.dir, sync_entries))
    ...
    if no_restart:
        ...
        info("  conny restart all")
    if errors:
        warn(f"Fallaron: {', '.join(errors)}")
        info("Revisa sus logs: conny logs <nombre>")
    nl()
...
def cmd_sync_check(args):
    ...
    Equivalente a 'conny sync --dry-run'.
    """
...
def cmd_sync_manifest(args):
    ...
    Sub-comandos:
      conny sync-manifest           — Ver estado actual
      conny sync-manifest add <f>   — Añadir archivo al sync
      conny sync-manifest remove <f>/ conny sync-manifest rm <f>
                                      — Excluir archivo del sync
      conny sync-manifest reset     — Borrar manifiesto (vuelve a defaults)
    """
    print_logo(compact=True)
    ...
    if sub in ("add", "agregar"):
        target = name or sub  # conny sync-manifest add smart_handoff.py
        if not target or target in ("add", "agregar"):
            fail("Especifica el archivo: conny sync-manifest add <archivo.py>")
            return
        ...
        info("Se incluirá en el próximo 'conny sync'")
        return
    ...
    if sub in ("remove", "rm", "excluir", "quitar"):
        ...
        if not target or target in ("remove", "rm", "excluir", "quitar"):
            fail("Especifica el archivo: conny sync-manifest rm <archivo.py>")
            return
        if target in include:
    ...
    # ── show (default) ────────────────────────────────────────────────────────
    root = Path(CONNY_DIR)
    auto = _autodetect_sync_files(CONNY_DIR)
    ...
    nl()
    info("Comandos:")
    dim("  conny sync-manifest add <archivo>   — añadir al sync")
    dim("  conny sync-manifest rm  <archivo>   — excluir del sync")
    dim("  conny sync-manifest reset            — volver a defaults")
    dim("  conny sync-check                     — preview sin ejecutar")
    nl()


def cmd_base(args):
    """
    Gestiona la instancia BASE de Conny — /home/ubuntu/conny.
    Es el directorio donde desarrollas; las instancias de clientes
    se sincronizan desde aquí con 'conny sync'.

    Sub-comandos:
      conny base              — Estado actual (PM2 + health)
      conny base start        — Registrar en PM2 y arrancar
      conny base restart      — Reiniciar
      conny base stop         — Detener
      conny base logs         — Logs en vivo
      conny base register     — Re-registrar en PM2 (si ya existía)
    """
    print_logo(compact=True)
    ...
    if not inst:
        ...
        info(f"Directorio base buscado: {CONNY_DIR}")
        return
    ...
    if sub in ("", "status", "estado"):
        ...
        if proc:
            ...
            info("Registra con: conny base start")
        ...
        dim("  2. conny base restart   ← prueba en la base")
        dim("  3. conny sync           ← propaga a clientes")
        nl()
        return
    ...
    if sub in ("start", "register", "iniciar", "registrar"):
        ...
        dim("  conny base restart    — reiniciar tras un cambio")
        dim("  conny base logs       — ver logs en vivo")
        dim("  conny sync            — propagar a clientes")
        nl()
        return
    ...
    if sub in ("restart", "reiniciar", "r"):
        ...
        dim("  conny sync   — propaga los cambios a todas las instancias")
        nl()
        return
...
def cmd_fix(args):
    ...
    info("Si alguna aún no responde en Telegram: conny logs <nombre>")
    nl()
...
def cmd_estilo(args):
    ...
    info("Conny leerá los últimos 50 mensajes del admin y aprenderá su forma de escribir")
...
def _admin_cmd(inst, master_key, cmd_text, user_id=None):
    """Enviar un comando al endpoint /test de Conny como si fuera el admin."""
    base_url = f"http://localhost:{inst.port}"
    ...
    if not user_id:
        try:
            ...
            conn = _sq.connect(f"{inst.dir}/conny.db")
        ...
        if missing:
            ...
            if confirm("¿Agregar las rutas faltantes ahora?", default=True):
                for m in missing:
                    ...
                    wpath = f"webhook/conny_{m.name}_{ws}" if ws else f"webhook/conny_{m.name}"
...
def cmd_activar(args):
    ...
    Uso:
      conny activar                  → activa la instancia base
      conny activar 1                → activa instancia 1
    """
    name = getattr(args, 'name', '') or getattr(args, 'subcommand', '') or ''
    inst = find_instance(name) if name else None
    if not inst:
    ...
    if not os.path.exists(db_path):
        ...
        info("¿Está corriendo? conny restart " + inst.name)
        return
    ...
    try:
        ...
        nl()
        info("Reiniciando Conny para aplicar...")
        pm2("restart", inst.pm2_name)
        time.sleep(2)
        h = health(inst.port)
        if h.get("status") == "online":
            ok(f"Conny online en :{inst.port}")
        else:
...
def cmd_bridge(args):
    ...
    Uso:
      conny bridge                   → estado del bridge
      conny bridge restart           → reiniciar bridge
      conny bridge qr                → mostrar QR para conectar
      conny bridge fix               → fijar a qué instancia apunta el bridge
      conny bridge fix 1             → fijar al bridge para la instancia 1
    """
    sub  = getattr(args, 'subcommand', '') or ''
    name = getattr(args, 'name', '')       or ''
    ...
    if target_inst:
        ...
        info("  Usa: conny bridge fix  para seleccionar la instancia correcta")
    ...
    if sub in ("restart", "reiniciar"):
        ...
        if not instances:
            fail("No hay instancias Conny"); return
        ...
        if demo:
            ok("DEMO_MODE=true — Conny responderá a TODOS sin pedir token")
        else:
            if not inst.is_active:
                warn("La instancia no está activada — usa: conny activar")
            else:
        ...
        info("Escríbele a Conny por WhatsApp para probar")
        ...
        try:
            ...
            if result.get("ok"):
                ...
                info("Espera ~20s y verifica con: conny bridge")
            else:
...
def cmd_tokens(args):
    ...
    Uso:
      conny tokens                   → listar tokens de la instancia base
      conny tokens 1                 → listar tokens de instancia 1
    """
    name = getattr(args, 'name', '') or getattr(args, 'subcommand', '') or ''
    inst = find_instance(name) if name else None
    if not inst:
...
def cmd_whatsapp_cli(args):
    ...
    Uso:
      conny whatsapp             → estado WhatsApp de todas las instancias
      conny whatsapp 1           → estado de instancia 1
      conny whatsapp connect 1   → conectar WhatsApp a instancia 1
      conny whatsapp disconnect  → desconectar
      conny whatsapp qr          → mostrar QR de reconexión
    """
    sub  = getattr(args, 'subcommand', '') or ''
    name = getattr(args, 'name', '')       or ''
    ...
    try:
        ...
        if wa_status != "open":
            warn("WhatsApp no conectado — usa: conny whatsapp qr")
    except Exception:
        wa_status = "offline"
    ...
    nl()
    if not sub:
        info("Subcomandos: conny whatsapp qr | connect | disconnect | status")
        return
    ...
    if sub == "qr":
        ...
        info("Cuando aparezca el QR corre: conny whatsapp qr")
...
def _fix_bridge_webhook(inst: Instance):
    ...
    BRIDGE_ENV = "/home/ubuntu/whatsapp-bridge/.env"
    secret = inst.webhook_secret or "conny_2026"
...
def cmd_info(args):
    ...
    Uso:
      conny info                 → info de la instancia base
      conny info 1               → info de instancia 1
    """
    name = getattr(args, 'name', '') or getattr(args, 'subcommand', '') or ''
    inst = find_instance(name) if name else None
    if not inst:
    ...
    if h and inst.master_key:
        try:
            ...
            if tokens:
                ...
                warn("Sin tokens activos — crea uno con: conny token")
        except Exception:
            pass
    nl()
    ...
    # Acciones rápidas
    info("Acciones:")
    print(f"    conny token {inst.name}       — generar token de activacion")
    print(f"    conny activar {inst.name}     — activar instancia")
    print(f"    conny bridge fix              — arreglar bridge WhatsApp")
    print(f"    conny restart {inst.name}     — reiniciar")
    print(f"    conny logs {inst.name}        — ver logs")
    nl()
...
def cmd_pair(args):
    ...
    Usos:
      conny pair list
      conny pair default <instancia>
      conny pair detach <chat_id>
      conny pair <instancia> <chat_id>
    """
    ...
    if sub == "default":
        if not name:
            fail("Uso: conny pair default <instancia>")
            return
        inst = find_instance(name)
        if not inst:
    ...
    if sub in ("detach", "rm", "remove"):
        if not name:
            fail("Uso: conny pair detach <chat_id>")
            return
    ...
    if not inst or not name:
        fail("Uso: conny pair <instancia> <chat_id>")
        return
...
def cmd_bb_config(args):
    """Configura el agente Conny de una instancia: nombre, prompt, personalidad y skills."""
...
def cmd_interactive(args):
    """Modo interactivo — shell de Conny."""
    print_logo(compact=True)
    section("Modo Interactivo", "Escribe comandos sin 'conny'. 'exit' para salir.")
    ...
    while True:
        try:
            sys.stdout.write(f"\n  {q(C.P2, 'ᗧ', bold=True)} {q(C.G2, 'conny')} {q(C.P3, '›')} ")
            sys.stdout.flush()
            line = input("").strip()
        except (EOFError, KeyboardInterrupt):
            nl()
...
# ALERTAS Y MONITOREO
# ══════════════════════════════════════════════════════════════════════════════
ALERTS_FILE = Path.home() / ".conny" / "alerts.json"
...
def cmd_alerts(args):
    ...
    if sub == "add":
        ...
        if not alerts:
            ...
            info("Crea una con: conny alerts add")
            return
...
def cmd_webhooks(args):
    ...
    if platform == "telegram":
        ...
        webhook_secret = ev.get("WEBHOOK_SECRET", "conny_2026")
        ...
        kv("Verify Token", ev.get("WA_VERIFY_TOKEN", "conny_verify"))
...
def cmd_search(args):
    ...
    if choice in [0, 2]:
        ...
        instances = get_instances()
        for inst in instances:
            log_path = f"{inst.dir}/logs/conny.log"
...
def cmd_batch(args):
    ...
    if operation == "restart":
        ...
        for inst in selected:
            ...
            with tarfile.open(output, "w:gz") as tar:
                for f in [".env", "conny.db", "instance.json"]:
                    p = f"{inst.dir}/{f}"
                    if os.path.exists(p):
                        tar.add(p, arcname=f)
...
# QUICK ACTIONS
# ══════════════════════════════════════════════════════════════════════════════
QUICK_ACTIONS_FILE = Path.home() / ".conny" / "quick_actions.json"
...
def cmd_quick(args):
    ...
    if sub == "add":
        ...
        if not actions:
            ...
            info("Crea una con: conny quick add")
            return
        ...
        info("Ejecutar: conny quick run <nombre>")
        info("Crear: conny quick add")
...
def cmd_debug(args):
    ...
    kv("CWD", os.getcwd())
    kv("CONNY_DIR", CONNY_DIR)
    kv("INSTANCES_DIR", INSTANCES_DIR)
    nl()
    
    # Variables de entorno
    info("Variables de entorno relevantes:")
    env_vars = ["CONNY_DIR", "INSTANCES_DIR", "NOVA_DIR", "TERM", "SHELL", "USER", "HOME"]
    for var in env_vars:
...
# ══════════════════════════════════════════════════════════════════════════════
PLUGINS_DIR = Path.home() / ".conny" / "plugins"
...
def cmd_plugins(args):
    ...
    if sub == "list":
        ...
        if not plugins:
            ...
            info("Los plugins van en: ~/.conny/plugins/")
            return
...
def cmd_cron(args):
    ...
    # Mostrar tareas conny existentes
    try:
        result = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        current = result.stdout if result.returncode == 0 else ""
        
        conny_tasks = [l for l in current.split('\n') if 'conny' in l.lower()]
        
        if conny_tasks:
            info("Tareas de Conny en cron:")
            for t in conny_tasks:
                print(f"    {q(C.G1, t)}")
        else:
            info("No hay tareas de Conny en cron")
    except Exception:
    ...
    templates = [
        ("Backup diario (3am)", "0 3 * * * /usr/local/bin/conny backup base"),
        ("Health check (cada 5min)", "*/5 * * * * /usr/local/bin/conny health --quiet"),
        ("Rotate keys (mensual)", "0 0 1 * * /usr/local/bin/conny rotate-keys --auto"),
        ("Limpieza logs (semanal)", "0 4 * * 0 /usr/local/bin/conny cleanup logs"),
    ]
...
def cmd_guide(args):
    """
    Guía completa paso a paso para operar Conny desde la terminal.
    ...
    print(f"          {q(C.CYN, 'conny zero <nombre>')}  ← borra datos, permite renombrar")

    hr()

    # ── SECCIÓN 2: Ver estado de las instancias ────────────────────────────────
    print(f"\n  {q(C.P1, '2.', bold=True)}  {q(C.W, 'VER ESTADO', bold=True)}")
    print(f"\n  {q(C.G3, 'Ver todas las instancias:')}     {q(C.CYN, 'conny list')}")
    print(f"  {q(C.G3, 'Dashboard en tiempo real:')}     {q(C.CYN, 'conny dashboard')}")
    print(f"  {q(C.G3, 'Health check rápido:')}          {q(C.CYN, 'conny health')}")
    print(f"  {q(C.G3, 'Estado detallado de uno:')}      {q(C.CYN, 'conny status <nombre>')}")
    print(f"\n  Ejemplo: {q(C.CYN, 'conny status clinica-perez')}")

    hr()

    # ── SECCIÓN 3: Levantar / Reiniciar ───────────────────────────────────────
    print(f"\n  {q(C.P1, '3.', bold=True)}  {q(C.W, 'LEVANTAR Y REINICIAR', bold=True)}")
    print(f"\n  {q(C.G3, 'Reiniciar una instancia:')}      {q(C.CYN, 'conny restart <nombre>')}")
    print(f"  {q(C.G3, 'Reiniciar TODAS:')}               {q(C.CYN, 'conny restart all')}")
    print(f"  {q(C.G3, 'Detener una:')}                   {q(C.CYN, 'conny stop <nombre>')}")
    print(f"\n  {q(C.G3, 'Si una instancia no responde, prueba en este orden:')}")
    print(f"    {q(C.YLW, '1)')} {q(C.CYN, 'conny health')}  → ¿está online?")
    print(f"    {q(C.YLW, '2)')} {q(C.CYN, 'conny logs <nombre>')}  → ¿qué error muestra?")
    print(f"    {q(C.YLW, '3)')} {q(C.CYN, 'conny restart <nombre>')}  → reiniciarla")
    print(f"    {q(C.YLW, '4)')} {q(C.CYN, 'conny webhooks <nombre>')}  → verificar webhook en Telegram")

    hr()

    # ── SECCIÓN 4: Logs ───────────────────────────────────────────────────────
    print(f"\n  {q(C.P1, '4.', bold=True)}  {q(C.W, 'VER LOGS (para detectar errores)', bold=True)}")
    print(f"\n  {q(C.G3, 'Logs en tiempo real:')}          {q(C.CYN, 'conny logs <nombre>')}")
    print(f"  {q(C.G3, 'Logs directos de PM2:')}          {q(C.CYN, 'pm2 logs conny-<nombre>')}")
    ...
    print(f"\n  {q(C.G3, 'Verificar y reconfigurar:')}    {q(C.CYN, 'conny webhooks <nombre>')}")
    ...
    print(f"\n  {q(C.G3, 'Encontrar el SECRET:')}  {q(C.CYN, 'conny config <nombre>')}")
    ...
    print(f"\n  {q(C.G3, 'Asistente de config:')}          {q(C.CYN, 'conny config <nombre>')}")
    print(f"  {q(C.G3, 'Editar .env directamente:')}      {q(C.CYN, 'nano /home/ubuntu/conny-instances/<nombre>/.env')}")
    print(f"\n  {q(C.YLW, 'Después de editar el .env siempre reinicia:')}  {q(C.CYN, 'conny restart <nombre>')}")

    hr()

    # ── SECCIÓN 7: Diagnóstico ────────────────────────────────────────────────
    print(f"\n  {q(C.P1, '7.', bold=True)}  {q(C.W, 'DIAGNÓSTICO COMPLETO DEL SISTEMA', bold=True)}")
    print(f"\n  {q(C.CYN, 'conny doctor')}   → revisa dependencias, puertos, configuración")
    print(f"  {q(C.CYN, 'conny debug')}    → info técnica detallada de todas las instancias")
    print(f"\n  {q(C.G3, 'Comandos PM2 directos (cuando conny CLI no basta):')}")
    print(f"    {q(C.CYN, 'pm2 list')}              → ver todos los procesos")
    print(f"    {q(C.CYN, 'pm2 restart all')}       → reiniciar todos")
    print(f"    {q(C.CYN, 'pm2 delete conny-X')}  → borrar proceso específico")
    ...
    problems = [
        (
            ...
            [
                ...
                f"Ejecuta: {q(C.CYN, 'conny webhooks <nombre>')} → opción 1 para ver la URL",
        ...
        (
            ...
            [
                "El proceso arrancó pero conny.py falló internamente",
                f"Ejecuta: {q(C.CYN, 'conny logs <nombre>')} y busca el error",
                "Causas comunes: falta una API key, .env incorrecto, puerto ocupado",
                f"Solución rápida: {q(C.CYN, 'conny restart <nombre>')}",
            ]
        ),
        (
            ...
            [
                ...
                f"Para liberar un puerto muerto: {q(C.CYN, 'pm2 delete conny-<nombre>')}",
            ]
        ),
        (
            ...
            [
                ...
                f"Solución: {q(C.CYN, 'pm2 delete conny-<nombre>')} → luego {q(C.CYN, 'conny restart <nombre>')}",
    ...
    quick_ref = [
        ("Instalar CLI globalmente",     "conny install"),
        ("Nueva instancia",              "conny new"),
        ("Ver todas",                    "conny list"),
        ("Dashboard live",               "conny dashboard"),
        ("Health check",                 "conny health"),
        ("Status de una",                "conny status <nombre>"),
        ("Generar token + probar bot",   "conny chat <nombre>"),
        ("Logs en vivo",                 "conny logs <nombre>"),
        ("Reiniciar una",                "conny restart <nombre>"),
        ("Reiniciar todas",              "conny restart all"),
        ("Reparar bug de --cwd",         "conny fix <nombre>"),
        ("Clonar runtime compartido a todos",  "conny sync"),
        ("♻  Entregar a nuevo cliente",  "conny zero <nombre>"),
        ("Parar una",                    "conny stop <nombre>"),
        ("Editar configuración",         "conny config <nombre>"),
        ("Ver/fijar webhook",            "conny webhooks <nombre>"),
        ("Diagnóstico completo",         "conny doctor"),
        ("Actualizar + propagar",        "conny upgrade"),
        ("Esta guía",                    "conny guide"),
        ("",                             ""),
        ("Pipeline de leads",            "conny pipeline [instancia]"),
        ("Por qué se van clientes",      "conny perdidos [instancia]"),
        ("Coach de ventas",              "conny coach [instancia]"),
        ("Clonar estilo del dueño",      "conny estilo [instancia]"),
        ("Reactivar inactivos",          "conny reactivar [instancia]"),
        ("Reporte ahora",               "conny reporte [instancia]"),
        ("Mensaje masivo",               "conny broadcast [instancia]"),
        ("Instagram DMs",               "conny instagram [instancia]"),
        ("Pagos desde chat",            "conny pagos [instancia]"),
    ]
    ...
    nl()
    info("Tip: escribe 'conny i' para modo interactivo con autocompletado")
    nl()
...
def cmd_help_extended(args):
    ...
    command_groups = [
        ...
        ("V8.0 — Calidad & Modelo", [
            ...
            ("rollforward [n]",   "Aplicar conny.py a todas las instancias"),
    ...
    for category, cmds in command_groups:
        print(f"  {q(C.P2, category, bold=True)}")
        for cmd_, desc in cmds:
            print(f"    {q(C.G2, f'conny {cmd_:<20}')} {q(C.G3, desc)}")
        nl()
    ...
    shortcuts = [
        ("conny i", "Modo interactivo"),
        ("conny l", "Listar instancias"),
        ("conny s", "Status"),
        ("conny r all", "Reiniciar todo"),
    ]
    for short, desc in shortcuts:
        print(f"    {q(C.CYN, short)}  →  {q(C.G3, desc)}")
    nl()
    
    info("Lenguaje natural: conny 'crear veterinaria para clínica peludo'")
    nl()
...
def cmd_modelo(args):
    ...
    # Subcomando directo (conny modelo [n] gemini-flash)
...
def _apply_model_change(inst: "Instance", alias: str):
    ...
    if alias == "reset":
        with Spinner(f"Restaurando modelo original...") as sp:
            ...
            if r.get("ok") or "reset" in str(r).lower():
                ...
                info("¿La instancia está corriendo conny.py?")
        return
    ...
    nl()
    ok(f"Para verificar: conny v8 {inst.name}")
...
SIMULATION_SCENARIOS = {
    ...
    "conversacion_larga":"Conversación larga — ¿Conny mantiene coherencia?",
}
...
def cmd_simular(args):
    ...
    if not h:
        ...
        info(f"Arráncala con: conny restart {inst.name}")
        return
    ...
    if ping.get("error") or not ping.get("v8"):
        ...
        info(f"Verifica: grep -c 'v8/ping' /home/ubuntu/conny/conny.py")
        info(f"Si es 0: sube el conny.py con V8 y reinicia")
        return
    ...
    if scenario_id == "all":
        ...
        if r.get("error"):
            ...
            info(f"Verifica: conny v8 {inst.name}")
            info(f"Si V8 no está activo: conny restart {inst.name}")
            return
    ...
    nl()
    info(f"Para ver el pipeline de leads: conny pipeline {inst.name}")
...
def _print_simulation_report(report: dict):
    ...
    if failed == 0 and warned == 0:
        ...
        info("Conny responde de forma humana y sin frases de bot")
...
def _print_scenario_result(r: dict):
    ...
    if turns:
        ...
        for i, t in enumerate(turns):
            ...
            print(f"         {q(C.G3, 'Conny:')} {q(color, t.get('conny', '')[:60])}")
            if t.get("failures"):
...
def cmd_v8(args):
    ...
    with Spinner("Verificando endpoints V8...") as sp:
        ...
        sp.finish(
            ...
            else "Endpoints V8 no encontrados — ¿el conny.py tiene V8?",
            ok=ping_ok
        )
    ...
    if not ping_ok:
        ...
        dim(f"   grep -c 'v8/ping' /home/ubuntu/conny/conny.py")
        dim(f"   (debe retornar 1 o más)")
        dim(f"2. Si el resultado es 0: el conny.py no tiene V8 — sube el nuevo archivo")
        dim(f"3. Si tiene V8: revisa los logs: conny logs {inst.name}")
        dim(f"4. Reinicia: conny restart {inst.name}")
        nl()
    ...
    if v8_status.get("error"):
        ...
        dim("1. El proceso conny.py no cargó los sistemas V8 — revisa los logs")
        dim("   conny logs " + inst.name)
        dim("2. La versión de conny.py no tiene V8 — verifica con 'cat conny.py | head -20'")
        dim("3. Reinicia la instancia: conny restart " + inst.name)
        nl()
        info("Estado del servidor básico:")
        kv("Status",          h.get("status", "?"))
        kv("Versión conny",  h.get("version", "no reportada"))
        kv("Clinic",          h.get("clinic", "?"))
    ...
    if v8_init_status == "v8_not_initialized":
        ...
        for note in v8_status.get("init_notes", []):
            dim(f"  • {note}")
        nl()
        warn("Solución: reinicia la instancia → conny restart " + inst.name)
        nl()
    ...
    nl()
    info(f"Simular conversaciones: conny simular {inst.name}")
    info(f"Correr tests automáticos: conny test {inst.name}")
    info(f"Cambiar modelo: conny modelo {inst.name}")
...
def cmd_quality(args):
    """
    Analiza la humanidad de las últimas respuestas de Conny.
    ...
    try:
        ...
        # conny usa "ts" como columna de timestamp en conversations
    ...
    if bot_hits:
        ...
        if avg_score < 0.75:
            ...
            info("Sube el nivel: conny filtro 3 " + inst.name)
        else:
    ...
    nl()
    info(f"Para mejorar: conny modelo {inst.name}  (cambiar a modelo más natural)")
    info(f"Para simular: conny simular {inst.name}  (probar escenarios)")
...
def _generate_local_briefing(inst: "Instance", hours: int = 24):
    ...
    try:
        ...
        # conny usa 'ts' como columna de timestamp
        try:
        ...
        if new_convs > 0 and new_apts == 0:
            ...
            info(f"Revisa: conny quality {inst.name}")
        elif stuck > 2:
            warn(f"Hay {stuck} conversaciones sin respuesta por más de 12h")
            info(f"Revisa: conny status {inst.name}")
        elif new_apts > 0:
...
def cmd_cost(args):
    ...
    if cost_per_month > 50:
        ...
        info(f"Para cambiar: conny modelo {inst.name}")
    else:
        ...
        if confirm("¿Medir latencia del LLM (más lento, ~5-20s)?"):
            ...
            if llm_latencies:
                ...
                if avg_llm > 10000:
                    ...
                    info(f"Gemini Flash o Llama-70B son más rápidos: conny modelo {inst.name}")
...
def cmd_watchdog(args):
    ...
    if sub == "stop":
        ...
        pid_file = Path.home() / ".conny" / "watchdog.pid"
        if pid_file.exists():
            try:
    ...
    if sub == "status":
        ...
        if pid_file.exists():
            ...
            info("Arranca con: conny watchdog start")
        return
    ...
    if background:
        ...
        try:
            ...
            if pid > 0:
                ...
                info(f"Ver estado: conny watchdog status")
                info(f"Detener: conny watchdog stop")
                return
...
def cmd_warmup(args):
    ...
    for i, user_msg in enumerate(warmup_prompts):
        with Spinner(f"Warmup {i+1}/{len(warmup_prompts)}: '{user_msg}'") as sp:
            start = time.time()
            # Usar el endpoint de test que existe en conny
            r = inst.api_call(
...
def cmd_campana(args):
    ...
    if sub == "list" or not sub:
        ...
        if r.get("error"):
            ...
            info("¿Los sistemas V8 están activos? conny v8 " + inst.name)
            return
        ...
        info(f"  conny campana activate {inst.name}")
        ...
        if r.get("ok") or "reactivar" in str(r).lower():
            ok("Campaña de reactivación enviada a Conny")
            info("Conny procesará los follow-ups de forma escalonada")
        else:
            # Fallback directo a API de Conny
...
def cmd_rollforward(args):
    """
    Aplica conny.py a todas las instancias (o a una específica).
    Actualiza el archivo sin perder configuración (.env y DB se conservan).
    """
    print_logo(compact=True)
    section("Roll Forward — Aplicar V8 a instancias",
            "Actualiza conny.py → conny.py en todas las instancias")

    # Verificar que existe conny.py en el directorio base
    # Buscar el conny.py actualizado — puede estar en varios lugares
    candidates = [
        f"{CONNY_DIR}/conny.py",
        "./conny.py",
        os.path.expanduser("~/conny/conny.py"),
        os.path.expanduser("~/conny.py"),
    ]
    v8_source = next((p for p in candidates if os.path.exists(p)), None)
    if not v8_source:
        fail("No se encontró conny.py")
        info(f"Directorio base buscado: {CONNY_DIR}/conny.py")
        info("Asegúrate de que el conny.py con V8 esté en el directorio base")
        return
    ...
    for inst in targets:
        with Spinner(f"Actualizando {inst.label}...") as sp:
            try:
                # Backup del conny.py actual
                current_conny = f"{inst.dir}/conny.py"
                if os.path.exists(current_conny):
                    backup_path = f"{inst.dir}/conny_backup_{int(time.time())}.py"
                    shutil.copy(current_conny, backup_path)

                # Copiar v8
                shutil.copy(v8_source, current_conny)
    ...
    if success_count > 0:
        info("Verifica con: conny v8 <instancia>")
        info("Prueba con: conny simular <instancia>")
...
def cmd_dashboard_v8(args):
    ...
    def draw():
        ...
        print(f"  {q(C.P1, '✦', bold=True)}  "
              f"{q(C.W, 'Conny V8 Dashboard', bold=True)}  "
              f"{q(C.G3, now)}  "
        ...
        hr()
        print(f"  {q(C.G3, 'Actualiza cada 3s  ·  modelo: conny modelo <n>  ·  v8: conny v8 <n>')}")
...
ROUTES["log"]          = cmd_logs
ROUTES["logfix"]       = cmd_logs   # conny logfix [n] → directo al --fix
ROUTES["logscan"]      = cmd_logs   # conny logscan [n] → directo al --scan
...
_v8_section = (
    "V8.0 — Calidad & Modelo", [
        ...
        ("campana [n]",      "Campañas de seguimiento y reactivación"),
        ("env-check [n]",    "Verificar .env contra mejores prácticas V8"),
        ("rollforward [n]",  "Aplicar conny.py a instancias"),
        ("dashboard-v8",     "Dashboard enriquecido con métricas V8"),
...
def cmd_trainer_skills(args):
    """Ver y gestionar skills de comportamiento de Conny."""
    print_logo(compact=True)
    ...
    if sub in ("on", "off", "activar", "desactivar"):
        # conny skills [on|off] [skill_id]
        skill_id = getattr(args, "name", "") or ""
        if not skill_id:
            fail("Formato: conny skill [skill_id] on/off")
            return
    ...
    if r.get("error"):
        ...
        info("¿El trainer está activo? Verifica: conny v8 " + inst.name)
        return
    ...
    nl()
    info("Para activar:  conny skill <instancia> on  <skill_id>")
    info("Para desactivar: conny skill <instancia> off <skill_id>")
...
def cmd_trainer_gateway(args):
    ...
    section(f"Gateway automático — {inst.label}",
            "Conny decide skills y preferencias sin obligarte a configurarlas a mano")
    ...
    nl()
    info("Comandos:")
    print(f"    {q(C.CYN, f'conny gateway {inst.name:<24}')} {q(C.G2, 'Ver estado')}")
    print(f"    {q(C.CYN, f'conny gateway {inst.name} on')} {q(C.G2, 'Activar todo el gateway')}")
    print(f"    {q(C.CYN, f'conny gateway {inst.name} off')} {q(C.G2, 'Desactivar todo el gateway')}")
    print(f"    {q(C.CYN, f'conny gateway {inst.name} admin-on')} {q(C.G2, 'Activar solo admin')}")
    print(f"    {q(C.CYN, f'conny gateway {inst.name} admin-off')} {q(C.G2, 'Desactivar solo admin')}")
    print(f"    {q(C.CYN, f'conny gateway {inst.name} user-on')} {q(C.G2, 'Activar solo usuario')}")
    print(f"    {q(C.CYN, f'conny gateway {inst.name} user-off')} {q(C.G2, 'Desactivar solo usuario')}")
...
def cmd_trainer_control(args):
    ...
    info("Ejemplos:")
    example_commands = [
        (f'conny control {inst.name} "no digas ay"', "Prohibir una frase"),
        (f'conny control {inst.name} "no digas ay, di entiendo"', "Forzar reemplazo"),
        (f'conny control {inst.name} "no uses claro al inicio"', "Bloquear arranque"),
        (f'conny control {inst.name} "saluda así: Hola, Conny por acá"', "Fijar saludo base"),
        (f'conny control {inst.name} "segunda burbuja: Estoy lista para ayudarle con la instancia"', "Fijar segunda burbuja"),
        (f'conny control {inst.name} "tercera burbuja: Si quiere, lo dejamos listo ahora"', "Fijar tercera burbuja"),
        (f'conny control {inst.name} "cierra así: Si quiere, lo dejamos listo ahora"', "Fijar cierre"),
        (f'conny control {inst.name} "si no entiendes di: Perdón, ayúdeme con un poco más de contexto"', "Fijar fallback"),
        (f'conny control {inst.name} "responde en 2 burbujas"', "Limitar burbujas"),
        (f'conny control {inst.name} "conmigo háblame de usted y más directo"', "Ajustar trato admin"),
    ]
    for command, description in example_commands:
        print(f"    {q(C.CYN, command)} {q(C.G2, description)}")
    reset_admin_command = f'conny control {inst.name} "reset admin"'
    print(f"    {q(C.CYN, reset_admin_command)} {q(C.G2, 'Reiniciar solo admin')}")


def cmd_trainer_learn(args):
    """Enseñar algo nuevo a Conny en lenguaje natural."""
    print_logo(compact=True)
    ...
    if not instruction:
        instruction = prompt("¿Qué quieres que aprenda Conny?",
    ...
    if r.get("ok"):
        ...
        if r.get("evolution_id"):
            ...
            info(f"Para revertir: conny desaprender {inst.name} {r['evolution_id']}")
    else:
        fail(f"Error: {r.get('error', 'sin respuesta')}")


def cmd_trainer_unlearn(args):
    """Revertir algo que Conny aprendió."""
    print_logo(compact=True)
...
def cmd_trainer_info(args):
    ...
    section(f"Conny Trainer — {inst.label}")
    ...
    commands = [
        ("conny skills <n>",          "Ver/gestionar skills de comportamiento"),
        ("conny skill <n> on <id>",   "Activar un skill"),
        ("conny skill <n> off <id>",  "Desactivar un skill"),
        ("conny gateway <n>",         "Ver estado del gateway automático"),
        ("conny control <n>",         "Ver control duro del admin"),
        ("conny aprender <n>",        "Enseñar algo en lenguaje natural"),
        ("conny desaprender <n>",     "Revertir algo aprendido"),
        ("conny simular-cliente <n>", "Simular conversación como cliente"),
    ]
...
def cmd_trainer_simulate_client(args):
    ...
    section(f"Simular Cliente — {inst.label}",
            "Te conviertes en cliente para entrenar a Conny")
    ...
    while True:
        ...
        with Spinner("Conny responde...") as sp:
            ...
            webhook_secret = inst.env.get("WEBHOOK_SECRET", "conny_ultra_5")
            r_msg = inst.api_call(
                "POST",
    ...
    nl()
    info("Para ver las reglas aprendidas: conny config " + inst.name)
...
# ══════════════════════════════════════════════════════════════════════════════
# conny watch — Live observatory feed in terminal
# ══════════════════════════════════════════════════════════════════════════════

def cmd_watch(args):
    """
    Live feed en tiempo real de todo lo que hace Conny.
    ...
    if ping.get("error"):
        ...
        info("Asegúrate de que conny.py tiene el Observatory integrado")
        return
...
def cmd_diagnose(args):
    ...
    if confirm("¿Pedir diagnóstico al agente IA?"):
        ...
        info(f"Para ver el stream en vivo: conny watch {inst.name}")
...
def cmd_obs_stats(args):
    ...
    if stats.get("error"):
        fail("Observatory no disponible")
        info("Verifica que el observatory esté inicializado: conny v8 " + inst.name)
        return
...
def _cmd_language(args):
    ...
    print()
    print(f"  Usage: conny lang <code>")
    print(f"  Example: conny lang en")
    print()
    print("  Available codes: " + ", ".join(SUPPORTED_LANGUAGES.keys()))


def main():
    parser = argparse.ArgumentParser(prog="conny", add_help=False)
    ...
    if cmd in ROUTES:
        ...
        if not nl_route(full_text):
            ...
            nl()
            info("'conny help' para ver comandos")
            info("'conny i' para modo interactivo")
