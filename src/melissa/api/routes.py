"""FastAPI routes extracted from melissa.py monolith."""
from __future__ import annotations
# TODO: These routes reference globals: melissa, db, llm_engine, Config, app
...
async def webhook(secret: str, request: Request):
    ...
    urgent = melissa.is_urgent(text or "")
    route = {"platform": platform}
    asyncio.create_task(
        melissa.enqueue_message(
            chat_id,
            text or "",
            urgent=urgent,
...
¿Confirmas enviarlo? Responde *CONFIRMAR* para enviar o *CANCELAR* para cancelar."""
        ...
        await melissa._send_message(ADMIN_TELEGRAM_ID, confirm_text)
        return {
            "ok": False, 
...
async def _handle_admin_confirm(chat_id: str, text: str) -> List[str]:
    ...
    await melissa._send_message(lead_chat_id, message)
...
async def smart_handoff_to_santiago(lead_chat_id: str, question: str, melissa_response: str = "") -> bool:
    """Notifica a Santiago (6908159885) cuando Melissa no sabe la respuesta."""
    import httpx
    
    SANTIAGO_TELEGRAM_ID = "6908159885"
    message = f"🔔 LEAD sin respuesta clara:\n\n📱 Chat: {lead_chat_id}\n❓ Pregunta: {question}\n🤖 Respuesta de Melissa: {melissa_response[:200] if melissa_response else 'Sin respuesta'}\n\n➡️ Contactar al lead con la respuesta."
...
async def export_data():
    ...
    try:
        if melissa:
            store = melissa._brand_store(clinic)
            if store:
...
async def brand_status():
    clinic = db.get_clinic() if db else {}
    if not melissa:
        return {"enabled": False, "reason": "melissa_not_ready"}
    store = melissa._brand_store(clinic)
    if not store:
        ...
        for turn_idx in range(max_turns):
            ...
            try:
                client_msg = await self._simulate_client_response(
                    melissa_resp, history, scenario["client_persona"], scenario["sector"], scenario
                )
            except Exception as e:
...
{addon}"""
    ...
    def _fallback_client_response(
        ...
        msg = _normalize_conv_text(melissa_msg)
    ...
    async def _simulate_client_response(self, melissa_msg: str, history: List[Dict],
        ...
        if not llm_engine:
            return self._fallback_client_response(melissa_msg, persona, sector, scenario)
...
Si Melissa hizo una pregunta concreta, responde con una frase completa.
...
Si se fue sin interés, responde "CLIENTE_SE_FUE"."""
    ...
    def _detect_loop(self, response: str, history: List[Dict]) -> bool:
        ...
        prev_melissa = [h["content"] for h in history if h["role"] == "assistant"][-3:]
        previous_signatures = [self._question_signature(prev) for prev in prev_melissa if self._question_signature(prev)]
...
class SelfTestSuite:
    ...
    def _test_first_contact_intro(self):
        """El primer saludo debe presentar a Melissa y aterrizar el negocio."""
        if not melissa:
            self._record("FIRST_CONTACT_INTRO", False, "melissa no inicializado")
            return
        try:
            clinic = {"name": "Clinica Demo", "services": ["Botox", "Armonizacion facial"]}
            personality = apply_archetype("amigable", "Melissa")
            result = melissa._normalize_first_patient_turn(
            ...
            assert "melissa" in parts[0].lower(), f"Debe presentarse: {parts[0]}"
...
class TrainerGateway:
    ...
    def _recommend_skill_ids(
        ...
        *,
        ...
        # WhatsApp tolera un poco más de calidez, pero sin llevar a Melissa a lo informal.
    ...
    def maybe_acknowledge_user_preference_message(
        ...
        *,
        ...
        """
        Si el mensaje del usuario solo está corrigiendo el estilo de Melissa,
...
class OwnerStyleController:
    """
    Plano de control duro del owner/admin sobre cómo habla Melissa.
...
async def _admin_control_status(chat_id: str) -> List[str]:
    ...
    return [
        "\n".join([
            ...
            '  "Saluda así: Hola, Melissa por acá..."',
...
async def _admin_control_apply(chat_id: str, text: str) -> List[str]:
    ...
    if not result.get("ok"):
        return [
            ...
            "Prueba con algo como: no digas 'ay' | no digas 'ay', di 'entiendo' | no empieces con 'claro' | saluda así: Hola, Melissa por acá",
        ]
