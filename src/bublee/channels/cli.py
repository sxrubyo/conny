...
INSTANCES_DIR = os.getenv("INSTANCES_DIR", str(Path.home() / "bublee/instances"))
...
def print_logo(compact=False, sector=None):
    from bublee.channels.logo_art import LOGO_ART
    print()
    YLW = _e("38;5;220")
    ...
    print(f"  {q(C.P1, '✦')} {q(C.G2, f'Bublee v{VERSION}')}  "
...
def edit_workspace_config() -> Dict[str, Any]:
    ...
    agent_name = questionary.text("Nombre del agente:", default=agent_cfg.get("display_name", "Bublee")).ask()
...
def cmd_config(args):
    ...
    if not name:
        ...
        print_logo(compact=True)
        section("Bublee Config", "Hub de configuración del producto")
...
def cmd_demo(args):
    ...
    if sub in ("off", "apagar", "stop", "desactivar"):
        ...
        if inst.is_base:
            try:
                update_env_key("/home/ubuntu/bublee/.env", "DEMO_MODE", "false")
            except Exception:
                pass
        pm2_name = "bublee" if inst.is_base else f"bublee-{inst.name}"
    ...
    with Spinner("Configurando modo demo...") as sp:
        ...
        if inst.is_base:
            try:
                update_env_key("/home/ubuntu/bublee/.env", "DEMO_MODE", "true")
            except Exception:
                pass
