...
class BubleeAdmin:
    ...
    async def handle(self, chat_id: str, text: str, clinic: Dict,
        ...
        try:
            ...
            try:
                ...
                pass
            log.error(f"[admin_dashboard] LLM falló: {e}")
            raise
    ...
    async def _admin_conversation(self, chat_id: str, text: str, clinic: Dict, db, llm_engine) -> List[str]:
        ...
        if any(t in text.lower() for t in research_triggers) or (
            ...
            try:
                from bublee_web_search import search_business
        ...
        sys_prompt = f"""{STYLE_GUIDELINES}\n\nEres Bublee, una empleada NUEVA que acaba de ser contratada como agente de IA de {clinic_name}.
...
"Hola, bienvenido a Clínica X, en qué te puedo ayudar?" ← NUNCA responder así al DUEÑO"""
        ...
        if not response or not response.strip():
            # LLM respondió vacío — no inventar texto
            import logging as _log
            _log.getLogger("bublee.admin").error(f"[admin] LLM devolvió respuesta vacía para {chat_id}")
            raise RuntimeError(f"LLM devolvió respuesta vacía al admin engine (provider desconocido)")
        ...
        for att in attachments:
            ...
            if not raw and att.get("file_id") and att.get("platform") == "telegram":
                try:
                    ...
                    log.warning(f"[admin] telegram binary download falló: {_e}")
            if not raw and att.get("media_id") and att.get("platform") == "whatsapp_cloud":
                try:
                    ...
                    log.warning(f"[admin] whatsapp cloud binary download falló: {_e}")
...
class AuthEngine:
    ...
    async def _start_activation(self, chat_id: str, token_raw: str) -> List[str]:
        from bublee import db
        from bublee_utils import is_admin_activation_token
        ...
        if token_type == "admin_pro":
            return ["Código Bublee Pro válido. Cómo te llamas?"]
    ...
    async def _handle_activation_flow(self, chat_id: str, text: str, session: Dict) -> List[str]:
        ...
        if step == "confirm":
            ...
            if token_type == "admin_pro":
                return [
                    "Listo. Bublee Pro Admin quedó activado para este chat.",
