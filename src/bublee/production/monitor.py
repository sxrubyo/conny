...
class BubleeProduction:
    ...
    async def handle(self, chat_id: str, text: str, clinic: Dict,
        ...
        try:
            ...
            if should_esc or wants_human:
                ...
                if admin_ids:
                    ...
                    try:
                        await self.bublee._send_message(admin_jid_esc, alert)
        ...
        sys_prompt = f"""Eres Bublee, agente de IA de {clinic_name}.
...
- NUNCA te presentes con "Soy Bublee tu recepcionista de X" — eso suena a bot
- Si es el primer mensaje, saluda así: "{time_greeting}! hablas con Bublee 😊 ||| en qué te puedo ayudar?"
...
- NUNCA digas el nombre completo de la clínica en el saludo — el paciente ya sabe dónde escribió"""
        ...
        if llm_engine:
            try:
                ...
                # NO hardcoded fallback — propagar el error al caller
                raise
        else:
        ...
        try:
            ...
            if confidence < 0.5 and admin_jid and not has_relevant_teaching:
                ...
                try:
                    await self.bublee._send_message(admin_jid, alert_msg)
