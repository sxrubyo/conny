#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# conny-cli — Deploy y gestión de instancias Conny
#
# Uso:
#   ./conny-cli nuevo-cliente            → crea nueva instancia interactivamente
#   ./conny-cli nuevo-cliente mi-clinica → nombre directo
#   ./conny-cli listar                   → ver todas las instancias activas
#   ./conny-cli estado mi-clinica        → ver estado de una instancia
#   ./conny-cli detener mi-clinica       → detener instancia
#   ./conny-cli reiniciar mi-clinica     → reiniciar instancia
#   ./conny-cli eliminar mi-clinica      → eliminar instancia completamente
#   ./conny-cli logs mi-clinica          → ver logs en vivo
# ═══════════════════════════════════════════════════════════════════════════════
...
instance_db()   { echo "$INSTANCES_DIR/$1/conny.db"; }
pm2_name()      { echo "conny-$1"; }
...
cmd_nuevo_cliente() {
    ...
    if [ -d "$DIR" ]; then
        ...
        info "Usa: ./conny-cli estado $NAME"
        exit 1
    fi
    ...
    # ── Master API Key ────────────────────────────────────────────────────────
    MASTER_KEY="conny_$(openssl rand -hex 8 2>/dev/null || echo "${NAME}_$(date +%s)")"
    ...
    # 2. Copiar archivos de Conny (sin DB, sin .env — sesión nueva limpia)
    for f in conny.py search.py knowledge_base.py requirements.txt; do
        if [ -f "$CONNY_TEMPLATE/$f" ]; then
            cp "$CONNY_TEMPLATE/$f" "$DIR/$f"
        fi
    done
    success "Archivos copiados"

    # 3. Copiar .env base y personalizarlo
    cp "$CONNY_TEMPLATE/.env" "$DIR/.env.base" 2>/dev/null || true
    ...
    if [ -f "$CONNY_TEMPLATE/.env" ]; then
        GROQ_KEY=$(grep "^GROQ_API_KEY=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY=$(grep "^GEMINI_API_KEY=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_2=$(grep "^GEMINI_API_KEY_2=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_3=$(grep "^GEMINI_API_KEY_3=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_4=$(grep "^GEMINI_API_KEY_4=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_5=$(grep "^GEMINI_API_KEY_5=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_6=$(grep "^GEMINI_API_KEY_6=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        GEMINI_KEY_7=$(grep "^GEMINI_API_KEY_7=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        OPENROUTER_KEY=$(grep "^OPENROUTER_API_KEY=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        OPENAI_KEY=$(grep "^OPENAI_API_KEY=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        SERP_KEY=$(grep "^SERP_API_KEY=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
        BRAVE_KEY=$(grep "^BRAVE_API_KEY=" "$CONNY_TEMPLATE/.env" | cut -d= -f2-)
    fi

    # Escribir .env limpio para esta instancia
    cat > "$DIR/.env" << ENVEOF
# ═══════════════════════════════════════════════════════════════════════════════
# Conny — Instancia: $NAME
...
HOST=0.0.0.0
BASE_URL=$BASE_URL
WEBHOOK_SECRET=conny_${NAME}_$(openssl rand -hex 4 2>/dev/null || echo "secret")

# Base de datos — aislada por cliente
DB_PATH=$DIR/conny.db
...
ENVEOF
    ...
    pm2 start "$DIR/conny.py" \
        --name "$PM2_NAME" \
        --interpreter "python3" \
        --restart-delay=3000 \
        --max-restarts=10 \
        --log "$DIR/logs/conny.log" \
    ...
    echo    "  ./conny-cli logs $NAME       → logs en vivo"
    echo    "  ./conny-cli estado $NAME     → estado"
    echo    "  ./conny-cli detener $NAME    → detener"
    echo ""
...
cmd_listar() {
    header "Instancias Conny activas"
    ...
    if [ ! -d "$INSTANCES_DIR" ] || [ -z "$(ls -A "$INSTANCES_DIR" 2>/dev/null)" ]; then
        ...
        echo "  Crea una con: ./conny-cli nuevo-cliente"
        echo ""
        return
    fi
    ...
    echo ""
    echo "  Para gestionar: ./conny-cli [estado|detener|reiniciar|logs|eliminar] [nombre]"
    echo ""
}
...
cmd_logs() {
    ...
    pm2 logs "$(pm2_name "$name")" 2>/dev/null || \
        tail -f "$(instance_dir "$name")/logs/conny.log" 2>/dev/null || \
...
case "$COMMAND" in
    "chat"|"conny-chat")
        ...
        CHAT_SCRIPT="$(dirname "$0")/conny-chat.py"
        if [ ! -f "$CHAT_SCRIPT" ]; then
            error "conny-chat.py no encontrado en $(dirname "$0")"
            exit 1
        fi
    ...
    *)
        echo ""
        echo -e "${PURPLE}  conny-cli${NC} — Gestión de instancias Conny"
        echo ""
        echo -e "  ${CYAN}Comandos:${NC}"
        echo    "  chat [nombre]             Hablar con Conny en lenguaje natural"
        ...
        echo    "  ./conny-cli chat                    → hablar con la instancia base"
        echo    "  ./conny-cli chat clinica-bella      → hablar con instancia específica"
        echo    "  ./conny-cli nuevo-cliente           → crear nuevo cliente"
        echo    "  ./conny-cli listar"
        echo ""
        ;;
esac
