...
import bublee_bridge as bridge_module  # noqa: E402
...
def build_bridge(tmp_path: Path) -> tuple[bridge_module.BubleeBridge, bridge_module.BridgeStore, Path]:
    ...
    bridge = bridge_module.BubleeBridge(
        store,
        FakeTransport(),
        logger,
...
def test_bridge_persists_turns_and_exports_session(tmp_path):
    ...
    try:
        assert bridge.process_line("Hola Bublee") == "ok"
        ...
        assert payload["turns"][0]["content"] == "Hola Bublee"
    finally:
        bridge.logger.close()
