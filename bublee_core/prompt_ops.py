...
from bublee_core.identity_policy import compact_identity_rule, off_topic_identity_rule
...
def build_business_context(clinic: dict, db, instance_path: str = None) -> str:
    ...
    if instance_path:
        ...
        paths_to_try = [
            ...
            Path(f"/home/ubuntu/bublee/instances/{instance_path}/knowledge"),
...
def build_compact_system_prompt(
    *,
    ...
    if is_first_turn:
        continuity_block = (
            "primer turno: saluda solo si la persona apenas saludo; si ya trajo una necesidad, entra directo a esa necesidad."
        )
    else:
        continuity_block = (
            "ya vienen hablando: NO saludes de nuevo, NO digas hola, NO te presentes de nuevo, "
            "NO repitas soy Bublee y NO cierres con en que puedo ayudarte. responde el ultimo mensaje."
        )
    ...
    human_mandates = [
        ...
        "Escribe corto, natural y directo por WhatsApp.",
        "Usa ||| para separar ideas en mensajes distintos.",
        "No inventes datos del negocio: usa solo informacion confirmada por el admin, la ficha, KB o reglas cargadas."
    ]

    tone_block = truncate_block(getattr(personality, "tone_instruction", "") or "", 360)
    kb_block = truncate_block(kb_context, 4000)
    strategy = truncate_block(reasoning.get("response_strategy", ""), 220)
    pre_prompt_injection = truncate_block(pre_prompt_injection, 4000)

    negocio_lines = [f"Eres {agent_name}, asesora conversacional de {clinic_name}"]
...
def build_system_prompt(
    *,
    ...
    off_topic_block = (
        off_topic_identity_rule(agent_name)
    )
    ...
    if is_first_turn:
        continuity_block = (
            "primer turno: si solo saludo, saluda breve y pregunta algo util; si ya dijo una necesidad, responde esa necesidad sin presentarte largo."
        )
    else:
        continuity_block = (
            "turno de continuidad: NO saludes otra vez, NO digas hola, NO te presentes otra vez, "
            "NO repitas soy Bublee, NO reinicies con en que puedo ayudarte. contesta lo ultimo recordando lo anterior."
        )
