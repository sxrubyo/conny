#!/usr/bin/env python3
"""
bublee-omni.py — El Ojo que Todo lo Ve v2.0

Centro de comando y control para todas las instancias Bublee.
...
MODOS:
  python3 bublee-omni.py server    →  servidor completo (puerto 9001)
  python3 bublee-omni.py chat      →  chat terminal con Omni
  python3 bublee-omni.py status    →  estado rápido
  python3 bublee-omni.py watch     →  monitor live
  python3 bublee-omni.py dashboard →  dashboard en terminal
  python3 bublee-omni.py report    →  generar reporte
  python3 bublee-omni.py analyze   →  análisis profundo
  python3 bublee-omni.py alerts    →  gestionar alertas
  python3 bublee-omni.py logs      →  ver audit log
"""
...
try:
    import readline
    HIST_FILE = Path.home() / ".bublee" / "omni_history"
...
# ══════════════════════════════════════════════════════════════════════════════
OMNI_VERSION = "2.0.0"
TEMPLATE_DIR = os.getenv("BUBLEE_DIR", "/home/ubuntu/bublee")
INSTANCES_DIR = os.getenv("INSTANCES_DIR", "/home/ubuntu/bublee/instances")
...
if not OMNI_KEY:
    # Antes: default fijo "omni_secret_change_me" — al estar en el código
    # fuente, es una credencial pública. Cualquiera con acceso al repo
    # podía autenticarse contra X-Omni-Key y controlar el panel (reiniciar
    # instancias, etc). Ahora se genera una random por sesión en vez de
    # usar un valor adivinable, y se avisa fuerte para que quede fija.
    OMNI_KEY = secrets.token_hex(24)
    print(f"[omni] ADVERTENCIA DE SEGURIDAD: OMNI_KEY no está seteada en el .env.")
    print(f"[omni] Se generó una temporal solo para esta sesión: {OMNI_KEY}")
    print(f"[omni] Agrégala a tu .env como OMNI_KEY={OMNI_KEY}")
    print(f"[omni] Si no la fijás, cambia en cada reinicio y cualquier integración con la key vieja deja de funcionar.")
...
# Base de datos
OMNI_DB = Path.home() / ".bublee" / "omni.db"
...
def print_logo(compact=False):
    print()
    if compact:
        print(f"  {q(C.AMB, '◉', bold=True)}  {q(C.W, 'bublee omni', bold=True)}  "
...
async def send_email(subject: str, body: str, to: str = None):
    ...
    try:
        ...
        msg['From'] = SMTP_USER
        msg['To'] = recipient
        msg['Subject'] = f"[Bublee Omni] {subject}"
...
async def execute_workflow_action(action: Dict, context: Dict):
    ...
    if action_type == "notify":
        ...
        if instance_name:
            pm2_name = "bublee" if instance_name == "base" else f"bublee-{instance_name}"
...
async def omni_brain(user_input: str, all_status: List[Dict], base_env: dict) -> str:
    ...
    system = f"""Eres Bublee Omni — el centro de comando de Santiago para toda la plataforma Bublee.
...
IMPORTANTE — Solo incluye una ACTION si Santiago te lo pide explícitamente o si detectas un problema real que requiere acción inmediata. NO incluyas ACTION:summary_all ni ninguna otra acción por defecto en cada respuesta. Si Santiago solo pregunta o saluda, responde con texto únicamente."""
    ...
    if not providers:
        return "No hay LLMs configurados en el .env de Bublee."
...
if HAS_FASTAPI:
    ...
    omni_app = FastAPI(title="Bublee Omni", version=OMNI_VERSION, lifespan=lifespan)
    ...
    async def dashboard_html():
        ...
        <html>
        <head>
            <title>Bublee Omni Dashboard</title>
        ...
        <body>
            <div class="container">
                <h1><span class="logo">◉</span> Bublee Omni</h1>
                ...
                <div class="footer">
                    Bublee Omni v{OMNI_VERSION} · Auto-refresh cada 30s · {datetime.now().strftime('%H:%M:%S')}
                </div>
            </div>
        </body>
        </html>
...
def main():
    ...
    if cmd == "status":
        ...
        if len(sys.argv) < 4:
            fail("Uso: bublee-omni.py demo <instancia> <on|off|status>")
            sys.exit(1)
        ...
        if _tok:
            ...
            ok(f"  Telegram: @adbubleebot → chat {_cid or '(sin chat_id)'}")
        else:
        ...
        if len(sys.argv) < 3:
            fail("Uso: bublee-omni.py delete-alert <id>")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            fail("Uso: bublee-omni.py metrics <instancia> [horas]")
            sys.exit(1)
        ...
        section("Comandos")
        for c, d in cmds:
            print(f"    {q(C.P2, f'bublee-omni.py {c:<24}', bold=True)}{q(C.G2, d)}")
        ...
        examples = [
            "bublee-omni.py server",
            "bublee-omni.py chat",
            "bublee-omni.py analyze clinica-bella",
            "bublee-omni.py metrics base 48",
            "bublee-omni.py create-alert",
        ]
        for ex in examples:
            print(f"    {q(C.G1, ex)}")
        ...
        print(f"bublee-omni {OMNI_VERSION}")
    
    else:
        fail(f"Comando desconocido: '{cmd}'")
        info("Usa 'bublee-omni.py help' para ver comandos")
        sys.exit(1)
