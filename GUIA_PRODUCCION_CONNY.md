...
Guía corta para operar Conny esta noche sin mezclar promesas con realidad.
...
- Conny conserva el archivo original y el texto extraído cuando aplica
- la carpeta se copia con `conny new`, `conny clone` y `conny sync`
...
- El enrutado correcto se define con `conny pair`.

Comandos útiles:

```bash
conny pair list
conny pair default clinica-de-las-americas
conny pair clinica-de-las-americas 123456789
conny pair detach 123456789
```
...
- si el admin pide cambio de tono, Conny debe ajustarse
- si algo no se entiende, debe pedir una aclaración corta

## Burbujas y buffering

Conny no responde una burbuja por una burbuja cuando el contexto llega partido.
...
`instance.json`, Conny ahora rellena automáticamente:
...
- no asumir que Conny ve todo tu historial de Gmail si no existe integración explícita
