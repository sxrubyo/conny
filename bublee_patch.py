#!/usr/bin/env python3
"""
bublee_patch.py — Aplica todos los fixes de producción a bublee.py
...
Uso: python3 bublee_patch.py /home/ubuntu/bublee/bublee.py
"""
...
def apply_bublee_patch(target_path: str | None = None):
    if target_path is None:
        target_path = str(Path(__file__).resolve().parent / "bublee.py")
    ...
    # Esto hace que Bublee aparezca online inmediatamente cuando llega un mensaje
    # ══════════════════════════════════════════════════════════════════════════════
    ...
    print(f"\nSiguiente paso:")
    print(f"  pm2 restart bublee --update-env")
    print(f"  pm2 logs bublee --lines 20 --nostream")
    return True


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python3 bublee_patch.py /ruta/a/bublee.py")
        sys.exit(1)
    apply_bublee_patch(sys.argv[1])
