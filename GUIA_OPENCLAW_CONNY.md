...
Conny ya soporta un patrón inspirado en `Openclaw` para Telegram:

- un solo `TELEGRAM_TOKEN` en la Conny base
...
La Conny base puede actuar como router compartido si en su `.env` tiene:
...
Conny resuelve el destino en este orden:
...
`/home/ubuntu/conny/shared_telegram_routes.json`
...
. .venv/bin/activate
python3 conny_cli.py pair list
```

Asignar un `chat_id` a una instancia:

```bash
python3 conny_cli.py pair clinica-de-las-americas 123456789
```

Definir la instancia por defecto:

```bash
python3 conny_cli.py pair default clinica-de-las-americas
```

Quitar una ruta:

```bash
python3 conny_cli.py pair detach 123456789
```

## Cambio de modelo

Conny ya acepta cambios de modelo en tres formas:
...
python3 - <<'PY'
import conny
print(conny._resolve_shared_telegram_target('555123'))
PY
```
...
for t in tests:
    print(t, '=>', conny.extract_model_request_from_text(t))
PY
```
...
```bash
pm2 restart conny-clinica-de-las-americas
...
- Conny entiende cambios de modelo en lenguaje natural
